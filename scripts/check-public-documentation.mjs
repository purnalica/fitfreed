...
const documentContracts = Object.freeze({
  ...
  linuxUserGuide: {
    ...
    evidence: (version) => [
      ...
      [/automatic recovery/i, "update recovery guidance"],
      [/removes\s+the\s+application[\s\S]*retains\s+the\s+separate\s+local\s+library/i, "removal retention boundary"],
      [/ARM64[\s\S]*AppImage[\s\S]*RPM[\s\S]*Flatpak[\s\S]*Snap/, "unsupported Linux boundary"],
      [/GitHub private vulnerability reporting|SECURITY\.md/, "security reporting route"],
    ],
  },
  windowsUserGuide: {
    path: (version) => `docs/user/public-windows-${version}.md`,
    title: (version) => `Public Windows ${version} Guide`,
    headings: [
      "Status",
      "Preserve the source data first",
      "Download and verify",
    ...
    evidence: (version) => [
      [/public unsigned Windows preview/i, "public unsigned Windows preview status"],
      [new RegExp("immutable `v" + escapeRegExp(version) + "`"), "exact immutable release tag"],
      [/x86-64 editions of Windows 11/, "supported Windows boundary"],
      [/GitHub-hosted `windows-2025`[\s\S]*not exact Windows 11 client/i, "hosted admission limitation"],
      [new RegExp(`FitFreed_${escapeRegExp(version)}_x64-setup\\.exe`), "exact NSIS setup"],
      [/SHA256SUMS\.minisig/, "detached checksum signature"],
      [/Get-AuthenticodeSignature[\s\S]*NotSigned/, "declared absent Authenticode identity"],
      [/unknown publisher/i, "unknown-publisher warning"],
      [/A SmartScreen reputation warning is not a trust result/, "SmartScreen interpretation"],
      [/Do not disable system-wide protections/i, "native-protection boundary"],
      [/current-user installation/, "current-user installation"],
      [/bundled WebView2 offline installer/i, "offline first-launch dependency"],
      [/English \(United States\) and Spanish \(Spain\)/, "initial locale boundary"],
      [/exact reimport does not duplicate canonical history/i, "reimport behavior"],
      [/complete-history training exploration/i, "implemented training exploration"],
      [/self-contained HTML report export/i, "implemented report export"],
      [/has no account, analytics, or synchronization service/i, "local-first privacy boundary"],
      [/%APPDATA%[\s\S]*org\.fitfreed\.desktop/, "Windows library location"],
      [/automatic recovery/i, "update recovery guidance"],
      [/removes\s+the\s+application[\s\S]*retains\s+the\s+separate\s+local\s+library/i, "removal retention boundary"],
      [/Windows 10[\s\S]*Windows on ARM[\s\S]*MSI[\s\S]*per-machine/, "unsupported Windows boundary"],
      [/GitHub private vulnerability reporting|SECURITY\.md/, "security reporting route"],
    ],
  },
  operations: {
    ...
    evidence: () => [
      ...
      [/GitHub-hosted `windows-2025`/i, "hosted Windows preview boundary"],
      [/no additional protected environment[\s\S]*Authenticode/i, "authority-free Windows builder"],
      [/public-unsigned-preview/, "explicit Windows preview trust"],
      [/complete-platform manifest version 8/i, "complete Windows preview candidate contract"],
      [/HARICA activation requires a later ADR/i, "deferred Windows signing authority"],
      [/Retired SignPath commands are historical compatibility code/i, "retired SignPath boundary"],
    ],
  },
  releaseArchitecture: {
    path: () => "docs/development/public-release.md",
    title: () => "Public Release Architecture",
    headings: [
      "Current boundary",
    ...
    evidence: () => [
      [/manifest version 8/i, "complete Windows preview manifest"],
      [/no additional signing authority/i, "authority-free Windows preview"],
      [/does not claim exact[\s\S]*Windows 11/i, "hosted Windows limitation"],
  ...
  windowsManualEvaluation: {
    ...
    evidence: () => [
      ...
      [/GitHub-hosted `windows-2025`[\s\S]*does not establish exact Windows 11 client admission/i, "hosted native boundary"],
      [/production package\s+intentionally contains no WebDriver instrumentation/, "separate behavior evidence"],
      [/does not require a second product-owner evaluation/i, "unchanged product-experience boundary"],
...
const supportingDocumentEvidence = Object.freeze({
  ...
  "CODE_SIGNING.md": () => [
    /^# Code signing policy$/m,
    /no Foundation\s+certificate or active free signing sponsorship/i,
    /ADR 0050[\s\S]*retired SignPath/i,
    /purnalica\/fitfreed/,
    /Windows package contract[\s\S]*setup[\s\S]*installed `fitfreed\.exe`[\s\S]*installed `uninstall\.exe`/i,
    /unsigned Windows preview[\s\S]*unknown publisher[\s\S]*Smart App Control/i,
    /HARICA Code Signing IV/i,
    /\*\*Authors:\*\*[\s\S]*\*\*Reviewers:\*\*[\s\S]*\*\*Approvers:\*\*/,
    /multi-factor authentication/i,
    /retired SignPath GitHub connector/i,
    /Every production signing request required manual approval/,
    /hardware security module/,
    /no account, analytics, telemetry, advertising, or synchronization service/i,
    /no more than once every 24 hours/,
    /current user[\s\S]*registered\s+uninstaller/i,
  ],
});
...
const requiredCatalogKeys = Object.freeze([
  ...
  "settings.workspaces.help",
  "settings.windowsHelp.heading",
  "settings.windowsHelp.intro",
  "settings.windowsHelp.supportedLabel",
  "settings.windowsHelp.supported",
  "settings.windowsHelp.install.summary",
  "settings.windowsHelp.install.verify",
  "settings.windowsHelp.install.smartScreen",
  "settings.windowsHelp.install.firstLaunch",
  "settings.windowsHelp.update.summary",
  "settings.windowsHelp.update.ordinary",
  "settings.windowsHelp.update.recovery",
  "settings.windowsHelp.update.offline",
  "settings.windowsHelp.removal.summary",
  "settings.windowsHelp.removal.application",
  "settings.windowsHelp.removal.library",
  "settings.windowsHelp.unsupported.summary",
  "settings.windowsHelp.unsupported.body",
  "settings.windowsHelp.privacy",
  "updates.heading",
  "updates.checkNow",
