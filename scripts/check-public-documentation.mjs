...
const documentContracts = Object.freeze({
  ...
  manualEvaluation: {
    ...
    evidence: () => [
      ...
      [/Create and reopen a report[\s\S]*exact table/, "VoiceOver report evaluation"],
      [/From a training session, create a report[\s\S]*source-changing reimport/, "realistic report evaluation"],
      [/any scenario is blocked/, "fail-closed acceptance rule"],
    ],
  },
  readiness: {
    ...
    evidence: () => [
      ...
      [/Keyboard, VoiceOver, 200% scaling/, "manual accessibility gate"],
    ],
  },
});
