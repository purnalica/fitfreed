...
const schema = JSON.parse(
  readFileSync(new URL("../schemas/update-recovery-v2.schema.json", import.meta.url), "utf8"),
);
...
export function validateLinuxUpdateRecoveryV2(manifest) {
  const errors = [];
  if (!validateSchema(manifest)) {
    errors.push(
      ...validateSchema.errors.map(
        ({ instancePath, message }) =>
          `Linux update recovery v2 schema violation at ${instancePath || "/"}: ${message}`,
      ),
    );
  }
  if (manifest?.source?.nativePackage?.version !== manifest?.source?.version) {
...
export function assertLinuxUpdateRecoveryTransition(currentPhase, nextPhase) {
  const permitted = transitions.get(currentPhase);
  if (!permitted || !transitions.has(nextPhase) || !permitted.has(nextPhase)) {
    throw new Error(`invalid Linux update recovery transition: ${currentPhase} -> ${nextPhase}`);
  }
  return nextPhase;
}
