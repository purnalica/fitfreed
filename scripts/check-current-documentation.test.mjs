...
test("accepts current documentation derived from the release compatibility source", () => {
  assert.deepEqual(
    validateCurrentDocumentation(loadCurrentDocumentation(repositoryRoot)),
    {
      ...
      currentLibrarySchemaVersion: 24,
      supportedLibrarySchemaVersions: Array.from({ length: 24 }, (_, index) => index + 1),
      checkedDocuments: 8,
    },
  );
});
...
test("rejects stale storage, report, and release-readiness claims together", () => {
  ...
  assert.throws(
    ...
    (error) => {
      assert.match(error.message, /storage architecture does not identify SQLite schema 24/);
