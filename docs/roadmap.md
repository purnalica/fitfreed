...
The active stage is the production migration defined by the accepted [UI and UX redesign plan](plans/ui-redesign.md) and [MVP redesign production migration plan](plans/mvp-redesign-production-migration.md). X3 is accepted; X4 has converted the contract into functional vertical increments, and X5 implementation begins with the truthful public entrance before migrating the ordinary application. Functional growth and public-candidate preparation remain frozen. The verified application remains the only production implementation and the [MVP experience delivery plan](plans/mvp-experience-delivery.md) remains its engineering evidence baseline. Milestone 3 resumes only after the selected direction is implemented and the complete product journey passes renewed independent experience acceptance.
