...
Milestone 1 and Milestone 2 capability implementation and the current Milestone 3 public-release engineering baseline are complete. Apple Silicon on macOS 15.0 or later is the accepted MVP platform boundary. Before activating production trust or preparing a public candidate, the project is executing the production UI and UX intervention through the [MVP experience delivery plan](plans/mvp-experience-delivery.md). D0 accepted the ordered evidence report with self-contained HTML export, the attributed deep-session workspace with routed and non-routed verticals, reusable segmentation, and user-authored sport classification. P1 published the truthful canonical product site with exact remote verification and no premature download action. E1, E2, all three E3 slices, all seven E4 slices, and all six E5 verticals have complete local and hosted evidence; every previously planned E6 correction and its exact-source local and hosted campaign are complete. The subsequent [independent product-experience audit](research/mvp-product-experience-audit.md) reopened E6 acceptance with three release-blocking findings, six material findings, and one advisory. PX-03 is resolved by a ten-year supported-signal acceptance gate and compact recoverable SQLite schema 24 with exact clean local and hosted evidence; the remaining findings retain their original audit disposition pending product-owner review. The E3 journey provides sport classification, indexed full-history search, chronology, source-separated calendar discovery, two-through-four-session comparison, exact lightweight return, and restart restoration. E4 exposes provider-neutral recorded structure, primary and transition route geometry, bounded private local traces, supported gap-aware time series, paginated exact evidence, recorded heart-rate, speed, and power zones, reusable user-authored segments, and session provenance. E5 lets a session become a durable personal interpretation, compose its route without exporting recorded coordinates, answer one versioned user-selected training-period comparison through findings, exact tables, accessible charts, and coverage, start a durable result from a question, an exact completed exploration, a session, or a blank page, deliberately refresh stale evidence through an exact current-candidate review without rewriting authorship, and navigate between a saved report and its exact source without mutating resumable exploration. Deterministic self-contained HTML is verified outside the application and source changes remain explicit after reimport. Milestone 3 release acceptance remains active but externally gated until the reopened experience outcomes pass: deterministic trust, candidate, publication, recovery, user, maintainer, and readiness paths pass local and hosted automation for their tested boundaries with production trust deliberately inactive, while production authority, protected GitHub configuration, exact-candidate evaluation, and public-release publication remain open in the consolidated readiness ledger.
