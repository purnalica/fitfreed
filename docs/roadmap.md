...
The active stage is therefore the design-first intervention in the [UI and UX redesign plan](plans/ui-redesign.md). Functional growth and public-candidate preparation are frozen until an explicit experience model, information and state architecture, competing interaction directions, and a coherent reviewed design contract exist. The verified application remains the only production implementation and the [MVP experience delivery plan](plans/mvp-experience-delivery.md) remains its engineering evidence baseline. Milestone 3 resumes only after the selected direction is implemented and the complete product journey passes renewed independent experience acceptance.
...
**Execution:** X0 through X3 produce and validate the experience model and design contract before production presentation changes. X4 derives replacement increments and X5 implements them through the ordinary application with TDD and complete lower-layer, UI, test, documentation, and independent product-experience evidence. The detailed gates are canonical in the [UI and UX redesign plan](plans/ui-redesign.md); the earlier E1 through E6 sequence remains the implementation and evidence baseline it supersedes for presentation direction.
