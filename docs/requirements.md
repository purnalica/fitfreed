...
- Ordinary session discovery will use locale-appropriate human-scale dates and quantities and will compose each session card only from evidence that is actually available. Its bounded calendar projection will show each activity in local-start order with recognizable sport identity, duration, and direct exact-session navigation; represented-year selection and a direct return to the current month will avoid month-by-month traversal of long histories. Exact timestamps, source separation, unrounded values, and provenance will remain reachable through deliberate evidence inspection rather than dominating the first scan.
