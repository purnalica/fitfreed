...
Download only from the immutable `FitFreed 0.1.12` Release in the [canonical GitHub repository](https://github.com/purnalica/fitfreed/releases). The Linux release set must contain these regular assets:

- `FitFreed_0.1.12_amd64.deb` and its updater signature `FitFreed_0.1.12_amd64.deb.sig`;
- `FitFreed_0.1.12_amd64.deb.inventory.json`;
...
Every line must report `OK`. The manifest, package inventory, checksum file, release-note version, Debian filename, and GitHub tag must all identify 0.1.12. A mismatch is a hard stop; do not rename an asset or edit the inventory to make the check pass.
...
gh release verify-asset v0.1.12 FitFreed_0.1.12_amd64.deb \
