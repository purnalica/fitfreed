...
9. Import the same package again and confirm that the application reports an exact repeat, reuses complete coverage evidence, and does not duplicate history.
10. Import `.artifacts/e2e/fixtures/invalid.zip` and confirm that the package is rejected with localized recovery guidance, one invalid daily-activity artifact, its family-level explanation, and no history change.
11. Import `.artifacts/e2e/fixtures/overlap.zip` and confirm that the existing history remains, the new day is added, and overview aggregates change accordingly.
12. Change between English (US) and Spanish (Spain) and verify the outcome, guidance, family names, reasons, next actions, filters, ranges, detail, availability, dates, and exact step formatting.
13. Close and reopen the application and confirm that the selected language and localized default overview are restored.
...
- The overview selects at most the latest 30 local dates by default and accepts an explicit range of at most 366 inclusive dates. Period comparison, training, sleep, recovery, user-controlled backup, portable export, and updates are not implemented yet.
