# Public Linux 0.1.11 Guide

## Status

This inactive pre-publication guide is rendered against the assigned 0.1.11 expansion source so its exact names,
...
Download only from the immutable `FitFreed 0.1.11` Release in the [canonical GitHub repository](https://github.com/purnalica/fitfreed/releases). The Linux release set must contain these regular assets:

- `FitFreed_0.1.11_amd64.deb` and its updater signature `FitFreed_0.1.11_amd64.deb.sig`;
- `FitFreed_0.1.11_amd64.deb.inventory.json`;
...
Every line must report `OK`. The manifest, package inventory, checksum file, release-note version, Debian filename, and GitHub tag must all identify 0.1.11. A mismatch is a hard stop; do not rename an asset or edit the inventory to make the check pass.
...
gh release verify-asset v0.1.11 FitFreed_0.1.11_amd64.deb \
...
1. Open the verified `FitFreed_0.1.11_amd64.deb` in Ubuntu App Center or the graphical software installer.
2. Confirm the proposed application is named FitFreed, identifies version 0.1.11, and comes from the package just verified.
...
5. Confirm **Settings → Updates** identifies version 0.1.11 before importing anything.
