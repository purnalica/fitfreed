...
Windows 0.1.5 release. No public Windows binary is available while the
...
immutable `v0.1.5` GitHub Release from `purnalica/fitfreed` when that is the version assigned to the expansion; source
archives, development packages, Actions artifacts, forks, and third-party packages are not that release.

FitFreed 0.1.5 will support x86-64 editions of Windows 11 that remain in Microsoft support when the exact candidate is
...
source export, and version 0.1.5 has no supported user-controlled library backup, restore, or portable normalized
export workflow.
...
Download only from the immutable `FitFreed 0.1.5` Release in the
...
- `FitFreed_0.1.5_x64-setup.exe` and its updater signature `FitFreed_0.1.5_x64-setup.exe.sig`;
- `FitFreed_0.1.5_x64-setup.exe.inventory.json` and `FitFreed_0.1.5_x64-setup.exe.build.json`;
...
```powershell
$package = "FitFreed_0.1.5_x64-setup.exe"
...
release-note version, setup filename, and GitHub tag must all identify 0.1.5. A mismatch is a hard stop; do not rename
...
```powershell
$signature = Get-AuthenticodeSignature -LiteralPath ".\FitFreed_0.1.5_x64-setup.exe"
...
gh release verify-asset v0.1.5 FitFreed_0.1.5_x64-setup.exe --repo purnalica/fitfreed
```

## Install and first launch

1. Double-click the verified `FitFreed_0.1.5_x64-setup.exe`.
...
Version 0.1.5 provides local activity, sleep, recovery, and longitudinal views; complete-history training exploration
...
There is no in-application deletion command in 0.1.5. Never delete only the SQLite file, its sidecars, or selected
