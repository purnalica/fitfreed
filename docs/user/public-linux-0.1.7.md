# Public Linux 0.1.7 Guide

## Status

This inactive pre-publication guide is rendered against the repository's current unreleased 0.1.7 development
version so its exact names, commands, links, and documentation contracts can be verified. It does not announce or
reserve a Linux 0.1.7 release. No public Linux binary is available while the
...
piece of evidence together. This guide becomes operative only for Linux assets in an immutable `v0.1.7` GitHub
...
Download only from the immutable `FitFreed 0.1.7` Release in the [canonical GitHub repository](https://github.com/purnalica/fitfreed/releases). The Linux release set must contain these regular assets:

- `FitFreed_0.1.7_amd64.deb` and its updater signature `FitFreed_0.1.7_amd64.deb.sig`;
- `FitFreed_0.1.7_amd64.deb.inventory.json`;
...
Every line must report `OK`. The manifest, package inventory, checksum file, release-note version, Debian filename, and GitHub tag must all identify 0.1.7. A mismatch is a hard stop; do not rename an asset or edit the inventory to make the check pass.
...
```sh
gh release verify v0.1.7 --repo purnalica/fitfreed
gh release verify-asset v0.1.7 FitFreed_0.1.7_amd64.deb \
...
1. Open the verified `FitFreed_0.1.7_amd64.deb` in Ubuntu App Center or the graphical software installer.
2. Confirm the proposed application is named FitFreed, identifies version 0.1.7, and comes from the package just verified.
...
5. Confirm **Settings → Updates** identifies version 0.1.7 before importing anything.
...
There is no in-application deletion command in 0.1.7. Never delete only the SQLite file, its sidecars, or selected recovery files as a repair technique.
