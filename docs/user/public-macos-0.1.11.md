# Public macOS 0.1.11 Guide

## Status

This pre-publication guide describes the macOS member of the proposed 0.1.11 macOS-plus-Linux release. It becomes the
operative version-matched guide only if an immutable `v0.1.11` [FitFreed public Release](https://github.com/purnalica/fitfreed/releases)
...
FitFreed 0.1.11 supports Apple Silicon on macOS 15.0 or later. It is experimental GPL-3.0-or-later software provided without warranty and at the user's own risk. Read the [project disclaimer](../../DISCLAIMER.md) before installation.

## Preserve the source data first

Keep every original provider ZIP unchanged in an independently protected location. FitFreed does not replace the source export, and version 0.1.11 has no supported user-controlled library backup, restore, or portable normalized export workflow.
...
Download only from the immutable `FitFreed 0.1.11` Release in the [canonical GitHub repository](https://github.com/purnalica/fitfreed/releases). The release must contain these regular assets:

- `FitFreed_0.1.11_aarch64.dmg`;
- `FitFreed_0.1.11_aarch64.app.tar.gz` and its `.sig` file;
...
Every line must report `OK`. The manifest, checksum file, release-note version, DMG name, and GitHub tag must all identify 0.1.11. A mismatch is a hard stop; do not rename an asset or edit the inventory to make the check pass.
...
```sh
gh release verify v0.1.11 --repo purnalica/fitfreed
gh release verify-asset v0.1.11 FitFreed_0.1.11_aarch64.dmg \
...
1. Open the verified `FitFreed_0.1.11_aarch64.dmg`.
2. Drag `FitFreed.app` to the Applications destination shown by the disk image.
3. Eject the disk image.
4. Open FitFreed from Applications.
5. Confirm the interface identifies version 0.1.11 before importing anything.
...
Version 0.1.11 provides provider-neutral views for:
...
There is no in-application deletion command in 0.1.11. Never delete only the SQLite file, its sidecars, or selected recovery files as a repair technique.
