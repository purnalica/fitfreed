...
reserve a Linux 0.1.1 release. No public Linux binary is available while the
...
piece of evidence together. This guide becomes operative only for Linux assets in an immutable `v0.1.1` GitHub
...
Download only from the immutable `FitFreed 0.1.1` Release in the [canonical GitHub repository](https://github.com/purnalica/fitfreed/releases). The Linux release set must contain these regular assets:

- `FitFreed_0.1.1_amd64.deb` and its updater signature `FitFreed_0.1.1_amd64.deb.sig`;
- `FitFreed_0.1.1_amd64.deb.inventory.json`;
...
Every line must report `OK`. The manifest, package inventory, checksum file, release-note version, Debian filename, and GitHub tag must all identify 0.1.1. A mismatch is a hard stop; do not rename an asset or edit the inventory to make the check pass.
...
gh release verify-asset v0.1.1 FitFreed_0.1.1_amd64.deb \
...
1. Open the verified `FitFreed_0.1.1_amd64.deb` in Ubuntu App Center or the graphical software installer.
2. Confirm the proposed application is named FitFreed, identifies version 0.1.1, and comes from the package just verified.
