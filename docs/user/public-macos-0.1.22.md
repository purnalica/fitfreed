# Public macOS 0.1.22 Guide

## Status

This is the operative version-matched macOS guide for FitFreed 0.1.22 once immutable `v0.1.22` GitHub Release
exists in the [canonical repository](https://github.com/purnalica/fitfreed/releases/tag/v0.1.22). Before that exact
Release exists, this document describes a candidate and is not permission to install another build.

FitFreed 0.1.22 supports Apple Silicon on macOS 15.0 or later. It is experimental GPL-3.0-or-later software provided
...
Download only `FitFreed_0.1.22_aarch64.dmg` from immutable Release `v0.1.22`. The same Release must contain the updater
...
damaged, unidentified, differently signed, or quarantined package. Confirm FitFreed reports version 0.1.22 before
importing data.
