# Public Windows 0.1.16 Guide

## Status

This is the operative version-matched guide for the public unsigned Windows preview in FitFreed 0.1.16 once immutable `v0.1.16` GitHub Release exists in the
[canonical repository](https://github.com/purnalica/fitfreed/releases/tag/v0.1.16). Before that exact Release exists,
...
Download only `FitFreed_0.1.16_x64-setup.exe` from immutable Release `v0.1.16`. The same Release must contain:

- `FitFreed_0.1.16_x64-setup.exe.sig`;
- `FitFreed_0.1.16_x64-setup.exe.inventory.json` and `FitFreed_0.1.16_x64-setup.exe.build.json`;
...
```powershell
$package = "FitFreed_0.1.16_x64-setup.exe"
...
tag must all identify 0.1.16. GitHub CLI users can additionally run:

```powershell
gh release verify v0.1.16 --repo purnalica/fitfreed
gh release verify-asset v0.1.16 FitFreed_0.1.16_x64-setup.exe --repo purnalica/fitfreed
```

The absence of Authenticode is intentional and must be confirmed rather than mistaken for a valid publisher:

```powershell
$signature = Get-AuthenticodeSignature -LiteralPath ".\FitFreed_0.1.16_x64-setup.exe"
...
4. Launch FitFreed from the Start menu and confirm version 0.1.16 before importing data.
