...
Download only from the immutable `FitFreed 0.1.7` Release in the [canonical GitHub repository](https://github.com/purnalica/fitfreed/releases). The release must contain these regular assets:

- `FitFreed_0.1.7_aarch64.dmg`;
- `FitFreed_0.1.7_aarch64.app.tar.gz` and its `.sig` file;
...
Every line must report `OK`. The manifest, checksum file, release-note version, DMG name, and GitHub tag must all identify 0.1.7. A mismatch is a hard stop; do not rename an asset or edit the inventory to make the check pass.
...
gh release verify-asset v0.1.7 FitFreed_0.1.7_aarch64.dmg \
...
1. Open the verified `FitFreed_0.1.7_aarch64.dmg`.
2. Drag `FitFreed.app` to the Applications destination shown by the disk image.
3. Eject the disk image.
4. Open FitFreed from Applications.
5. Confirm the interface identifies version 0.1.7 before importing anything.
...
Version 0.1.7 provides provider-neutral views for:
