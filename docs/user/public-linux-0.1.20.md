# Public Linux 0.1.20 Guide

## Status

This is the operative version-matched Linux guide for FitFreed 0.1.20 once immutable `v0.1.20` GitHub Release exists
in the [canonical repository](https://github.com/purnalica/fitfreed/releases/tag/v0.1.20). Before that exact Release
exists, this document describes a candidate and is not permission to install another build.

FitFreed 0.1.20 supports x86-64 Ubuntu Desktop 24.04 and 26.04 LTS through a Debian package. It is experimental
...
Download only `FitFreed_0.1.20_amd64.deb` from immutable Release `v0.1.20`. The same Release must contain its updater
...
package digest and GitHub release provenance. Every check must identify version 0.1.20 and the exact package name.

## Install and first launch

Open the verified `FitFreed_0.1.20_amd64.deb` in Ubuntu App Center or another native graphical Debian installer and
approve the ordinary administrator authorization required by package management. Launch FitFreed from the desktop
application menu and confirm version 0.1.20 before importing data. Do not extract the package, copy files manually,
