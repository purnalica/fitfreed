...
rather than parallel CI-only test paths; Windows update and packaged capability campaigns remain later Milestone 5
boundaries.
...
the current user, verifies native identity and removal, and proves separately located application data was preserved.
Only that complete success writes the marker. The hosted Windows Server result proves native compilation, automation
portability, and engineering package behavior; it does not replace signed Windows 11 or human evidence.
