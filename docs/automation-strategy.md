...
the x86-64 MSVC desktop host through the same versioned project commands rather than parallel CI-only test paths;
Windows packaging, update, and packaged capability campaigns remain later Milestone 5 boundaries.
...
`windows-2025-x86_64-host`, and executable-input fingerprint match the current revision. Reused evidence still enters
...
workspace and pinned-updater tests, strict Clippy, and an all-target, all-feature desktop-host build. Only that complete
success writes the marker. The hosted Windows Server result proves native compilation and automation portability; it
does not replace Windows 11 package or human evidence.
