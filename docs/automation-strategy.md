...
Hosted verification separates increment confidence from candidate admission. Every revision runs the versioned impact classifier, documentation links, and independent repository-safety gate. README, product-status, static product-page, and the closed set of publication-only compositor, verifier, test, and Pages-workflow changes additionally install the locked JavaScript toolchain; verify SSOT rendering, local resources, release-state honesty, automated accessibility, compositor and update-subtree preservation, workflow topology, and publication tests; authenticate the current immutable GitHub Release and its source-bound provenance; source exact current update objects from that Release and any recovery package from its immutable predecessor Release; and compose and preflight the Pages artifact without rebuilding or repackaging unchanged application inputs. The monotonic preflight preserves identical live bytes, permits a higher authenticated sequence, and rejects erasure, replay, or same-sequence replacement. The closed documentation-verification set similarly runs its focused tests and workflow lint without entering a native package lane; those files are excluded from the application fingerprint because they cannot enter the application or release artifact. Application, shared dependency, updater-client, control-plane, or unknown changes run the complete portable quality and Linux-host checks but do not automatically start packaged platform campaigns. An explicit focused dispatch runs exactly one macOS package, complete Linux capability, Linux Insights, Linux update, Windows capability, complete Windows host, Windows package-installation, or Windows update-recovery boundary. The Linux Insights focus generates only its synthetic archive, constructs and installs the same isolated Debian package, executes only the packaged Insights scenario, preserves its synthetic library through package removal, and retains the same privacy-safe failure evidence; candidate admission continues to run all seven scenarios. The Windows package focus installs locked dependencies, constructs the release-shaped NSIS setup, and performs its native inventory cycle; it does not repeat presentation, portable automation, Rust, updater, Authenticode, packaged-capability, or complete-evidence work. The Windows update focus first proves bounded native state-lock serialization, prepared-state durability, the complete watchdog-candidate confirmation handshake under simultaneous native no-sharing handles, release of the watchdog cleanup boundary only at process exit, and acknowledgement through the native Windows recovery-state boundary. It then builds the two instrumented production-identity NSIS packages and runs only native update recovery; it cannot repeat the already accepted packaged capability journey. An explicit autonomous candidate dispatch is the only ordinary CI action that runs all admitted lanes and can write reusable complete-verification evidence. A missing comparison or unknown path fails closed to portable quality; it cannot silently claim candidate admission. Within the macOS lane, the production package and cold-launch gate precede the long synthetic import and Insights campaigns so a startup failure stops expensive downstream work early; every gate still runs after that check passes. Evidence-only documentation commits must not recursively rebuild an unchanged application merely to record the preceding successful run.

Product-site automation always completes rendering, source validation, active-Release authentication, update-snapshot
reopening, and complete-tree composition. It uploads, deploys, preflights, and remotely verifies that tree only when
the repository package version equals the authenticated immutable public Release version. A newer source version is
validated without deployment so its revision remains an unambiguous identity for the protected exact-tag release
workflow. This version gate complements, and does not replace, serialized Pages concurrency or exact-byte preflight.
...
`npm run verify:macos-public-candidate -- <candidate-directory> <version> <revision>` is the secret-free native
admission boundary for one digest-reopened initial public macOS candidate. It accepts only manifest version 3 on a
supported Apple Silicon Mac, proves that the application copied from the DMG is the sealed bundle, repeats objective
Developer ID, notarization, stapling, and Gatekeeper inspection, launches the production executable twice against one
temporary private library, removes the temporary application, and requires that library to remain integral. The
manually dispatched **Public macOS candidate admission** workflow first authenticates the exact artifact ID against a
successful protected build whose promotion is still waiting, then performs this check without secrets, publication
permission, a rebuild, or packaged E2E.
...
`.github/workflows/linux-candidate-launch-diagnostics.yml` is a secret-free, manually dispatched diagnostic boundary
for an already sealed candidate whose hosted graphical launch failed without a causal process message. It downloads
...
needs them. `npm run verify:full` runs the complete chain plus source-bound cold-launch, production-bundle, and
update-recovery-preparation evidence once from the exact clean candidate revision; it is not preceded by an equivalent
campaign. Release-workflow, release-policy, documentation, and semantic-version-only corrections use focused
contracts and the protected exact-artifact pipeline instead. A clean-gate failure receives one focused causal
correction before one new complete run.

`npm run render:product-surfaces` projects canonical public status into the README and product page;
`check:product-surfaces`, `check:site`, `check:presentation-inventory`, `check:vendored-updater`, `check:workflows`, and
the platform workflow checks reject drift in their respective boundaries. `npm run check:docs` verifies the public
documentation contracts. Continuous integration invokes the same underlying versioned tasks as separate diagnosable
steps and never uploads an unsigned package to a public release.

The exact-artifact Linux diagnostic entry points are `npm run diagnose:linux-candidate-launch -- <candidate-directory>
<version> <revision> <ubuntu-version>` and `npm run check:linux-candidate-launch-diagnostics-workflow`. They are not
part of an ordinary product, package, E2E, or performance campaign.

The production Vite build owns the progressive-startup graph boundary. It generates shell-only locale projections
from the canonical catalogs, walks the eager entry closure, and then embeds exact entry and critical-style copies in
the generated HTML. Their versioned outputs remain available to deferred chunks. Reaching the complete application,
complete application stylesheet, or either complete catalog through the eager closure fails the build; so does an
ambiguous resource boundary, missing retained asset, unsafe closing tag, or source map in an inline startup resource.
The same build command type-checks both the application project and the Node-owned Vite configuration and plugins
before composition. Packaged-configuration tests keep Tauri's automatic asset-CSP hash injection enabled and reject
an arbitrary inline-script allowance; host compilation admits only the generated startup script's exact hash.
This makes an accidental cold-launch payload regression a deterministic local and CI failure rather than relying
on a later hosted timing campaign to discover it.

The macOS cold-launch command activates the exact newly spawned production PID through `NSRunningApplication` before
waiting for the first painted shell. Registration retries are bounded and remain inside the measured interval. The
adapter never resolves an application by display name, never launches a substitute process, and requires neither
Apple Events automation nor accessibility authority.
...
The manually dispatched public workflow is a separate publication boundary. Its secret-free preflight, ephemeral authority installation, signed candidate preparation, sealed digest-bound transport, separate candidate-evaluation pause, second protected approval, GitHub attestations, exact draft promotion, Pages deployment, and remote verification reuse versioned project commands. The protected build has no publication permission; promotion receives no Apple or updater authority. During bootstrap governance, the same accountable maintainer may approve both protected deployments under [ADR 0047](architecture/decisions/0047-permit-bootstrap-solo-release-approval.md). Normal CI can parse and test every deterministic part but has no trigger, environment, key, tag, approval, or publication authority that could create a release.
