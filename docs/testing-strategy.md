...
- The framework-independent progressive startup applies the persisted locale, appearance, and zoom before the first
...
- Analytical-chart zoom tests hover and drag both rendered ECharts slider handles through sustained message-channel-separated input at zrender's stable SVG or canvas viewport root in single-series SVG, default multi-lane SVG, and maximum four-lane canvas charts. The driver follows zrender's native listener family: WebView2 receives `pointerdown`, `pointermove`, and `pointerup`, while the other admitted WebViews receive mouse events. Canvas-handle discovery admits only the bottom control band of the current canvas geometry. It identifies distinct two-dimensional accent components and selects the lowest vertically aligned pair with a credible track span; it never averages unrelated chart marks that happen to share an x-coordinate with a handle or accepts controls painted for a preceding chart height. The tests require each handle's observed visual position to move by at least 20 logical pixels and the session workspace to remain responsive; renderer fingerprints, configuration, or control presence are insufficient because hover emphasis and delayed reconciliation can change a renderer without changing its range. Viewport-targeted events preserve renderer-relative coordinates while exercising zrender's own hit-test and drag lifecycle. They must not retain a child shape or replaceable SVG/canvas content element as their target, or depend on animation-frame visibility. Tauri WebDriver pointer actions currently lose the pressed-button state and can expose child-relative coordinates that zrender cannot interpret, so WebDriver Actions are not admissible evidence for this interaction until that boundary is verified independently. The chart contract keeps slider `realtime` processing disabled: the handle follows sustained input, while the analytical range and series are recalculated once when the user releases it.
...
- Completed ECharts `dataZoom` actions and actual chart resizes must synchronously flush zrender exactly once. Lifecycle
  unit tests protect both adapter boundaries, while the packaged handle-position oracle proves their user-visible
  result when an occluded embedded WebView suspends animation-frame callbacks. The zoom flush occurs after release and
  cannot reintroduce per-movement analytical recalculation.
...
- GitHub Actions runs impact classification, documentation links, public documentation contracts, and repository safety for every pull request and `main` revision. README, canonical product-status, static product-page, and the closed publication-only compositor, verifier, test, and Pages-workflow set run their SSOT, resource, release-state, accessibility, publication, composition, and update-preservation checks without invalidating unchanged application-package evidence. Application, shared dependency, control-plane, or unknown changes run the complete portable checks plus Rust workspace and desktop-host tests and strict linting on pinned Ubuntu 24.04. They do not automatically start unrelated packaged-platform campaigns. Focused dispatch selects exactly one native boundary while explicit candidate admission runs every admitted lane and alone writes complete fingerprint-bound evidence. Missing comparison evidence fails closed to portable quality rather than claiming candidate acceptance.
...
- Windows recovery-state preparation tests create a current-schema synthetic library and exact native identity, then
  ...
  become available only after process exit. A Windows-state test completes cleanup to a retained outcome, requires the
  native acknowledgement boundary to remove that receipt, and proves a repeated acknowledgement reports no retained
  result. Installation-coordinator tests require watchdog readiness before the `replacement-started` handoff, discard
  only while still
...
- A separate read-only `windows-2025` capability job builds an instrumented current-user NSIS package with the isolated
  ...
  generated run state; CI may retain only privacy-safe WebdriverIO diagnostics after failure. This job runs for its
  explicit focused boundary or complete candidate verification and must succeed before its executable fingerprint receives reusable evidence. Hosted Windows
...
- Unix host tests create and reopen the local library boundary with exact `0700` directory and `0600` file modes,
  ...
  Repair planning independently proves that an exclusive directory handle is never reopened: current-user objects need
  at most DACL repair, token-default objects require owner authority on the already verified handle, missing authority
  fails before mutation, and foreign ownership remains rejected.
- The mandatory candidate macOS job prepares the source-bound production package, enforces its process-to-painted-shell cold-launch budget, then qualifies full-scale import, dense supported-signal storage and queries, and longitudinal read models before building the instrumented Tauri application under an isolated target and executing the focused packaged E2E journey with independently generated synthetic fixtures. The same job can be selected independently when macOS is the active failed boundary; ordinary executable increments do not launch it. The cold-launch adapter activates only the exact spawned PID through AppKit, includes that activation in the measured interval, and fails if native activation or the subsequent painted-shell signal is unavailable; it does not rely on bundle-name lookup, accessibility permission, or a second process. The E2E build produces only the `.app` consumed by WebdriverIO; an instrumented DMG adds no behavioral evidence, while the separate production package and installation gates retain complete DMG coverage. The tested WebdriverIO configuration rejects the production executable path. The ordinary functional instrumented application has the stable `org.fitfreed.desktop.e2e` bundle identifier, distinct from the production identifier, so macOS can retain a dedicated Desktop assignment across rebuilds. The synthetic update packages instead obtain the canonical production identifier from `tauri.conf.json`: update recovery deliberately rejects any other application identity, and the update campaign must exercise that production security boundary while retaining its package, library, and process isolation. The packaged journey resizes the real WebView across the accepted desktop and compact boundaries and measures sidebar, workspace, current-location, localization, zoom, and overflow behavior rather than inferring layout from component presence.
- Windows cold-launch acceptance treats every sample as an isolated process tree. After accepting or rejecting the
  painted-shell observation, it terminates the complete child tree rooted at the exact spawned PID through bounded
  native Windows termination before the next sample. A direct host-only kill is insufficient because WebView2
  descendants can survive it and contaminate later samples; failure to close the exact tree rejects the campaign.
...
The native Windows counterpart runs through `npm run verify:windows-update-e2e` in the focused or complete hosted Windows lane. It
...
exit must release the handle. It then executes the Windows-state outcome-acknowledgement test before building the
