...
the distinction between a recovery-state read failure and an ordinary channel failure. The package-shaped Linux
campaign must additionally drive the visible explicit retry through the installed Debian application, fail native
authorization or installation deliberately, prove one durable increment per attempt, and require manual reinstall
after exhaustion while the previous application, library, and recovery evidence remain intact.
