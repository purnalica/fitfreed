...
Mapping reads one expanded artifact at a time before the canonical visibility transaction. Each mapped observation carries its artifact locator, artifact SHA-256, `json-root` source-record locator, provider, current adapter version `polar-flow-archive@14`, and this mapping version into reconciliation. Reconciliation stores a provenance row for create, equivalent, enrichment, preservation, and conflict decisions. Current import operations record the combined `polar-flow-mapping-set@9` contract while daily-activity provenance retains the more specific `polar-flow-daily-activity@1` mapping. Historical operations may retain `polar-flow-mapping-set@1` through `polar-flow-mapping-set@8`.
...
Current adapter version `polar-flow-archive@14` retains the shape-based matrix, filename/content consistency, duplicate-identity rejection, version-scoped exact-repeat reuse, and source-subject resolution under [ADR 0005](../../architecture/decisions/0005-use-library-scoped-source-subject-correlation.md). Recognition still does not imply that every JSON structure or semantic variant is supported. The privacy-safe acceptance predicate must pass the supplied reference export for the exact current adapter revision before a candidate reaches product-owner review, but one evaluated package cannot establish universal historical compatibility.
