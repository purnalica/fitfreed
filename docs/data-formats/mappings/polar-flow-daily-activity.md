...
Mapping reads one expanded artifact at a time before the canonical visibility transaction. Each mapped observation carries its artifact locator, artifact SHA-256, `json-root` source-record locator, provider, current adapter version `polar-flow-archive@13`, and this mapping version into reconciliation. Reconciliation stores a provenance row for create, equivalent, enrichment, preservation, and conflict decisions. Current import operations record the combined `polar-flow-mapping-set@8` contract while daily-activity provenance retains the more specific `polar-flow-daily-activity@1` mapping. Historical operations may retain `polar-flow-mapping-set@1` through `polar-flow-mapping-set@7`.
