...
The protected release authority signs the exact payload and every updater package independently with the key named by
`fitfreed.keyId`. One payload may bind multiple platform targets to stable policy, compatibility, localized notes,
withdrawal state, direct Pages URLs, byte sizes, digests, and Tauri signatures. Its untrusted Tauri mirror contains the
same complete target set. The envelope and every bound package are promoted with the complete product site in one Pages
deployment artifact; human-facing installers and complete evidence sets remain in the corresponding GitHub Release
under [ADR 0020](../../architecture/decisions/0020-compose-product-and-update-pages.md). Adding a platform never
publishes a replacement snapshot that silently removes an already supported target.
