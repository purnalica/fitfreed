...
| Environment-qualified import, Insights, packaged UI, update-recovery, and cold-launch budgets | Pending hosted verification | [Hosted campaign `33554185602`](https://github.com/purnalica/fitfreed/actions/runs/33554185602) remains the accepted baseline for source `7583ca9`. Exact descendant `fff7df8` passes the complete portable, Linux, and macOS budget and packaged campaigns in [campaign `33977220148`](https://github.com/purnalica/fitfreed/actions/runs/33977220148). Exact correction `26a231d` stops at the independent portable component-test race recorded above, so it produces no newer native evidence. The current tree preserves the query contract and passes the constrained local presentation campaign. | Synchronize the current exact source and pass the complete hosted campaign without weakening any product budget or assertion. |
...
| Exact-source continuous integration and repository safety | Pending hosted verification | [Repository safety `33993588622`](https://github.com/purnalica/fitfreed/actions/runs/33993588622) passes for exact source `26a231d`; its [continuous integration `33993588621`](https://github.com/purnalica/fitfreed/actions/runs/33993588621) stops at the asynchronous component-test readiness defect recorded above. The current release-affecting changed tree corrects that defect, so no earlier immutable executable-input marker can be reused. | Synchronize one exact clean revision and pass repository safety plus the complete hosted campaign for its computed fingerprint. |
