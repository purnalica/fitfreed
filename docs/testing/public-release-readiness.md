...
| Exact 0.1.12 macOS-plus-Linux candidate, native admission, and publication | In progress | Version 0.1.12 and stable sequence 12 are assigned across source, release policy, update matrix, release notes, localized guides, product surfaces, and package metadata. The installed first-launch availability harness uses the exact correction admitted against retained 0.1.11 bytes. | Pass exact-source CI and safety, native Linux construction, protected complete-platform composition, both Ubuntu admission rows, protected promotion, and final remote verification. Do not repeat unchanged product E2E, update-recovery, or subjective evaluation. |
