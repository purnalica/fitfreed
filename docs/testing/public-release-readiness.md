...
| Environment-qualified import, Insights, packaged UI, update-recovery, and cold-launch budgets | Pending hosted verification | [Hosted campaign `33554185602`](https://github.com/purnalica/fitfreed/actions/runs/33554185602) remains the accepted baseline for source `7583ca9`. Exact descendant `5e531e4` passes the complete portable, Linux, and macOS budget and packaged campaigns in [campaign `33993889001`](https://github.com/purnalica/fitfreed/actions/runs/33993889001), but the two Windows jobs fail before their complete native evidence. The current tree preserves every product budget and interaction assertion while correcting those earlier gates locally. | Synchronize the current exact source and pass the complete hosted campaign without weakening any product budget or assertion. |
...
| Exact-source continuous integration and repository safety | Pending hosted verification | [Repository safety `33993889053`](https://github.com/purnalica/fitfreed/actions/runs/33993889053) passes for exact source `5e531e4`. Its [continuous integration `33993889001`](https://github.com/purnalica/fitfreed/actions/runs/33993889001) passes every macOS and Linux job but exposes the two Windows defects recorded above. The current release-affecting changed tree corrects both and changes native executable inputs, so no earlier complete marker can be reused. | Synchronize one exact clean revision and pass repository safety plus the complete hosted campaign for its computed fingerprint. |
