...
The FitFreed 0.1.0 product experience is **accepted for the current MVP scope but is not publicly available**.
...
| Public tag, immutable GitHub Release, Pages deployment, and binary availability | Not authorized | Normal commits and pushes cannot trigger the manual workflow or approve its protected jobs. | The release owner must authorize the exact tag and dispatch, then may approve the separate promotion gate only after exact-candidate acceptance. |
