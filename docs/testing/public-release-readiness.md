...
through standard output. Local executable source `ecd4358` preserves a closed native inventory subphase, consumes the
single canonical installed-package script path, and uses one fresh bounded local named pipe for Windows painted-shell
measurement. The subsequent local tree divides the native installation lifecycle into allowlisted granular subphases
whose PowerShell and JavaScript definitions are contract-tested together; arbitrary diagnostic text remains redacted.
All affected portable Windows contracts, focused Rust contracts, strict lint, formatting, documentation, and
repository-content gates pass. Exact hosted evidence for that correction remains pending synchronization.
...
| Environment-qualified import, Insights, packaged UI, update-recovery, and cold-launch budgets | Pending hosted verification | [Hosted campaign `33554185602`](https://github.com/purnalica/fitfreed/actions/runs/33554185602) remains the accepted product baseline for source `7583ca9`. Exact descendant `b94a8f4` passes the complete portable, Linux, packaged Windows capability, and packaged macOS boundaries in [campaign `34031313078`](https://github.com/purnalica/fitfreed/actions/runs/34031313078). Its Windows inventory, update recovery, and separate cold-launch performance boundaries expose the three instrumental defects corrected by local executable source `ecd4358`; every focused local contract passes without changing a product budget or functional assertion. | Synchronize the correction, pass the affected Windows native boundaries, and record one complete exact-source campaign only if its executable-input classification requires it. |
...
| Exact-source continuous integration and repository safety | Pending hosted verification | [Repository safety `34031313021`](https://github.com/purnalica/fitfreed/actions/runs/34031313021) passes for exact source `b94a8f4`. Its [continuous integration `34031313078`](https://github.com/purnalica/fitfreed/actions/runs/34031313078) passes every Linux and macOS job plus packaged Windows capability, but does not record a complete marker after the two Windows harness failures. Local executable source `ecd4358` corrects those failures and the separately observed Windows cold-launch transport, so no earlier complete marker can be reused for its Windows executable inputs. | Synchronize the exact clean revision and pass repository safety plus the required native Windows evidence for its computed fingerprint. |
