...
| Developer ID Application and App Store Connect notarization authority | In progress; Developer ID password and notarization authority pending | The workflow accepts an exact certificate fingerprint and App Store Connect API key through ephemeral files and a temporary keychain. On 2026-09-10 the project owner confirmed an active individual Apple Developer Program membership with a future renewal date. Apple issued a G2 Developer ID Application certificate; installation of the official Apple intermediate completed its trust chain, and the local keychain reports exactly one valid identity with its private key. The protected `public-macos-release` environment contains the expected non-secret team identifier, exact certificate fingerprint, and encrypted certificate bundle. Exact comparison verified the identifiers without recording their values here, and the externally retained bundle has owner-only permissions. Its export password and App Store Connect authority are not configured. | Admit the certificate export password directly, create and admit an App Store Connect team API key, verify custody and revocation behavior, and let the objective trust inspector pass on the exact candidate. An interactive Apple ID is not required by the release workflow. |
