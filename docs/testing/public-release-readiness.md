...
| Environment-qualified import, Insights, packaged UI, update-recovery, and cold-launch budgets | Pending hosted verification | [Hosted campaign `33554185602`](https://github.com/purnalica/fitfreed/actions/runs/33554185602) remains the accepted product baseline for source `7583ca9`. Exact source `48a88d3` passes the portable and Linux host, Debian construction and installation, Linux update recovery, and packaged Windows capability in [campaign `34049176597`](https://github.com/purnalica/fitfreed/actions/runs/34049176597). Linux packaged capability exposes one transient chart-resize interaction failure; Windows update recovery was cancelled after its build began. Exact descendant `9e88fdb` passes the complete Windows host through NSIS construction in focused [run `34054394508`](https://github.com/purnalica/fitfreed/actions/runs/34054394508), then loses the concrete inventory phase. Local commits `0048f52` and `bfeec51` preserve that phase and isolate its next native experiment. | Synchronize both commits; close Windows package inventory; correct and focus the Linux chart-resize boundary; pass the still-unproved Windows update and performance boundaries; then run one complete candidate campaign. |
