...
| Independent release-checksum authority | Public trust activated; protected credential pair incomplete | The separately generated authority remains outside the repository. The versioned release-signing configuration contains only the active `release.primary-1` public key, validated as distinct from updater authority. The encrypted private key is installed in the protected release environment; its separately held password is still required before a checksum signature can be admitted. | Supply the matching password only through the protected environment, then pass the secret-free Linux expansion preflight and protected signing boundary. |
