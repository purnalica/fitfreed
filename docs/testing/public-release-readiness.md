...
next publishable candidate is now version 0.1.5. The public `v0.1.0` tag is fixed at a source revision whose
...
| Signed/notarized evidence, candidate preparation, sealed transport, separate technical and human admission, immutable Release, provenance, Pages ordering, and remote verification automation | In progress for explicit final-DMG trust | The [public release guide](../development/public-release.md), [operations runbook](../development/public-release-operations.md), ADRs [0019](../architecture/decisions/0019-separate-candidate-build-from-public-promotion.md), [0020](../architecture/decisions/0020-compose-product-and-update-pages.md), [0046](../architecture/decisions/0046-separate-windows-candidate-build-and-admission-hosts.md), and [0047](../architecture/decisions/0047-permit-bootstrap-solo-release-approval.md), all three static workflow policies, actionlint, and synthetic orchestration tests cover the closed one-, two-, and three-platform product-site and update-channel topologies. Dispatches 0.1.0 through 0.1.3 exposed and closed the preflight, preparation-token, private-key, and updater-public-key handoff defects. Exact source `5200bf8` passed the three required hosted checks; its `v0.1.4` [dispatch `34623724477`](https://github.com/purnalica/fitfreed/actions/runs/34623724477) proved updater artifact signing and Apple application trust, then exposed the unowned final-DMG boundary before sealing. Current source explicitly finalizes the DMG and gives every objective trust command a privacy-safe stage identity. | Pass focused release-boundary checks plus exact-source CI and repository safety, then obtain authority for a new immutable 0.1.5 tag and protected candidate build. Production artifact trust, native exact-candidate execution, the later promotion approval, immutable predecessor releases, and publication remain separate gates. |
| Exact-source continuous integration and repository safety | Pending hosted verification for the 0.1.5 correction | Exact source `5200bf8` passes [product site `34622199380`](https://github.com/purnalica/fitfreed/actions/runs/34622199380), [repository safety `34622199403`](https://github.com/purnalica/fitfreed/actions/runs/34622199403), and [impact-classified continuous integration `34622199331`](https://github.com/purnalica/fitfreed/actions/runs/34622199331) without repeating packaged E2E. The protected `v0.1.4` run then exposed the final-DMG trust boundary that ordinary CI cannot execute. | Pass the required workflows on the exact 0.1.5 correction before creating its tag. Do not repeat unchanged product E2E or performance campaigns; reserve exact native admission for the sealed candidate. |
...
| Developer ID Application and App Store Connect notarization authority | Passed for protected application signing, notarization, and stapling; final-DMG and exact-candidate trust pending | The workflow accepts an exact certificate fingerprint and App Store Connect team API key through ephemeral files and a temporary keychain. On 2026-09-10 the project owner confirmed an active individual Apple Developer Program membership with a future renewal date. Apple issued a G2 Developer ID Application certificate; installation of the official Apple intermediate completed its trust chain, and the local keychain reports exactly one valid identity with its private key. The protected `public-macos-release` environment contains the expected non-secret team identifier, exact certificate fingerprint, encrypted certificate bundle, separately supplied export password, App Store Connect private key, issuer identifier, and key identifier. Exact comparison verified the identifiers without recording their values here. The protected 0.1.2 through 0.1.4 executions signed the application, received Apple's accepted application notarization result, stapled the application, and signed the DMG; 0.1.4 then exposed that the final DMG still required its own explicit notarization and stapling boundary. No unsealed output was retained, and unconditional cleanup passed. | Confirm recoverable external custody and revocation procedures, then let explicit final-DMG notarization and objective trust inspection pass on the exact sealed candidate. An interactive Apple ID is not required by the release workflow. |
...
| Sealed Developer ID-signed and Apple-notarized 0.1.5 candidate | Awaiting exact candidate | Candidate preparation, digest-bound transport, seven-day retention, and promotion reopening are implemented. The failed 0.1.2 through 0.1.4 executions produced only unsealed runner-local intermediate output and stopped before complete candidate verification, transport sealing, or retention. | Dispatch an authorized exact tag, approve only the protected build, and retain its sealed candidate and digest without publishing it. |
...
| Public tag, immutable GitHub Release, Pages deployment, and binary availability | Awaiting exact 0.1.5 candidate authority | The immutable `v0.1.0` through `v0.1.4` source tags exist only as rejected attempts; none produced a sealed candidate, Release, Pages deployment, or binary publication. The project owner authorized each exact tag and protected build separately; no version-specific authority implies authority for another tag, and promotion remains separately unauthorized. | After the exact-source gates pass, authorize `v0.1.5` tag creation, dispatch, and protected candidate build. Approve the separate promotion gate only after exact-candidate acceptance and explicit publication authority. |
...
FitFreed 0.1.5 may be called a public release only when every applicable row is **Passed** for one exact immutable source and candidate, both GitHub approval records correspond to that workflow run, and no required evidence belongs only to a development, synthetic, ad-hoc, private-alpha, earlier-source, or rebuilt artifact.
