...
next publishable candidate is now version 0.1.6. The public `v0.1.0` tag is fixed at a source revision whose
...
| Signed/notarized evidence, candidate preparation, sealed transport, separate technical and human admission, immutable Release, provenance, Pages ordering, and remote verification automation | In progress for objective certificate inspection | The [public release guide](../development/public-release.md), [operations runbook](../development/public-release-operations.md), ADRs [0019](../architecture/decisions/0019-separate-candidate-build-from-public-promotion.md), [0020](../architecture/decisions/0020-compose-product-and-update-pages.md), [0046](../architecture/decisions/0046-separate-windows-candidate-build-and-admission-hosts.md), and [0047](../architecture/decisions/0047-permit-bootstrap-solo-release-approval.md), all three static workflow policies, actionlint, and synthetic orchestration tests cover the closed one-, two-, and three-platform product-site and update-channel topologies. Dispatches 0.1.0 through 0.1.4 exposed and closed the preflight, preparation-token, private-key, updater-public-key, and final-DMG gaps. Exact source `3ad0ef5` passed the required hosted checks; its `v0.1.5` [dispatch `34628632360`](https://github.com/purnalica/fitfreed/actions/runs/34628632360) proved updater artifact signing plus explicit final-DMG signing, notarization, log inspection, and stapling, then exposed the invalid separate-argument form used for leaf-certificate extraction. Current source uses and tests the operational single-argument form. | Pass focused release-boundary checks plus exact-source CI and repository safety, then obtain authority for a new immutable 0.1.6 tag and protected candidate build. Production artifact trust, native exact-candidate execution, the later promotion approval, immutable predecessor releases, and publication remain separate gates. |
| Exact-source continuous integration and repository safety | Pending hosted verification for the 0.1.6 correction | Exact source `3ad0ef5` passes [repository safety `34627485220`](https://github.com/purnalica/fitfreed/actions/runs/34627485220) and [impact-classified continuous integration `34627485182`](https://github.com/purnalica/fitfreed/actions/runs/34627485182) without repeating packaged E2E. Its protected `v0.1.5` run then exposed the certificate-extraction command shape at a production-signature boundary that ordinary CI cannot create. | Pass the required workflows on the exact 0.1.6 correction before creating its tag. Do not repeat unchanged product E2E or performance campaigns; reserve exact native admission for the sealed candidate. |
...
| Sealed Developer ID-signed and Apple-notarized 0.1.6 candidate | Awaiting exact candidate | Candidate preparation, digest-bound transport, seven-day retention, and promotion reopening are implemented. The failed 0.1.2 through 0.1.5 executions produced only unsealed runner-local intermediate output and stopped before complete candidate verification, transport sealing, or retention. | Dispatch an authorized exact tag, approve only the protected build, and retain its sealed candidate and digest without publishing it. |
...
| Public tag, immutable GitHub Release, Pages deployment, and binary availability | Awaiting exact 0.1.6 candidate authority | The immutable `v0.1.0` through `v0.1.5` source tags exist only as rejected attempts; none produced a sealed candidate, Release, Pages deployment, or binary publication. The project owner authorized each exact tag and protected build separately; no version-specific authority implies authority for another tag, and promotion remains separately unauthorized. | After the exact-source gates pass, authorize `v0.1.6` tag creation, dispatch, and protected candidate build. Approve the separate promotion gate only after exact-candidate acceptance and explicit publication authority. |
...
FitFreed 0.1.6 may be called a public release only when every applicable row is **Passed** for one exact immutable source and candidate, both GitHub approval records correspond to that workflow run, and no required evidence belongs only to a development, synthetic, ad-hoc, private-alpha, earlier-source, or rebuilt artifact.
