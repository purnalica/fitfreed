...
| Exact-source continuous integration and repository safety | Awaiting the updater-trust activation revision | Exact source `23b0f1e` remains the prior complete executable baseline. The public trust-root activation changes release inputs and therefore requires fresh push evidence before preflight may reuse any prior platform result. | Commit the reviewed activation, then require successful repository-safety and impact-classified continuous-integration runs for that exact revision. Reserve the complete native matrix for final candidate admission. |
