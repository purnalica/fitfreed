...
candidate, and immutable public bytes. The 0.1.11 Linux expansion retains the separately established checksum authority and
...
| Exact 0.1.11 macOS-plus-Linux candidate, native admission, and publication | Rejected without publication | Immutable `v0.1.11` at `2172070c6210304bc70f8f46a5453b87cd6058f1` passed exact-source CI and safety, native Linux construction, and protected signing, notarization, reopening, and sealing in workflow `34788907954`. Ubuntu 24.04 passed exact installation, launch, p95 `906.854 ms` across 100 fresh processes, removal, and retained-library integrity. Ubuntu 26.04 installed and started the same exact Debian member but its desktop services activated too late for the fixed ten-second installation smoke observer; the product-performance distribution did not run in that row. | Keep 0.1.11 fixed and unpublished. Diagnose the retained bytes on Ubuntu 26.04 with the corrected thirty-second availability observation. Preserve the separate p95 `2,500 ms` production budget and prepare a new source-bound candidate only after that exact-byte result passes. Do not repeat unchanged product E2E, update-recovery, or subjective evaluation. |
