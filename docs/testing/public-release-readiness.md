...
| Developer ID Application and App Store Connect notarization authority | In progress; external activation pending | The workflow accepts an exact certificate fingerprint and App Store Connect API key through ephemeral files and a temporary keychain. On 2026-09-10 the project owner submitted an individual Apple Developer Program enrollment; Apple reports that the membership purchase is pending and may take up to 48 hours to process. No Developer ID certificate or App Store Connect authority exists yet. | Wait for active membership, then create a Developer ID Application certificate and an App Store Connect team API key, configure only their protected release inputs and non-secret identifiers, verify custody and revocation, and let the objective trust inspector pass on the exact candidate. An interactive Apple ID is not required by the release workflow. |
