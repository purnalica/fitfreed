...
| Exact 0.1.11 macOS-plus-Linux candidate, native admission, and publication | In progress | Version, sequence, policy, matrix, notes, user guides, and the focused Ubuntu 24.04 harness correction are prepared without a product-behavior change. | Pass exact-source continuous integration and repository safety, create the immutable tag, construct and seal the protected candidate, pass both native admission rows, approve promotion, and verify remote convergence. Do not repeat unchanged product E2E, update-recovery, or subjective evaluation. |
