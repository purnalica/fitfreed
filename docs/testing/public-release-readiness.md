...
first publishable candidate is now version 0.1.2. The public `v0.1.0` tag is fixed at a source revision whose
...
passed, and neither tag will be moved or reused.
...
| Signed/notarized evidence, candidate preparation, sealed transport, separate technical and human admission, immutable Release, provenance, Pages ordering, and remote verification automation | In progress for the protected preparation correction | The [public release guide](../development/public-release.md), [operations runbook](../development/public-release-operations.md), ADRs [0019](../architecture/decisions/0019-separate-candidate-build-from-public-promotion.md), [0020](../architecture/decisions/0020-compose-product-and-update-pages.md), [0046](../architecture/decisions/0046-separate-windows-candidate-build-and-admission-hosts.md), and [0047](../architecture/decisions/0047-permit-bootstrap-solo-release-approval.md), all three static workflow policies, actionlint, and synthetic orchestration tests cover the closed one-, two-, and three-platform product-site and update-channel topologies. [Dispatch `34501952100`](https://github.com/purnalica/fitfreed/actions/runs/34501952100) exposed reliance on a nonexistent repository-visibility variable before authority admission. The corrected `v0.1.1` [dispatch `34535866497`](https://github.com/purnalica/fitfreed/actions/runs/34535866497) passed that preflight and every protected prerequisite, then exposed that candidate preparation re-enters the same live preflight without a job-scoped GitHub token. Current source supplies the read-only token only to that preparation step and makes its presence a workflow contract. | Pass focused and complete local checks plus exact-source CI and repository safety, then obtain authority for a new immutable 0.1.2 tag and protected candidate build. Production artifact trust, native exact-candidate execution, the later promotion approval, immutable predecessor releases, and publication remain separate gates. |
| Exact-source continuous integration and repository safety | Pending hosted verification for the 0.1.2 correction | Exact source `e4746ac` passes [repository safety `34533874575`](https://github.com/purnalica/fitfreed/actions/runs/34533874575), [impact-classified continuous integration `34533874682`](https://github.com/purnalica/fitfreed/actions/runs/34533874682), and the complete local revision-bound campaign. The protected `v0.1.1` run then exposed the missing token at a release-only boundary that ordinary CI does not execute with production authority. | Pass both required workflows on the exact 0.1.2 correction before creating its tag. Reserve the complete native matrix for final candidate admission. |
...
| Sealed Developer ID-signed and Apple-notarized 0.1.2 candidate | Awaiting exact candidate | Candidate preparation, digest-bound transport, seven-day retention, and promotion reopening are implemented. The failed 0.1.1 execution stopped before artifact construction, so no production-authority candidate exists. | Dispatch the authorized exact tag, approve only the protected build, and retain its sealed candidate and digest without publishing it. |
...
| Public tag, immutable GitHub Release, Pages deployment, and binary availability | Awaiting exact 0.1.2 candidate authority | The immutable `v0.1.0` and `v0.1.1` source tags exist only as rejected attempts; neither produced a candidate, Release, Pages deployment, or binary publication. The project owner authorized and approved the failed 0.1.1 candidate build. That version-specific authority does not imply authority for a different tag, and promotion remains separately unauthorized. | After the exact-source gates pass, authorize `v0.1.2` tag creation, dispatch, and protected candidate build. Approve the separate promotion gate only after exact-candidate acceptance and explicit publication authority. |
...
FitFreed 0.1.2 may be called a public release only when every applicable row is **Passed** for one exact immutable source and candidate, both GitHub approval records correspond to that workflow run, and no required evidence belongs only to a development, synthetic, ad-hoc, private-alpha, earlier-source, or rebuilt artifact.
