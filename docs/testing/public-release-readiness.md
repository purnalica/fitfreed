...
| Protected GitHub environment, immutable Releases, and Actions-backed HTTPS Pages | Open external gate | Repository-level immutable Releases are enabled. Actions-backed Pages is configured at the verified `fitfreed.org` origin with enforced HTTPS and exact-byte publication checks. The required `public-macos-release` environment and bootstrap reviewer policy are not configured; the existing `github-pages` deployment environment is not a substitute for either release approval. | Configure the project owner as the required reviewer, allow initiator approval under [ADR 0047](../architecture/decisions/0047-permit-bootstrap-solo-release-approval.md), preserve the remaining controls in the [operations runbook](../development/public-release-operations.md), and verify the complete state through secret-free preflight. |
