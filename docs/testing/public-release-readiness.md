...
candidate, and immutable public bytes. The 0.1.12 Linux expansion retains the separately established checksum authority and
must still establish exact-candidate evidence.
...
| Exact 0.1.12 macOS-plus-Linux candidate, native admission, and publication | In progress; immutable Release published, remote acceptance open | Exact workflow [`34830250376`](https://github.com/purnalica/fitfreed/actions/runs/34830250376) passed exact-source CI and safety, native Linux construction, protected complete-platform composition, Apple signing and notarization, both Ubuntu admission rows, protected promotion, provenance, and immutable Release publication. Final verification found that the public origin still served 0.1.7 because a preceding product workflow and the release workflow had deployed distinct complete artifacts with the same revision identity. ADR 0048 prevents recurrence. | Deploy the authenticated 0.1.12 snapshot from the distinct correction revision and rerun only the original run's failed remote verifier. Do not rebuild packages, replace Release assets, or repeat product E2E, native admission, update recovery, or subjective evaluation. |
