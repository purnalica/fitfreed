...
next publishable candidate is now version 0.1.3. The public `v0.1.0` tag is fixed at a source revision whose
...
| Signed/notarized evidence, candidate preparation, sealed transport, separate technical and human admission, immutable Release, provenance, Pages ordering, and remote verification automation | In progress for the updater-authority handoff correction | The [public release guide](../development/public-release.md), [operations runbook](../development/public-release-operations.md), ADRs [0019](../architecture/decisions/0019-separate-candidate-build-from-public-promotion.md), [0020](../architecture/decisions/0020-compose-product-and-update-pages.md), [0046](../architecture/decisions/0046-separate-windows-candidate-build-and-admission-hosts.md), and [0047](../architecture/decisions/0047-permit-bootstrap-solo-release-approval.md), all three static workflow policies, actionlint, and synthetic orchestration tests cover the closed one-, two-, and three-platform product-site and update-channel topologies. [Dispatch `34501952100`](https://github.com/purnalica/fitfreed/actions/runs/34501952100) exposed reliance on a nonexistent repository-visibility variable before authority admission. The corrected `v0.1.1` [dispatch `34535866497`](https://github.com/purnalica/fitfreed/actions/runs/34535866497) exposed the missing preparation token. Corrected `v0.1.2` [dispatch `34611861394`](https://github.com/purnalica/fitfreed/actions/runs/34611861394) passed both preflights and Apple trust creation, then proved that Tauri 2.11.4's bundle path ignores `TAURI_SIGNING_PRIVATE_KEY_PATH` and reads `TAURI_SIGNING_PRIVATE_KEY`. Current source maps both names to the same protected external file path and rejects key contents or mismatched paths before packaging. | Pass focused release-boundary checks plus exact-source CI and repository safety, then obtain authority for a new immutable 0.1.3 tag and protected candidate build. Production artifact trust, native exact-candidate execution, the later promotion approval, immutable predecessor releases, and publication remain separate gates. |
| Exact-source continuous integration and repository safety | Pending hosted verification for the 0.1.3 correction | Exact source `104570f` passes [repository safety `34540384089`](https://github.com/purnalica/fitfreed/actions/runs/34540384089), [impact-classified continuous integration `34540384064`](https://github.com/purnalica/fitfreed/actions/runs/34540384064), and focused local release contracts. The protected `v0.1.2` run then exposed the updater-variable mismatch at a production-authority boundary that ordinary CI cannot execute. | Pass both required workflows on the exact 0.1.3 correction before creating its tag. Do not repeat unchanged product E2E or performance campaigns; reserve exact native admission for the sealed candidate. |
...
| Sealed Developer ID-signed and Apple-notarized 0.1.3 candidate | Awaiting exact candidate | Candidate preparation, digest-bound transport, seven-day retention, and promotion reopening are implemented. The failed 0.1.2 execution produced only unsealed runner-local intermediate output and stopped before updater signing, complete candidate verification, transport sealing, or retention. | Dispatch the authorized exact tag, approve only the protected build, and retain its sealed candidate and digest without publishing it. |
...
| Public tag, immutable GitHub Release, Pages deployment, and binary availability | Awaiting exact 0.1.3 candidate authority | The immutable `v0.1.0`, `v0.1.1`, and `v0.1.2` source tags exist only as rejected attempts; none produced a sealed candidate, Release, Pages deployment, or binary publication. The project owner authorized the 0.1.2 tag and protected build, but that version-specific authority does not imply authority for a different tag, and promotion remains separately unauthorized. | After the exact-source gates pass, authorize `v0.1.3` tag creation, dispatch, and protected candidate build. Approve the separate promotion gate only after exact-candidate acceptance and explicit publication authority. |
...
FitFreed 0.1.3 may be called a public release only when every applicable row is **Passed** for one exact immutable source and candidate, both GitHub approval records correspond to that workflow run, and no required evidence belongs only to a development, synthetic, ad-hoc, private-alpha, earlier-source, or rebuilt artifact.
