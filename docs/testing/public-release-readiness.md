...
first publishable candidate is now version 0.1.1: the public `v0.1.0` tag is fixed at a source revision whose
secret-free dispatch failed before any protected authority was admitted, and the tag will not be moved or reused.
...
| Signed/notarized evidence, candidate preparation, sealed transport, separate technical and human admission, immutable Release, provenance, Pages ordering, and remote verification automation | In progress for the corrected release preflight | The [public release guide](../development/public-release.md), [operations runbook](../development/public-release-operations.md), ADRs [0019](../architecture/decisions/0019-separate-candidate-build-from-public-promotion.md), [0020](../architecture/decisions/0020-compose-product-and-update-pages.md), [0046](../architecture/decisions/0046-separate-windows-candidate-build-and-admission-hosts.md), and [0047](../architecture/decisions/0047-permit-bootstrap-solo-release-approval.md), all three static workflow policies, actionlint, and synthetic orchestration tests cover the closed one-, two-, and three-platform product-site and update-channel topologies. [Dispatch `34501952100`](https://github.com/purnalica/fitfreed/actions/runs/34501952100) proved that the secret-free gate stopped before protected authority, but also exposed reliance on a nonexistent default runner variable for repository visibility. The corrected preflight reads live repository metadata through the same read-only GitHub API boundary as Pages and environment policy. | Pass focused and complete local checks, exact-source CI and repository safety, then dispatch a new immutable 0.1.1 tag. Production authority, native exact-candidate execution, protected approvals, immutable predecessor releases, and publication remain separate platform gates below or in the applicable milestone plan. |
| Exact-source continuous integration and repository safety | Passed for the updater-trust activation revision | Exact source `cf28907` passes [repository safety](https://github.com/purnalica/fitfreed/actions/runs/34465118932) and [impact-classified continuous integration](https://github.com/purnalica/fitfreed/actions/runs/34465118992). The latter accepts the active `stable.primary-1` release input, portable quality gates, the complete Linux desktop host, Debian package construction and lifecycle, and the bounded updater refinement. | Re-run after any executable, dependency, release-input, workflow, or policy change. Reserve the complete native matrix for final candidate admission. |
...
| Public tag, immutable GitHub Release, Pages deployment, and binary availability | Authorized through corrected candidate build only | The immutable `v0.1.0` source tag exists, but its preflight failed and no protected build, Release, Pages deployment, or binary publication occurred. The project owner authorized continuing toward the corrected candidate; promotion remains separately unauthorized. | Create and dispatch `v0.1.1` only after its exact-source gates pass. Approve the separate promotion gate only after exact-candidate acceptance and explicit publication authority. |
...
FitFreed 0.1.1 may be called a public release only when every applicable row is **Passed** for one exact immutable source and candidate, both GitHub approval records correspond to that workflow run, and no required evidence belongs only to a development, synthetic, ad-hoc, private-alpha, earlier-source, or rebuilt artifact.
