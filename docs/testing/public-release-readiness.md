...
| Windows public-trust route | Awaiting project-owner selection | The [Milestone 5 evaluation](../plans/milestone-5.md#windows-distribution-evaluation) compares available European individual certificates with a transparent unsigned preview. No replacement Authenticode authority has been purchased or admitted, and no unsigned public profile is authorized by the current release contracts. | Select HARICA-backed Authenticode or the documented unsigned-preview boundary, then record a separate ADR and implement only that route. |
| Exact Windows 11 candidate, native admission, product acceptance, and publication | Awaiting route and exact candidate | Hosted Windows Server engineering already passes package construction, installation, update recovery, cold launch, filesystem recovery, and data-performance budgets. The current complete-platform workflow deliberately cannot create a public Windows candidate without the retired SignPath path or a newly admitted unsigned-preview profile. | After route selection, seal one source-bound candidate, pass its exact Windows 11 installation and trust-profile gates plus the bounded product verdict, then obtain separate publication approval. |
