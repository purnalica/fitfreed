...
candidate, and immutable public bytes. The 0.1.9 Linux expansion has established its separate checksum authority and
...
| Assigned complete macOS-plus-Linux version and predecessor contract | Passed for proposed 0.1.9 | Publication metadata assigns stable sequence 9 to version 0.1.9. Upgrade-matrix version 2 declares 0.1.7 on `darwin-aarch64` with library schema 37 as the exact recovery baseline. Stable-v3 retains the active `stable.primary-1` updater trust. | Keep the version, target set, predecessor, release notes, guides, manifest, and public snapshot identical in the exact candidate. |
| Independent release-checksum authority | Passed for proposed 0.1.9 | The separately generated authority remains outside the repository. The versioned release-signing configuration contains only the active `release.primary-1` public key, validated as distinct from updater authority. Its encrypted private key and separately held password are installed only in the protected release environment. Workflow `34746306839` admitted that environment and reached real detached package signing; no private value entered source, logs, or retained artifacts. | Preserve external custody and require exact signature verification before candidate sealing and publication. |
| Exact 0.1.9 macOS-plus-Linux candidate, native admission, and publication | In progress | Immutable `v0.1.9` at `909eccf477870f4c607f93b8240556d78aee8729` and workflow `34748270550` passed native Linux input construction and protected signed, notarized complete-platform composition, then sealed one candidate. Both supported Ubuntu rows installed and validated its exact Debian member but failed to observe the interactive-shell signal within ten seconds. The same sealed Debian bytes report the correct version-and-revision-bound shell in bounded disposable Ubuntu 24.04 x86-64 reproductions, so package and general Linux startup are not the unresolved variable. The failed harness discarded causal process diagnostics and mislabeled the Unix output boundary as a startup channel. | Run the single-row, secret-free exact-artifact diagnostic once. Use its bounded path-redacted process evidence to identify the owning layer; do not rebuild, repeat E2E, or promote. Both admission rows, explicit promotion, and remote convergence remain required. |
