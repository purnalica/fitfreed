...
| Environment-qualified import, Insights, packaged UI, update-recovery, and cold-launch budgets | Pending hosted verification | [Hosted campaign `33554185602`](https://github.com/purnalica/fitfreed/actions/runs/33554185602) remains the accepted baseline for source `7583ca9`. Exact descendant `5da60b7` passes the complete portable and Linux campaigns in [campaign `33998318315`](https://github.com/purnalica/fitfreed/actions/runs/33998318315), while Windows stops before native performance evidence and macOS records a 3,034 ms cold-launch p95 against the unchanged 2,500 ms budget. Exact predecessor `5e531e4` passed that hosted macOS measurement at 2,433 ms. The current tree preserves every product budget and interaction assertion, passes the complete packaged E2E campaign locally, and corrects the two Windows gates exposed by `5da60b7`. | Synchronize the current exact source and pass the complete hosted campaign without weakening any product budget or assertion. |
...
| Exact-source continuous integration and repository safety | Pending hosted verification | [Repository safety `33998318356`](https://github.com/purnalica/fitfreed/actions/runs/33998318356) passes for exact source `5da60b7`. Its [continuous integration `33998318315`](https://github.com/purnalica/fitfreed/actions/runs/33998318315) passes the portable and both Linux package jobs, proves successful isolated Windows installation and startup, then exposes test-only cross-target compilation and asynchronous E2E synchronization defects. The macOS cold-launch measurement also misses its unchanged budget. The current release-affecting tree corrects the Windows defects locally and changes native executable inputs, so no earlier complete marker can be reused. | Synchronize one exact clean revision and pass repository safety plus the complete hosted campaign for its computed fingerprint. |
