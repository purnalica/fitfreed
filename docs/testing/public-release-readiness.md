...
| Hosted Windows candidate, native admission, product acceptance, and publication | In progress for 0.1.21 after rejected 0.1.20 | Hosted Windows Server engineering passes package construction, installation, update recovery, cold launch, filesystem recovery, and data-performance budgets. The expansion workflow binds setup, installed application, and uninstaller to `NotSigned`, preserves the exact Windows 11 client evidence gap, and requires installation, launch, update authority, data preservation, removal, immutable publication, and remote verification. Exact 0.1.20 exposed incomplete process-tree isolation between cold-launch samples before promotion; 0.1.21 closes the exact spawned PID's complete child tree between samples. | Seal and admit the exact 0.1.21 portable distribution. Do not claim exact Windows 11 client, Smart App Control, organizational-policy, or Authenticode compatibility from hosted evidence. |
