...
| SignPath Foundation service and production Authenticode authority | Open external gate | The repository publishes its code-signing policy and version-controls the required project, policy, artifact-configuration, protected-input, and approval contracts. No SignPath application acceptance, organization, active project, signing policy, certificate fingerprint, API token, or real signed response has been admitted as evidence. | SignPath Foundation must accept FitFreed; configure the exact identifiers in the operations runbook; record only the protected organization identifier, API token, and independently verified public certificate fingerprint; then approve and admit both exact signing responses. |
