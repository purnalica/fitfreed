...
| Exact 0.1.10 macOS-plus-Linux candidate, native admission, and publication | In progress | Version, sequence, policy, matrix, notes, user guides, and the corrected two-gate hosted session are prepared without a product-behavior change. | Pass exact-source continuous integration and repository safety, create the immutable tag, construct and seal the protected candidate, pass both native admission rows, approve promotion, and verify remote convergence. Do not repeat unchanged product E2E, performance, update-recovery, or subjective evaluation. |
