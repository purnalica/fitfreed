...
secret-free hosted admission record for those unchanged bytes; its hosted result and the bounded experience delta
decision remain open before promotion.
...
| Public stable-channel contracts, compile-time trust, replay protection, withdrawal, compatibility policy, and atomic Pages staging | Passed for the sealed 0.1.7 candidate | The [update-trust architecture](../architecture/update-trust.md), version 2 channel schemas, versioned configuration and policy, synthetic signing, local TLS packaged-update journey, closed update snapshot verifier, and ADR 0020 compositor tests cover the update contract. The sealed candidate's complete reopening authenticates its `stable.primary-1` updater signature, embedded active trust, exact updater archive, sequence 8 envelope, and byte-identical Pages staging without publishing them. The first public release has no application-version predecessor. | Exercise every real application predecessor declared by a later release's upgrade matrix; remote-verify the exact 0.1.7 endpoint only after authorized promotion. |
| Signed/notarized evidence, candidate preparation, sealed transport, separate technical and human admission, immutable Release, provenance, Pages ordering, and remote verification automation | Passed through sealed candidate creation; hosted native admission pending | The [public release guide](../development/public-release.md), [operations runbook](../development/public-release-operations.md), ADRs [0019](../architecture/decisions/0019-separate-candidate-build-from-public-promotion.md), [0020](../architecture/decisions/0020-compose-product-and-update-pages.md), [0046](../architecture/decisions/0046-separate-windows-candidate-build-and-admission-hosts.md), and [0047](../architecture/decisions/0047-permit-bootstrap-solo-release-approval.md), the release and candidate-admission static workflow policies, actionlint, and synthetic orchestration tests cover the trust topology. Exact `v0.1.7` dispatch `34650206971` passed every protected build boundary and retained the sealed artifact; independent local download, digest reopening, trust inspection, installation, and lifecycle admission pass. | Pass the secret-free hosted native admission. Promotion, provenance publication, immutable Release creation, Pages deployment, and remote verification remain later authorized workflow stages. |
...
| Version-matched public user, maintainer, support, security, disclaimer, release-note, and evaluation guidance | Passed for the sealed candidate; publication wording remains inactive | Requirements, roadmap, experience specification, migration plan, readiness state, generated product surfaces, 0.1.7 release notes, and localized user guides describe the same unavailable candidate and supported boundary. | Activate only through the candidate's atomic authorized promotion; do not announce availability beforehand. |
...
| Sealed Developer ID-signed and Apple-notarized 0.1.7 candidate | Passed | Exact dispatch `34650206971` retained artifact `10283667124` for seven days under the version-and-revision-bound name. Its emitted transport SHA-256 matches the independently downloaded archive, whose manifest, release and Pages trees, updater signature, Apple trust, and native lifecycle all reopen successfully. | Keep the original run waiting and the artifact available until the separate release decision. Never rebuild or alter it. |
...
| Clean install, Gatekeeper launch, import, exploration, update, interruption recovery, library preservation, and removal against the sealed public candidate | Passed locally for every applicable first-release boundary; hosted admission pending | The accepted fingerprint-bound product campaign proves import, exploration, accessibility, performance, and synthetic update recovery for the candidate source. Exact downloaded 0.1.7 bytes pass DMG installation, Gatekeeper launch through the first painted shell, restart, private schema-37 library integrity, application removal, and retained-library integrity. Complete reopening authenticates the exact updater payload and embedded production trust. No application-version update applies because the first release declares no predecessor. | Pass the same secret-free exact-byte native admission on the hosted Apple Silicon runner. Do not ask the product owner to repeat functional QA. |
| Exact-candidate product experience | Accepted baseline; one bounded delta decision remains | Revision-isolated source `47a521e` passed native inspection, exact hosted checks, and the bounded product-owner profile on 2026-09-09. Candidate source adds the tested start-aligned reveal of the existing personal-range editor; all other later executable changes belong to versioning, release trust, or update-recovery acceptance. Functional correctness, keyboard behavior, scaling, appearance, localized update, recovery, and automatable accessibility remain exact-candidate automation responsibilities. | After hosted admission, evaluate only whether that narrow visibility correction requires an additional subjective check; never repeat the functional matrix manually. |
| Public tag, immutable GitHub Release, Pages deployment, and binary availability | Tag and sealed candidate passed; publication unauthorized | `v0.1.7` is fixed at `e610d7e`, and dispatch `34650206971` is waiting at the second protected approval with no GitHub Release, Pages update, or binary publication. Earlier tags remain rejected and immutable. | Obtain explicit promotion authority only after hosted native admission and the bounded experience delta decision. |
