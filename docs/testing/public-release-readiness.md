...
The FitFreed 0.1.0 product experience is **accepted for the current MVP scope but is not publicly available**. The
next publishable candidate is now version 0.1.4. The public `v0.1.0` tag is fixed at a source revision whose
...
not accept this macOS candidate or change the ordered publication dependency.
...
| Public stable-channel contracts, compile-time trust, replay protection, withdrawal, compatibility policy, and atomic Pages staging | Passed; exact public-candidate evidence remains open | The [update-trust architecture](../architecture/update-trust.md), version 2 channel schemas, versioned configuration and policy, synthetic signing, local TLS packaged-update journey, closed update snapshot verifier, and ADR 0020 compositor tests cover the update contract. The historical [canonical-origin deployment](https://github.com/purnalica/fitfreed/actions/runs/32147443685) byte-accepted the product surface while preserving the then-inactive update boundary. The reviewed `stable.primary-1` public trust is now active for a future candidate without publishing a signed snapshot. | Re-run the release-shaped update and recovery evidence against the exact sealed public candidate that embeds this trust root. |
| Signed/notarized evidence, candidate preparation, sealed transport, separate technical and human admission, immutable Release, provenance, Pages ordering, and remote verification automation | In progress for the updater public-key handoff correction | The [public release guide](../development/public-release.md), [operations runbook](../development/public-release-operations.md), ADRs [0019](../architecture/decisions/0019-separate-candidate-build-from-public-promotion.md), [0020](../architecture/decisions/0020-compose-product-and-update-pages.md), [0046](../architecture/decisions/0046-separate-windows-candidate-build-and-admission-hosts.md), and [0047](../architecture/decisions/0047-permit-bootstrap-solo-release-approval.md), all three static workflow policies, actionlint, and synthetic orchestration tests cover the closed one-, two-, and three-platform product-site and update-channel topologies. [Dispatch `34501952100`](https://github.com/purnalica/fitfreed/actions/runs/34501952100) exposed reliance on a nonexistent repository-visibility variable before authority admission. The corrected `v0.1.1` [dispatch `34535866497`](https://github.com/purnalica/fitfreed/actions/runs/34535866497) exposed the missing preparation token. Corrected `v0.1.2` [dispatch `34611861394`](https://github.com/purnalica/fitfreed/actions/runs/34611861394) proved that Tauri 2.11.4's bundle path reads `TAURI_SIGNING_PRIVATE_KEY`; source `5fe834e` maps that name and FitFreed's path input to the same protected file. Its exact [repository safety `34614348537`](https://github.com/purnalica/fitfreed/actions/runs/34614348537) and impact-classified [continuous integration `34614348485`](https://github.com/purnalica/fitfreed/actions/runs/34614348485) passed without repeating packaged E2E. The resulting `v0.1.3` [dispatch `34619731032`](https://github.com/purnalica/fitfreed/actions/runs/34619731032) proved that correction and Apple trust, then exposed the independent empty Tauri public-key input. Current source derives the final Tauri artifact configuration from the explicitly selected key in the canonical active trust set and shares that boundary with Linux. | Pass focused release-boundary checks plus exact-source CI and repository safety, then obtain authority for a new immutable 0.1.4 tag and protected candidate build. Production artifact trust, native exact-candidate execution, the later promotion approval, immutable predecessor releases, and publication remain separate gates. |
| Exact-source continuous integration and repository safety | Pending hosted verification for the 0.1.4 correction | Exact source `5fe834e` passes [repository safety `34614348537`](https://github.com/purnalica/fitfreed/actions/runs/34614348537), [impact-classified continuous integration `34614348485`](https://github.com/purnalica/fitfreed/actions/runs/34614348485), and focused local release contracts. The protected `v0.1.3` run then exposed the updater public-key handoff at a production-authority boundary that ordinary CI cannot execute. | Pass both required workflows on the exact 0.1.4 correction before creating its tag. Do not repeat unchanged product E2E or performance campaigns; reserve exact native admission for the sealed candidate. |
| Version-matched public user, maintainer, support, security, disclaimer, release-note, and evaluation guidance | In progress | Current-source requirements, roadmap, experience specification, migration plan, readiness state, and generated product surfaces distinguish the rejected X7-R8.13 application, X7-R8.14 correction, later scope, and unavailable public distribution. | Preserve this current-source evidence, then generate version-matched guidance for the eventual sealed public candidate. |
| Production updater authority and embedded public trust | In progress | On 2026-09-10 the project owner confirmed external generation of the production updater key pair. The checked-in configuration activates the reviewed `stable.primary-1` public key after canonical Tauri/Minisign validation. The private-key file has restricted local permissions, and both the private key and its separately supplied password were admitted directly to the protected `public-macos-release` environment without exposing either value or retaining a machine-local path. The protected 0.1.2 execution proved the missing private-key variable; the protected 0.1.3 execution proved the corrected private-key handoff and then failed before updater signing because Tauri still received an empty public-key input. Cleanup removed the authority after both attempts. Current focused tests require both private-key names to identify one protected file and require Tauri artifact generation to use the exact selected canonical public key. Recovery-custody confirmation and a real updater sign-and-verify operation remain open. | Confirm recoverable external custody and prove signing plus exact-candidate verification without exposing private material. |
| Developer ID Application and App Store Connect notarization authority | Passed for protected signing, notarization, and stapling; exact-candidate trust pending | The workflow accepts an exact certificate fingerprint and App Store Connect team API key through ephemeral files and a temporary keychain. On 2026-09-10 the project owner confirmed an active individual Apple Developer Program membership with a future renewal date. Apple issued a G2 Developer ID Application certificate; installation of the official Apple intermediate completed its trust chain, and the local keychain reports exactly one valid identity with its private key. The protected `public-macos-release` environment contains the expected non-secret team identifier, exact certificate fingerprint, encrypted certificate bundle, separately supplied export password, App Store Connect private key, issuer identifier, and key identifier. Exact comparison verified the identifiers without recording their values here, GitHub confirms only the secret names, and both externally retained private files have owner-only permissions. The protected 0.1.2 and 0.1.3 executions signed the application, received Apple's accepted notarization result, stapled the application, and signed the DMG before separate updater failures; neither unsealed output was retained, and unconditional cleanup passed. | Confirm recoverable external custody and revocation procedures, then let the objective trust inspector pass on the exact sealed candidate. An interactive Apple ID is not required by the release workflow. |
| Protected GitHub environment, immutable Releases, and Actions-backed HTTPS Pages | Passed for repository configuration | Repository-level immutable Releases are enabled. Actions-backed Pages is configured at the verified `fitfreed.org` origin with enforced HTTPS and exact-byte publication checks. On 2026-09-10 the GitHub API confirmed that `public-macos-release` requires the bootstrap project-owner reviewer, permits initiator approval under [ADR 0047](../architecture/decisions/0047-permit-bootstrap-solo-release-approval.md), disables administrator bypass, and admits only tags matching the single `v*` policy. The environment also reports the required Administration-read secret name; GitHub does not expose its value, and the protected build verifies its effective immutable-Release permission before installing any signing authority. No environment secret value was read or retained as evidence. | Re-run the secret-free policy check before each candidate build and after any environment, reviewer, deployment-policy, Pages, immutable-Release setting, or protected-token change. Protected credentials remain the separate authority gates above. |
| Sealed Developer ID-signed and Apple-notarized 0.1.4 candidate | Awaiting exact candidate | Candidate preparation, digest-bound transport, seven-day retention, and promotion reopening are implemented. The failed 0.1.2 and 0.1.3 executions produced only unsealed runner-local intermediate output and stopped before updater signing, complete candidate verification, transport sealing, or retention. | Dispatch an authorized exact tag, approve only the protected build, and retain its sealed candidate and digest without publishing it. |
...
| Exact-candidate product experience | Passed for the revision-isolated 0.1.0 experience baseline; awaiting a sealed public candidate | Revision-isolated source `47a521e` passed native inspection, exact hosted checks, and the bounded product-owner profile on 2026-09-09. Functional correctness, keyboard behavior, scaling, appearance, localized update, recovery, and automatable accessibility remain exact-candidate automation responsibilities; the product owner is not responsible for reproducing those checks. | Repeat the bounded profile only for a later sealed public candidate whose experience-affecting executable inputs differ. |
| Public tag, immutable GitHub Release, Pages deployment, and binary availability | Awaiting exact 0.1.4 candidate authority | The immutable `v0.1.0`, `v0.1.1`, `v0.1.2`, and `v0.1.3` source tags exist only as rejected attempts; none produced a sealed candidate, Release, Pages deployment, or binary publication. The project owner authorized each exact tag and protected build separately; no version-specific authority implies authority for another tag, and promotion remains separately unauthorized. | After the exact-source gates pass, authorize `v0.1.4` tag creation, dispatch, and protected candidate build. Approve the separate promotion gate only after exact-candidate acceptance and explicit publication authority. |
...
FitFreed 0.1.4 may be called a public release only when every applicable row is **Passed** for one exact immutable source and candidate, both GitHub approval records correspond to that workflow run, and no required evidence belongs only to a development, synthetic, ad-hoc, private-alpha, earlier-source, or rebuilt artifact.
