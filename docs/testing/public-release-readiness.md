...
| Windows SignPath repository integration | Passed locally | ADR 0049, the pinned official SignPath action, two closed artifact configurations, the authority-free NSIS uninstaller bridge, independent returned-byte inspection, native installation inventory, and workflow mutation contracts implement the production signing path without local certificate custody. The superseded PFX authority adapter and protected values are absent from the production workflow. No product E2E or performance campaign is repeated because this change affects only release construction and verification. | Obtain exact-source hosted repository-safety and workflow evidence after synchronization. |
| SignPath Foundation service and production Authenticode authority | Submitted; awaiting external evaluation | The repository publishes its code-signing policy and version-controls the required project, policy, artifact-configuration, protected-input, and approval contracts. The SignPath Foundation open-source application was submitted on 2026-09-14. No application acceptance, SignPath organization, active project, signing policy, certificate fingerprint, API token, or real signed response has been admitted as evidence. | SignPath Foundation must accept FitFreed; configure the exact identifiers in the operations runbook; record only the protected organization identifier, API token, and independently verified public certificate fingerprint; then approve and admit both exact signing responses. |
| Exact Windows 11 candidate, native admission, product acceptance, and publication | Awaiting exact candidate | Hosted Windows Server engineering already passes package construction, installation, update recovery, cold launch, filesystem recovery, and data-performance budgets. The implemented complete-platform workflow cannot create a production candidate until the SignPath external gate closes and a separately authorized version tag and disposable supported Windows 11 admission runner exist. | Produce one source-bound SignPath-signed input, seal the complete candidate, pass exact Windows 11 technical admission and the bounded product verdict, then obtain separate publication approval. |
