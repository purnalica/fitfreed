...
| Developer ID Application and App Store Connect notarization authority | In progress; membership active | The workflow accepts an exact certificate fingerprint and App Store Connect API key through ephemeral files and a temporary keychain. On 2026-09-10 the project owner confirmed an active individual Apple Developer Program membership with a future renewal date. The expected non-secret team identifier is configured in the protected `public-macos-release` environment and was verified by exact comparison without recording its value here. No Developer ID certificate or App Store Connect authority exists yet. | Create a Developer ID Application certificate and an App Store Connect team API key, configure only their protected release inputs and remaining non-secret identifiers, verify custody and revocation, and let the objective trust inspector pass on the exact candidate. An interactive Apple ID is not required by the release workflow. |
