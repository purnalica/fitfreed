...
The 2026-09-06 completion audit retains D0 and E1 through E5 as implemented engineering evidence. Historical E6
experience acceptance remains invalid because its independent audit failed. X7-R8.14 is the implemented machine-admitted
replacement, but the exact current-source platform campaign and the bounded product-owner review remain open. Those
two gates prevent closure; they do not reopen D0 or repeat the accepted E1 through E5 campaigns.
...
| Focused commit and synchronization discipline | Every completed correction is a focused local commit. The complete outgoing range passes repository-content, secret, and public-identity safeguards. Commits `0048f52` and `bfeec51` remain ahead of `origin/main` because bounded SSH attempts could not obtain a Secretive signature. | **Pending synchronization** |
| Exact current-source native platform evidence | Focused run `34054394508` proves the complete Windows host through release-shaped NSIS construction and leaves only the native inventory phase unresolved. The previous candidate proves the Linux host, Debian construction, installation, and update recovery, while Linux packaged capability retains one chart-resize interaction failure. The local Windows diagnostic and package-only dispatch pass every affected contract but cannot obtain native evidence before synchronization. | **Pending hosted verification** |

## Readiness ledger

| Gate | State | Current evidence | Required closure |
|---|---|---|---|
| Confirmed 0.1.0 product scope, provider compatibility, canonical formats, persistence, migrations, and reimport semantics | Passed for the corrected machine-admitted source | E1 through E5 and X7-R8.9 through X7-R8.13 retain their earlier evidence. The X7-R8.13 review accepted recognizable sport identity and rejected range responsiveness. X7-R8.14 bounds both sustained ECharts zoom and exact route-preview scheduling; the complete exact local and hosted campaigns, repeated independent audit, and revision-isolated native inspection pass without changing persistence contracts. | Pass a new bounded product-owner experience review. |
...
| Environment-qualified import, Insights, packaged UI, update-recovery, and cold-launch budgets | Pending hosted verification | [Hosted campaign `33554185602`](https://github.com/purnalica/fitfreed/actions/runs/33554185602) remains the accepted product baseline for source `7583ca9`. Exact source `48a88d3` passes the portable and Linux host, Debian construction and installation, Linux update recovery, and packaged Windows capability in [campaign `34049176597`](https://github.com/purnalica/fitfreed/actions/runs/34049176597). Linux packaged capability exposes one transient chart-resize interaction failure; its retained paint proves that the 420-pixel canvas carried the preceding 360-pixel chart geometry. The current tree synchronously flushes actual ECharts resizes and rejects handles outside the current bottom control band. Windows update recovery was cancelled after its build began. Exact descendant `9e88fdb` passes the complete Windows host through NSIS construction in focused [run `34054394508`](https://github.com/purnalica/fitfreed/actions/runs/34054394508), then loses the concrete inventory phase. Local commits `0048f52` and `bfeec51` preserve that phase and isolate its next native experiment. | Synchronize the local increments; run only the focused Windows package inventory and Linux packaged Insights boundaries; pass the still-unproved Windows update and performance boundaries; then run one complete candidate campaign. |
...
| Signed/notarized evidence, candidate preparation, sealed transport, independent technical and human admission, immutable Release, provenance, Pages ordering, and remote verification automation | Passed for the implemented automation | The [public release guide](../development/public-release.md), [operations runbook](../development/public-release-operations.md), ADRs [0019](../architecture/decisions/0019-separate-candidate-build-from-public-promotion.md), [0020](../architecture/decisions/0020-compose-product-and-update-pages.md), and [0046](../architecture/decisions/0046-separate-windows-candidate-build-and-admission-hosts.md), all three static workflow policies, actionlint, and synthetic orchestration tests cover the closed one-, two-, and three-platform product-site and update-channel topologies. The earlier [hosted campaign](https://github.com/purnalica/fitfreed/actions/runs/32280636479) covers the initial public workflow boundary; the new Windows workflow has local contract evidence only. | Production authority, native exact-candidate execution, independent approvals, immutable predecessor releases, and publication remain separate platform gates below or in the applicable milestone plan. |
| Exact-source continuous integration and repository safety | Pending hosted verification | Push [run `34053818461`](https://github.com/purnalica/fitfreed/actions/runs/34053818461) passes the impact classifier and skips every package campaign for unchanged verified application behavior. Focused Windows [run `34054394508`](https://github.com/purnalica/fitfreed/actions/runs/34054394508) executes no packaged E2E and proves every host step through NSIS construction. The two verified local descendants are not synchronized, so no exact repository-safety or native result exists for them. | Synchronize the exact clean revision, pass repository safety, execute only the named affected platform boundaries, and reserve the complete matrix for final candidate admission. |
...
| Protected GitHub environment, immutable Releases, and Actions-backed HTTPS Pages | Open external gate | Repository-level immutable Releases are enabled. Actions-backed Pages is configured at the verified `fitfreed.org` origin with enforced HTTPS and exact-byte publication checks. The required `public-macos-release` environment and independent release reviewer are not configured; the existing `github-pages` deployment environment is not a substitute for either release approval. | Configure the remaining controls in the [operations runbook](../development/public-release-operations.md), designate the independent reviewer before release dispatch, and verify the complete state through secret-free preflight. |
...
| Corrected product experience: comprehension, trust, usefulness, natural navigation, and presentation quality | Awaiting human evaluation | The X7-R8.13 review accepted recognizable sport identity and rejected source `a447910` at XH-30. X7-R8.14 isolates and bounds both relevant pointer-range systems and passes complete exact local and hosted verification, the repeated independent audit, and revision-isolated native inspection. Automation cannot accept experience or identify the participant's internal control retrospectively. | Run one bounded uncoached product-owner experience review against the admitted revision-isolated application. |
...
| Exact-candidate product experience | Awaiting human evaluation | The prior revision-isolated X6 application was reviewed and rejected. The corrected X7-R8.14 revision-isolated application passes bundle inspection and is admitted by exact hosted executable-input evidence. Functional correctness, keyboard behavior, scaling, appearance, localized update, recovery, and automatable accessibility remain exact-candidate automation responsibilities; the product owner is not responsible for reproducing those checks. | Run the bounded experience profile. A later sealed public candidate remains a separate release gate. |
