...
| Exact 0.1.9 macOS-plus-Linux candidate, native admission, and publication | Awaiting exact candidate | Linux package and complete-platform composition engineering pass for the frozen product baseline. The immutable `v0.1.8` attempt stopped before candidate creation because its copied platform-expansion signer inherited both mutually exclusive Tauri private-key inputs; cleanup passed and nothing was published. Version 0.1.9 centralizes the corrected path-only child boundary. No immutable 0.1.9 tag, sealed candidate, Ubuntu 24.04 and 26.04 admission record, GitHub Release, or stable sequence 9 snapshot exists. | Build once from the authorized tag, admit the sealed bytes on both Ubuntu versions, then explicitly promote and remotely verify the same candidate. |
