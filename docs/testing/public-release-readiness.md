...
next publishable candidate is now version 0.1.7. The public `v0.1.0` tag is fixed at a source revision whose
...
| Signed/notarized evidence, candidate preparation, sealed transport, separate technical and human admission, immutable Release, provenance, Pages ordering, and remote verification automation | In progress for stable-metadata signing | The [public release guide](../development/public-release.md), [operations runbook](../development/public-release-operations.md), ADRs [0019](../architecture/decisions/0019-separate-candidate-build-from-public-promotion.md), [0020](../architecture/decisions/0020-compose-product-and-update-pages.md), [0046](../architecture/decisions/0046-separate-windows-candidate-build-and-admission-hosts.md), and [0047](../architecture/decisions/0047-permit-bootstrap-solo-release-approval.md), all three static workflow policies, actionlint, and synthetic orchestration tests cover the closed one-, two-, and three-platform product-site and update-channel topologies. Dispatches 0.1.0 through 0.1.5 exposed and closed the preflight, preparation-token, private-key, updater-public-key, final-DMG, and certificate-extraction gaps. Exact source `575efec` passed the required hosted checks; its `v0.1.6` [dispatch `34636087709`](https://github.com/purnalica/fitfreed/actions/runs/34636087709) passed application and final-DMG trust inspection, then exposed mutually exclusive private-key inputs at the standalone metadata signer. Current source isolates that child to the path input and preserves the bundle input only where required. | Pass focused release-boundary checks plus exact-source CI and repository safety, then obtain authority for a new immutable 0.1.7 tag and protected candidate build. Production stable-metadata signing, native exact-candidate execution, the later promotion approval, immutable predecessor releases, and publication remain separate gates. |
| Exact-source continuous integration and repository safety | Pending hosted verification for the 0.1.7 correction | Exact source `575efec` passes [product site `34630394106`](https://github.com/purnalica/fitfreed/actions/runs/34630394106), [repository safety `34630394235`](https://github.com/purnalica/fitfreed/actions/runs/34630394235), and [impact-classified continuous integration `34630394151`](https://github.com/purnalica/fitfreed/actions/runs/34630394151) without repeating packaged E2E. Its protected `v0.1.6` run then exposed the standalone signer environment conflict at a production-authority boundary that ordinary CI cannot create. | Pass the required workflows on the exact 0.1.7 correction before creating its tag. Do not repeat unchanged product E2E or performance campaigns; reserve exact native admission for the sealed candidate. |
...
| Production updater authority and embedded public trust | Passed for protected artifact signing; exact-candidate verification pending | On 2026-09-10 the project owner confirmed external generation of the production updater key pair. The checked-in configuration activates the reviewed `stable.primary-1` public key after canonical Tauri/Minisign validation. The private-key file has restricted local permissions, and both the private key and its separately supplied password are admitted only to the protected `public-macos-release` environment. The 0.1.2 and 0.1.3 executions exposed the two independent private- and public-key handoff defects. The 0.1.4 execution emitted the updater archive and signature, proving protected artifact signing without exposing or retaining authority. Exact-candidate signature reopening did not run because later DMG trust failed first. | Confirm recoverable external custody and let complete candidate verification authenticate the signature without exposing private material. |
| Developer ID Application and App Store Connect notarization authority | Passed for protected application and final-DMG signing, notarization, stapling, and objective trust inspection; exact sealed-candidate evidence pending | The workflow accepts an exact certificate fingerprint and App Store Connect team API key through ephemeral files and a temporary keychain. On 2026-09-10 the project owner confirmed an active individual Apple Developer Program membership with a future renewal date. Apple issued a G2 Developer ID Application certificate; installation of the official Apple intermediate completed its trust chain, and the local keychain reports exactly one valid identity with its private key. The protected `public-macos-release` environment contains the expected non-secret team identifier, exact certificate fingerprint, encrypted certificate bundle, separately supplied export password, App Store Connect private key, issuer identifier, and key identifier. Exact comparison verified the identifiers without recording their values here. The protected 0.1.2 through 0.1.6 executions signed the application, received Apple's accepted application notarization result, stapled the application, and signed the DMG. Version 0.1.5 additionally proved independent final-DMG notarization, log inspection, and stapling; version 0.1.6 passed the corrected objective application and disk-image trust inspection before later stable-metadata signing failed. No unsealed output was retained, and unconditional cleanup passed. | Confirm recoverable external custody and revocation procedures, then retain the passed objective trust evidence inside the exact sealed candidate. An interactive Apple ID is not required by the release workflow. |
...
| Sealed Developer ID-signed and Apple-notarized 0.1.7 candidate | Awaiting exact candidate | Candidate preparation, digest-bound transport, seven-day retention, and promotion reopening are implemented. The failed 0.1.2 through 0.1.6 executions produced only unsealed runner-local intermediate output and stopped before complete candidate verification, transport sealing, or retention. | Dispatch an authorized exact tag, approve only the protected build, and retain its sealed candidate and digest without publishing it. |
...
| Public tag, immutable GitHub Release, Pages deployment, and binary availability | Awaiting exact 0.1.7 candidate authority | The immutable `v0.1.0` through `v0.1.6` source tags exist only as rejected attempts; none produced a sealed candidate, Release, Pages deployment, or binary publication. The project owner authorized each exact tag and protected build separately; no version-specific authority implies authority for another tag, and promotion remains separately unauthorized. | After the exact-source gates pass, authorize `v0.1.7` tag creation, dispatch, and protected candidate build. Approve the separate promotion gate only after exact-candidate acceptance and explicit publication authority. |
...
FitFreed 0.1.7 may be called a public release only when every applicable row is **Passed** for one exact immutable source and candidate, both GitHub approval records correspond to that workflow run, and no required evidence belongs only to a development, synthetic, ad-hoc, private-alpha, earlier-source, or rebuilt artifact.
