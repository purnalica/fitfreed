...
The current `SYN-025` Insights slice has two complementary generated envelopes. The release-mode read-model benchmark creates ten years and four opaque origins directly in a temporary production-schema library, with daily observations and one varied training session per origin and date, to isolate indexed queries and application calculations. The packaged journey imports a two-year, one-origin provider archive with available, unavailable, and missing daily activity plus one varied training session per date before measuring Tauri, React, and WebView filtering and comparison for both domains. Their exact methods and limits are documented in the [performance benchmark guide](../development/performance-benchmarks.md).
