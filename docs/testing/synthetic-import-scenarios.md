...
The current `SYN-025` Insights slice has two complementary generated envelopes. The release-mode read-model benchmark creates ten years and four opaque origins directly in a temporary production-schema library, with daily observations, one varied training session, and one varied primary sleep period per origin and date, to isolate indexed queries and application calculations. Sleep overview and comparison use summary projections while exact detail retrieves one timeline. The packaged journey imports a two-year, one-origin provider archive with available, unavailable, and missing daily activity plus one varied training session and primary sleep period per date before measuring Tauri, React, and WebView filtering and comparison for all three domains and exact sleep detail. Their exact methods and limits are documented in the [performance benchmark guide](../development/performance-benchmarks.md).
