...
| `SYN-031` | Question-led library home and resumable exploration | Empty, partial, multi-domain, repeated, and extended libraries expose only answerable questions, derive post-import effects from the exact committed outcome, keep active import and temporary History restrictions visible across workspace changes, route positive session and sport aggregates to complete evidence, mount one selected explorer, and restore the exact Home origin or one valid versioned destination across restart. |
...
| `SYN-034` | Bundled provider sport-catalogue recognition | One training session carries a documented Polar sport identifier with no target-derived or personal classification evidence; the packaged application resolves it through the bundled catalogue, presents the correct `en-US` and `es-ES` names and provider-neutral family, drills into its exact session, preserves recognition across an exact package repeat, and reopens the same identity after a new application process starts. |
...
The implemented training-session matrix covers a minimal summary, absent optional measurements, one and multiple child exercises, fractional timestamp normalization, official duration and distance limits, invalid null and field shapes, filename/content start mismatch, duplicate source identity, ignored route and sample detail, source-subject-scoped identity, equivalent reimport, newer amendment, older preservation, equal-revision conflict, provenance, query recovery, and interrupted version 5 migration. Application and transport tests cover default and explicit ranges, multi-origin separation, exact duration and energy arithmetic, optional distance and measurement coverage, invalid facts, comparisons, and empty libraries. Component tests distinguish loading, empty, and failed reads; enter ranges; open and close detail; compare periods; clear results; reject invalid input without discarding valid history; change locale; and reload multiple sessions. The evidence-layout matrix separately covers routed, signal-only, structure-only, zone-only, unsupported-only, summary-only, exact-but-not-drawable route, and unresolved-loading compositions through one presentation decision shared by workbench leadership and detail navigation. The general packaged journey exercises multiple sessions, missing optional metrics, an amendment, cumulative addition, every training control, both locales, exact repeat, accessibility, 200% text sizing, and restart without exposing source references. The isolated `SYN-032` archive adds a signal-led session with supported gaps and a transition series, a structure-led session with two source laps and one pause, and a structured zone-bearing session with heart-rate, speed, power, and one unsupported group. Its packaged journey proves evidence priority, exact-detail focus, localized compact layout, and zero automated accessibility violations.

The provider-sport matrix covers every identifier in the bundled snapshot, deliberately unsupported fixture identifiers, equal stable provider name keys across distinct source identifiers, exact target-derived recognition, personal classification, explicit personal unification, mapping enrichment, catalogue replacement, invalid catalogue rejection, reimport, and restart. Fixture generation fails when the known bundled identifier is absent or any deliberately unknown identifier collides with the active snapshot, so catalogue evolution cannot silently invalidate an unknown-state scenario. The isolated `SYN-034` archive prevents target or personal evidence from masking the catalogue path. Its packaged journey proves that a fresh import, exact repeat, locale change, Home summary, filtered session navigation, and process restart all consume one persisted recognized sport identity.

The implemented recorded-zone matrix covers unevaluated, absent, present-empty, populated, and unsupported-group states; heart-rate, speed, and power kind/unit pairs; exact ordered bounds; missing time; speed distance; power muscle load; group and zone bounds; mapping-aware enrichment; atomic persistence; coherent snapshot invalidation; opaque capabilities; and lossless transport. Application and component tests keep missing, zero, and not-applicable aggregates distinct, preserve exact table alternatives, localize values in both supported locales, and prevent unsupported provider vocabulary from reaching presentation. The packaged journey imports fictional supported and unsupported groups and verifies their accessible session detail without inventing a timeline, lap, phase, or derived segment.
...
The implemented longitudinal matrix covers empty and partial libraries, disjoint domain bounds, the ordered union of multiple origins, unavailable and missing activity, zero and multiple training sessions per date, sleep and recovery gaps, default and explicit global ranges, unequal comparisons, inconsistent adapter bounds and catalogs, exact decimal transport, and refresh after import. Component and packaged tests enter and reset the shared range, inspect every exact daily field, navigate to the same date in every available authoritative explorer, open exact sleep and recovery detail, compare all four domain summaries, preserve a valid result after invalid input, change locale, repeat and extend imports, restart, run automated accessibility checks, and verify 200% text sizing without merging or exposing opaque origins or implying causation.

The implemented Library Home matrix covers empty and populated libraries, partial measurements, multiple origins, stable domain and question order, conservative question availability, exact-repeat and cumulative imports, superseded or failed import correlation, explicit source-review guidance, valid and stale durable destinations, restart restoration, and explicit return to Home. Application tests prove that Home delegates to the authoritative domain read models and calculates no duplicate report values. Persistence tests prove schema migration, singleton and version constraints, restart, save, and clear behavior. Presentation tests open every offered question, reject and retry a failed save, avoid mounting unrelated explorers, preserve temporary shell navigation, clear only on explicit return, and reveal only the exact committed import outcome.
...
The current `SYN-025` slice has four complementary generated envelopes. The full-scale release-mode import campaign requires a 10,000-entry provider archive with 5,999 daily-activity summaries, 4,000 training summaries, two million deliberately excluded synthetic sample items, and at least 5 GiB of expanded JSON. Seven fresh processes measure production import, exact repeat, memory, and paired activity and training queries against exact result counts. The dense-history campaign imports 520 weekly sessions with four supported 3,601-slot signals per session: exactly 2,080 series and 7,490,080 persisted samples. Three fresh processes gate import, exact repeat, memory, current-schema database size, complete-history discovery, bounded signal overview, and exact sample pagination. The release-mode read-model benchmark creates ten years and four opaque origins directly in a temporary production-schema library, with daily observations, one varied training session, one primary sleep period, and one nightly-recovery observation per origin and date, to isolate indexed queries, application calculations, and longitudinal composition. Its latest four training sessions additionally contain one million synthetic route points and 65,536 recorded zones in total; bounded route overview, exact-point pages, and complete bounded-zone queries prove their independent retrieval boundaries. Sleep and recovery overview and comparison use lightweight projections while exact detail retrieves one complete identity. The packaged journey imports a two-year, one-origin provider archive with available, unavailable, and missing daily activity plus one varied training session, primary sleep period, and recovery observation per date before measuring Tauri, React, and WebView filtering and comparison for all four detailed domains and the integrated longitudinal view, plus exact sleep and recovery detail. Their exact methods and limits are documented in the [performance benchmark guide](../development/performance-benchmarks.md).
