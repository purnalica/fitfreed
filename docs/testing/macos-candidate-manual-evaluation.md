...
5. Remove only the application and confirm the separate library remains. Reinstall the same candidate and confirm it reopens that library.
...
1. Traverse every header, language, package-selection, import, update, filter, reset, comparison, detail, close, table-scroll, and acknowledgement control in both forward and reverse order.
...
5. Open and close every detail view, follow each longitudinal navigation target, scroll every wide exact table, and clear every disposable comparison result.
...
1. Navigate by window, landmark, heading, form control, table, and link. Confirm the hierarchy identifies import, updates, each detailed explorer, longitudinal dashboard, comparisons, and details without relying on visual position.
...
7. Reopen the application and confirm restored overviews and update-recovery notices enter the navigation order predictably.
...
2. Complete import, every explorer, filter, detail, comparison, longitudinal navigation, update panel, and terminal outcome at 200%.
...
3. Interpret the terminal history effect, all five coverage totals, at least one family reason, and its next action.
4. Find an exact daily activity value, training session, sleep period, and recovery night; distinguish missing, unavailable, and zero.
...
8. Quit and reopen FitFreed, locate persisted history and locale, and find the application version and explicit update check.
