...
| Cold launch p95 at or below 2.5 seconds | One-hundred-process verification pending; reference acceptance open | The clean overlap source passed locally, but its twenty-process hosted campaign again exceeded the budget because the configured p95 estimator selected the maximum observation. The unchanged estimator and budget now run over one hundred fresh processes and report the maximum separately. | Complete local and hosted verification of the strengthened campaign, then run the unchanged command on the provisional reference profile. |
