...
| Cold launch p95 at or below 2.5 seconds | Pending clean local verification; reference acceptance open | The source-bound [process-to-painted-shell command](../development/performance-benchmarks.md#cold-launch-benchmark) and its p95 budget are versioned. Rewritten unpublished history invalidated the earlier source identity, so its result is not retained as current evidence. | Pass the unchanged command from a clean revision locally and on the provisional reference profile. |
...
No current source-bound result is retained yet. The next clean local campaign must record only privacy-safe aggregate evidence here; exact host details remain local. A non-reference result will remain regression evidence rather than an estimate of 8 GB behavior or reference-profile acceptance.
