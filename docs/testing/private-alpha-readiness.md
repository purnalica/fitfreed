...
| Cold launch p95 at or below 2.5 seconds | Hosted phase diagnosis failed; overlap remediation under verification; reference acceptance open | The clean phase-instrumented source passed locally but exceeded the hosted budget. Its phase evidence identifies synchronous library recovery followed by WebView/renderer startup as the material serialized costs. | Verify the bounded recovery/WebView overlap locally and in hosted automation, then run the unchanged command on the provisional reference profile. |
