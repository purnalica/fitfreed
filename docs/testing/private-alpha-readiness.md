...
| Cold launch p95 at or below 2.5 seconds | Hosted regression failed; reference acceptance open | The clean progressive-startup campaign passed locally and improved hosted median and p95, but the hosted p95 still exceeded the unchanged budget. Phase-level diagnosis is now required before further remediation. | Use the versioned phase evidence to correct the responsible startup phase, pass the unchanged command locally and in hosted automation, then run it on the provisional reference profile. |
