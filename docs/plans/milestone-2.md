...
**Status:** active. The overview, interactive range, and daily-detail slices are locally complete; hosted gates are pending because GitHub did not allocate runners. The version 2 Insights contract preserves the 30-day default and adds a validated explicit range of up to 366 inclusive dates. It keeps observation origins separate, distinguishes available values, unavailable values, and missing observations, and calculates exact totals and rounded averages. Schema version 5 provides its date-first query index; the responsive localized presentation provides date entry, visual and exact table alternatives, and daily detail without exposing opaque origin references. Canonical daily activity remains at version 1 until additional source-field meaning and units are established; undocumented calories, distance, metabolic, and sleep-summary values do not enter the model speculatively. Two-period comparison and complete performance evidence remain open within this increment.
...
**Acceptance evidence:** domain and mapping tests protect meaning and reconciliation; adapter and migration tests protect real-shaped input and historical libraries; UI and packaged E2E tests enter ranges, exercise every comparison control, verify exact values and accessible alternatives in both locales, reload persisted selections where specified, and cover invalid, empty, partial, and multiple-item behavior.
