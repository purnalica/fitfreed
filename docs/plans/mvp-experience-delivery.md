...
| Training discovery and session detail | Search, filters, chronology, calendar, comparison, sport classification, routed and non-routed detail, structure, signals, zones, segmentation, provenance, and return state exist | Dense continuous sections obscure progressive inspection and make the primary sports-history value difficult to scan | Workspace hierarchy, visible discovery modes, progressive session sections, exact return context, and every existing control retained |
| Activity, sleep, recovery, and aligned history | Bounded application queries, exact detail, comparisons, and cross-navigation exist | Each explorer is functional but inherits the document-width shell and weak conceptual location | Shared workspace hierarchy, unambiguous location and return, honest missing-data states, and no regression in exact evidence |
| Reports | Versioned composition, persistence, refresh, preview, privacy review, export, and origin navigation exist | The complete authoring flow is presented as a long linear surface instead of a clearly staged editor and preview workspace | Distinct report library, composition, preview/review hierarchy, preserved add/remove/reorder behavior, and origin-aware return |
