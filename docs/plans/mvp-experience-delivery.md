...
Active as of 2026-08-20. D0, P1, E2, P2, all three E3 slices, all seven E4 slices, and all six E5 verticals retain complete functional evidence, but the E6 audit reopened E1 presentation acceptance. Every planned production hierarchy correction is now implemented and passed locally: the application has explicit Home ownership, persistent responsive sidebar navigation, a broad workspace, value-first empty and populated entrances, progressive analytical and training workspaces, staged report authorship, secondary Sources, and categorized Settings. Static contracts, 177 React tests, and the complete packaged functional and performance journeys pass with real desktop and compact geometry. E1 through E6 remain release-incomplete until this committed source passes the exact local and hosted evidence campaign and the later independent product audit finds no material gap. The current exact functional baseline is recorded in the [public-release readiness ledger](../testing/public-release-readiness.md). The canonical <https://fitfreed.org/> origin has valid apex and `www` DNS, verified `purnalica` ownership, a valid certificate, enforced HTTPS, the intended redirect behavior, and exact English and Spanish hosted-byte acceptance under [ADR 0023](../architecture/decisions/0023-use-fitfreed-org-as-the-public-origin.md) and [ADR 0024](../architecture/decisions/0024-generate-localized-product-pages.md).
