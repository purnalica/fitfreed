...
| Settings and updates | Persistent language, appearance, zoom, default restoration, invalid recovery, update checks, and recovery notices exist | Corrected and passed locally: Settings separates Appearance & language from Updates and keeps one mounted update-discovery lifecycle. The later X7-R3 contract makes default restoration draft-only, cancellation non-persistent, saving the sole preference write, and top-level navigation an explicit keep-or-discard decision. | Retain the category hierarchy and verify the current transaction through the X7-R3 gates and independent product audit |
