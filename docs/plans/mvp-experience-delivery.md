...
Active as of 2026-08-20. D0, P1, E2, P2, all three E3 slices, all seven E4 slices, and all six E5 verticals retain complete functional evidence. Every previously planned production hierarchy correction is implemented: the application has explicit Home ownership, persistent responsive sidebar navigation, a broad workspace, value-first empty and populated entrances, progressive analytical and training workspaces, staged report authorship, secondary Sources, and categorized Settings. Static contracts, 179 React tests, and the complete packaged functional and performance journeys pass with real desktop and compact geometry. Exact source `45850d01dbac4519066af324bca38c69c0378425` passed [repository safety](https://github.com/purnalica/fitfreed/actions/runs/32425237419) and the complete [hosted acceptance campaign](https://github.com/purnalica/fitfreed/actions/runs/32425237418), including the immutable reusable evidence record. The subsequent [independent product-experience audit](../research/mvp-product-experience-audit.md) remains the product-owner review baseline and keeps E6 acceptance reopened: the first populated Home does not yet deliver an evidence-backed personal revelation, realistic import progress is not foregrounded, and six additional material findings plus one advisory affect recognition, interpretation, progressive disclosure, or information efficiency. PX-03 is resolved by compact recoverable SQLite schema 24 and exact local and hosted ten-year dense-history evidence. The other engineering evidence remains valid for its tested boundaries, but the product experience is not accepted. No parallel demonstration executable will replace the ordinary application during review. The current release boundary remains in the [public-release readiness ledger](../testing/public-release-readiness.md). The canonical <https://fitfreed.org/> origin has valid apex and `www` DNS, verified `purnalica` ownership, a valid certificate, enforced HTTPS, the intended redirect behavior, and exact English and Spanish hosted-byte acceptance under [ADR 0023](../architecture/decisions/0023-use-fitfreed-org-as-the-public-origin.md) and [ADR 0024](../architecture/decisions/0024-generate-localized-product-pages.md).
...
**Reopened presentation acceptance:** the E6 production audit found that E1 delivered destination behavior through a sticky top header rather than the persistent left sidebar required by the navigation contract and used a constrained document column rather than the accepted desktop workspace hierarchy. The prior React and packaged evidence exercised every control but did not assert this layout contract. Functional evidence remained valid; visual-shell acceptance did not. The complete planned correction is now implemented locally. The instrumented macOS package passed the full functional journey with native logical resizing at 1440- and 900-pixel widths, desktop and compact sidebar geometry, every workspace hierarchy, both locales, 100%–200% content zoom, current-location semantics, Axe, and no page-level horizontal overflow; the isolated Insights performance journey also passed unchanged. Closure now requires unchanged gates on the exact committed source and the independent final audit.

**Manual acceptance finding on 2026-08-20:** the first real-data evaluation scored the current UX **1 out of 5** and blocks presentation acceptance. A retained instrumented build occupied the normal production bundle path and therefore exposed a test-only archive picker that failed outside WebDriver. After rebuilding the ordinary production package, the current Polar mapping rejected four recognized training artifacts from an export accepted by an earlier mapping; the application discarded the staged import correctly but presented only a severe generic compatibility message with no useful next action. Automated success therefore did not represent the actual first-use boundary. Closure requires isolated production and instrumented artifacts, regression evidence for the provider shape, a successful real-package-compatible import, and an audit of error hierarchy, tone, explanation, and recovery guidance. No personal export, locator, or value belongs in repository evidence.
...
| Sources | Import, cancellation, reimport, coverage, acquisition guidance, and provenance exist | Corrected and passed locally: value-first Home owns an empty startup and leads into secondary Sources; the existing two-path acquisition, import, cancellation, reimport, coverage, compatibility, and provenance surface remains intact. The packaged journey retained both locales, official-link boundaries, source diagnostics, accessibility, restart, and import performance. | Retain secondary Sources and every source limitation through the exact committed-source gates and independent product audit |
| Settings and updates | Persistent language, appearance, zoom, reset, invalid recovery, update checks, and recovery notices exist | Corrected and passed locally: Settings now separates Appearance & language from Updates; switching categories preserves an unsaved preview, leaving Settings still discards it, and the continuously mounted update component keeps one discovery lifecycle. React and the packaged journey retained persistence, reset, invalid recovery, localization, restart, update actions, accessibility, and performance. | Retain the category hierarchy and every operation through the exact committed-source gates and independent product audit |
| Presentation tests | Functional React, packaged E2E, localization, focus, zoom, theme, and Axe evidence exists | Corrected and passed locally: static contracts protect the shell and every workspace boundary; React verifies inactive content, preserved disposable state, and prior behavior; the real packaged journey traverses desktop and compact geometry, both locales, 100%–200% zoom, and every production location. | Pass the exact committed-source local and hosted campaign, then the independent product audit |
...
8. After every planned E6 correction and its internal evidence are complete, run an independent product-experience audit from a clean first-run perspective. The audit must not assume that milestone status, green tests, or the accepted prototype prove the production experience. It evaluates the complete journey rather than treating UX as visual design: obtaining data, selecting it, import compatibility, comprehension, the first evidence-backed personal revelation, follow-on curiosity, recovery from failure, and only then the screen-level interaction and presentation. It records every material finding and reopens the MVP for any issue that prevents a person from understanding the promise, learning something valuable from their history, or wanting to continue within the first minutes.
...
**Independent audit result:** the 2026-08-20 [audit](../research/mvp-product-experience-audit.md) did invalidate product-experience acceptance. It recorded PX-01 through PX-03 as release-blocking, PX-04 through PX-09 as material, and PX-10 as advisory. Their original findings and exit rule remain canonical until the complete journey is audited again. PX-03 is now resolved: its long supported-signal gate and compact recoverable SQLite schema 24 pass exact clean local evidence and the complete hosted campaign for source `45850d01dbac4519066af324bca38c69c0378425`. PX-01, PX-02, and PX-04 through PX-10 remain unchanged pending product-owner disposition. Existing executable evidence is retained for its tested boundaries and is rerun only after an affected implementation enters the source.
