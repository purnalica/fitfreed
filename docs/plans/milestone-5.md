...
checkout because the new package command omitted generation of Tauri's derived Windows icon. Version 0.1.15 makes
that reusable command self-contained; exact-source CI, immutable tag, protected approvals, native candidate
...
| Allowed change | Prepare, verify, tag, and publish version 0.1.15 through the implemented unsigned public trust profile. Keep rejected 0.1.13 and 0.1.14 immutable and unpublished. Remove or archive SignPath-specific material only after the mandatory full dependency inventory. Preserve application behavior, updater and release-checksum authority, accepted native and E2E evidence, performance budgets, and stable macOS-plus-Linux claims. |
...
| M5.6 Exact candidate and promotion | In progress | The manual Windows workflow builds and seals the unsigned native input on `windows-2025`, composes one protected complete-platform candidate, admits its exact Linux and Windows packages before promotion, and publishes only after a second protected approval. Immutable public 0.1.12 satisfies the predecessor dependency. Workflow `35102076630` rejected immutable 0.1.13 before sealing because Windows npm did not forward its leading mode option. Workflow `35107772122` proved that 0.1.14 corrected that boundary and built the independent Linux input, but its clean Windows build rejected the absent generated `icons/icon.ico` before sealing. Version 0.1.15 makes the dedicated package command generate its derived icons before invoking Tauri; exact-source CI, immutable tag, protected approvals, candidate execution, immutable publication, Pages convergence, and remote verification remain. |
