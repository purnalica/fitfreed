...
| M5.4 Windows performance and reliability admission | In progress | The manual hosted performance workflow, production NSIS cold-launch lifecycle, portable Windows benchmark paths, native peak-memory measurement, and isolated NTFS disk-exhaustion recovery gate are implemented and locally contract-tested. Native execution and the remaining Windows filesystem matrix remain open. |
...
starts. The second slice creates a bounded 64 MiB NTFS VHD only on the elevated disposable runner, fills it through
...
exact VHD and removes only its validated temporary directory. Native execution, the other Windows filesystem cases,
and exact Windows 11 measurements remain open.
