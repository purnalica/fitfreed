...
| Field | Boundary |
|---|---|
| Acceptance outcome | Keep the dedicated Windows watchdog image distinct from the user-facing runnable predecessor, then launch the preserved `fitfreed.exe` when native recovery is temporarily unavailable so the user can inspect the retained state and request an explicit retry. |
| Exact evidence | Exact source `c0242f8` passes the complete local gate and focused [run `34273700195`](https://github.com/purnalica/fitfreed/actions/runs/34273700195) accepts portable evidence reuse, every exact native Windows recovery test, the release-shaped `success`, installer-failure, and candidate-failure scenarios, including localized outcome acknowledgement. The first new failure is the recovery-retry scenario. Privacy-safe artifact `10076428936` records phase `native-recovery-unavailable`, one classified `installer-failed` attempt, installed version 0.2.0, retained active and attempt state, no outcome, and neither an installed nor preserved user-facing application process. Source and history tracing locate the regression in `c120b5a`: the necessary dedicated watchdog image replaced the context's only runnable-predecessor path. `LaunchRunnablePredecessor` consequently starts `fitfreed-update-recovery.exe` without watchdog arguments. Ordinary Windows setup derives native identity from that filename, while the NSIS registration is bound to `fitfreed.exe`, so startup fails closed before the retry interface becomes available. The E2E restart helper also still targets the pre-`c120b5a` watchdog filename and has not yet reached execution. |
| Causal hypothesis | `WindowsUpdateRecoveryWatchdogContext` conflates two independently verified executable roles in the same preserved tree: `fitfreed-update-recovery.exe` is the external watchdog that survives NSIS product-process termination, while `fitfreed.exe` is the user-facing runnable predecessor. The shared path was correct before the watchdog needed a distinct image name and became invalid when only the watchdog role changed. The retained tree and state are valid; selecting the watchdog image for fallback launch causes the observed immediate exit. The complete startup and retry trace adds two consequences of that same conflation: a preserved application must enter an explicitly validated fallback mode instead of claiming ordinary installed identity, and a retry watchdog must bind its direct parent to that fallback image rather than to the fixed native installation path. |
| Smallest falsifying test | Resolve a synthetic active Windows recovery from the dedicated watchdog path and require the resulting context to expose that exact image only as its watchdog executable while exposing the sibling verified product executable as its runnable predecessor. Retain the existing rejection of `fitfreed.exe` as watchdog authority. Require the private startup parser to accept only an absolute installed target for Windows fallback and retry-watchdog modes, and require retry parent selection to use the verified runnable predecessor while initial and restart watchdogs use the installed executable. Add a script contract that requires restart interruption to target the dedicated watchdog and fallback interaction to target the product executable with the fixed installed target. These tests must fail against the conflated context and absent private modes before production code changes. |
| Allowed change | Split the Windows recovery context into explicit watchdog and runnable-predecessor executable paths derived from the already validated fixed tree. Use only the watchdog path for initial start, reattachment, and retry orchestration; use only the product path for fallback launch and observation. Add closed Windows-only private fallback and retry-watchdog startup modes carrying the manifest-bound installed target, validate fallback execution against the active attempt before renderer startup, preserve that target for intervention and retry commands even when a failed native operation has left it temporarily absent, and bind the retry watchdog to its exact fallback parent. Correct the E2E restart helper to stop the dedicated watchdog and reopen fallback WebDriver with the private fallback mode while preserving every fallback assertion against `fitfreed.exe`. Clarify the roles in current architecture, recovery-contract, testing, and contributor documentation. Do not change update phases, package bytes, manifest schema, package authority, retry limits, timeouts, application identity, user-visible behavior, or existing journey outcomes. |
| Evidence retained | Focused run `34060294715` accepts native Windows construction, installation, inventory, removal, and application-data preservation. Exact source `b94a8f4` accepts packaged Windows capability parity. Focused run `34061136225` accepts packaged Linux Insights and package removal. Focused runs through `34261252899` locate and close earlier native lifecycle boundaries. Run `34268776525` isolates acknowledgement dispatch. Run `34273700195` accepts that correction plus the first three release-shaped scenarios and locates the remaining failure at preserved fallback launch. None of those accepted boundaries may run again in this iteration. |
| Exit or stop | Run the new role-separation falsifiers first, then only affected Rust, JavaScript, workflow, documentation, repository-content, secret, format, and lint gates. Commit and push only after those gates pass, cancel the redundant push-triggered broad workflow, and dispatch `windows-update` exactly once. Stop at the first new failure; do not rerun blindly. Do not run `windows-capability`, local E2E, any Linux or macOS package campaign, Windows performance, or a complete candidate before this boundary closes. |
...
| M5.2 Windows-native update recovery | In progress | Native adapters derive current-user NSIS identity, invoke only recovery-owned silent installers, distinguish installer failure from an invalid resulting native identity, bind process control to creation time plus canonical executable path, preserve digest- and PE-identity-checked packages with a bounded complete runnable predecessor tree, and verify recovered critical files against that image. The closed version 3 state contract and Rust preparation atomically bind and reopen both packages, the runnable tree, matching library, manifest, lock files, and active authority. Serialized lifecycle mutation admits only legal transitions and binds `launching` to exact Windows process evidence; exclusive watchdog and candidate leases revalidate their distinct authorities, confirmation requires the exact leased target installation and library, restoration atomically recovers the verified source pair with a closed three-attempt failure policy, and terminal maintenance validates the complete pair before durable receipt and cleanup. Exact source `c8dab34` and focused run `34261252899` accept the native ownership and process-lifetime boundaries and reach a cleaned, running 0.2.0 installation with its localized `updated` result. Run `34268776525` proves the remaining deterministic failure is the acknowledgement command's missing Windows-state dispatch; native hosted recovery remains open at that boundary before the remaining scenarios can execute. |
