...
| Field | Boundary |
|---|---|
| Acceptance outcome | Preserve the exact Windows library and runnable locations while accepting the ordinary and verbatim path representations that Windows returns for the same native path. |
| Exact evidence | Commit `e2f7f86` distinguishes the authenticated x86 NSIS setup wrapper from the required x86-64 runnable application. Focused [run `34145475055`](https://github.com/purnalica/fitfreed/actions/runs/34145475055) on that exact source passes the Windows-native durability regression, builds and installs the packages, reaches the authorized update, preserves the recovery set, and publishes `active`. The journey then stops at its first assertion because the manifest contains the canonical verbatim library path `\\?\C:\...\fitfreed.sqlite` while the harness expects the equivalent ordinary path `C:\...\fitfreed.sqlite`. Commit `4fc696b` introduced the assertion to prove that recovery protects the scenario's exact synthetic library; its contract is path identity, not one textual Windows representation. The adjacent runnable-path assertion has the same representation risk. |
| Causal hypothesis | Applying the same narrow ordinary-versus-verbatim Windows path equivalence already used by the production recovery adapter will preserve the exact library and executable targets while allowing the native canonical representation emitted by Rust. Distinct drive paths, UNC shares, and unsupported device paths must remain unequal. |
| Smallest falsifying test | A pure JavaScript contract must equate ordinary and verbatim drive paths, equate ordinary and verbatim UNC paths, tolerate ASCII case differences, and reject a different target or an unsupported device path. The recovery journey must use that contract for both the library and runnable assertions. No package or local E2E build is required for this test. |
| Allowed change | Add the focused Windows path-text equivalence helper, its portable contract tests, and use it only in the two recovery-manifest assertions. Do not change the manifest, runtime canonicalization, recovery state, package contents, product behavior, scenarios, timeouts, or any accepted native boundary. |
| Evidence retained | Focused run `34060294715` accepts native Windows construction, installation, inventory, removal, and application-data preservation. Exact source `b94a8f4` accepts packaged Windows capability parity. Focused run `34061136225` accepts packaged Linux Insights and package removal. Focused run `34064970864` accepts both NSIS builds, predecessor installation, update discovery, and authorization. Focused run `34092015048` accepts isolation and fail-fast ordering. Focused run `34092790974` accepts icon preparation and the exact Windows-native durability regression. Focused run `34141473809` locates the setup-wrapper identity defect. Focused run `34145475055` proves its correction by reaching a durably published recovery set. None of those accepted boundaries may run again in this iteration. |
| Exit or stop | Run only the portable Windows update-harness contract locally. After the correction is verified and pushed, dispatch `windows-update` exactly once because the remaining native scenarios have not yet executed. Stop at the first new failure boundary or accept the complete recovery journey; do not retry blindly. Do not run `windows-capability`, local E2E, any Linux or macOS package campaign, Windows performance, or a complete candidate before this boundary closes. |
