...
| Field | Boundary |
|---|---|
| Acceptance outcome | Identify the exact Windows recovery-preparation stage that rejects the first real NSIS replacement, without repeating accepted package capability or building failure-only fixtures before the ordinary path succeeds. |
| Exact evidence | Focused [run `34092790974`](https://github.com/purnalica/fitfreed/actions/runs/34092790974) on exact source `4ac549a` skips every unrelated platform campaign, generates the required icons, and passes the exact Windows-native prepared-state durability regression. It then builds and installs the real predecessor, discovers and authorizes 0.2.0, and rejects preparation before publishing `update-recovery/active`. The retained screenshot exposes only the deliberately generic `update-recovery-failed` user message and the retained Webdriver log contains no backend error because the failing boundary emitted no diagnostic. The writable-file correction is therefore accepted, but it was not the complete cause of the unpublished attempt. |
| Causal hypothesis | A preparation stage after input authorization and before `active` is independently rejecting the real installed tree, package pair, library backup, manifest, reopened attempt, or authority publication. A feature-gated stage diagnostic at those exact boundaries will distinguish the cause without exposing paths or changing production behavior. Deferring construction of the installer-failure variant until its first scenario is reached removes one irrelevant NSIS build whenever the ordinary path fails. |
| Smallest falsifying test | The Windows harness unit contract must prove that the ordinary candidate is already available, the failure-only variant is absent, and the latter is requested only when its first scenario needs it. The existing focused recovery-state unit test must still publish and reopen one complete attempt. Formatting, documentation, repository-content, and secret checks must pass. No local or unrelated packaged E2E campaign is permitted. |
| Allowed change | Add feature-gated, privacy-safe recovery-preparation stage diagnostics and defer only the failure-specific candidate build. Do not change recovery validation, file durability, path equivalence, metadata, reparse-point, digest, PE identity, installation identity, process identity, transition, timeout, update, or user-interface behavior. |
| Evidence retained | Focused run `34060294715` accepts native Windows construction, installation, inventory, removal, and application-data preservation. Exact source `b94a8f4` accepts packaged Windows capability parity. Focused run `34061136225` accepts packaged Linux Insights and package removal. Focused run `34064970864` accepts both NSIS builds, predecessor installation, update discovery, and authorization. Focused run `34092015048` accepts isolation and fail-fast ordering. Focused run `34092790974` accepts icon preparation and the exact Windows-native durability regression. None of those accepted boundaries may run again in this iteration. |
| Exit or stop | Dispatch `windows-update` exactly once after local verification. Stop at the first diagnostic line and use that concrete stage and error as the cause boundary; do not retry the campaign blindly. Do not run `windows-capability`, local E2E, any Linux or macOS package campaign, Windows performance, or a complete candidate before this boundary closes. |
