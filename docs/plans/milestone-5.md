...
|---|---|---|
| M5.0 Native portability admission | In progress | Exact source `5e531e4` passes [repository safety `33993889053`](https://github.com/purnalica/fitfreed/actions/runs/33993889053), the complete portable and Linux-host job, installed Linux capability, packaged Linux update recovery, and the complete packaged macOS job in [hosted run `33993889001`](https://github.com/purnalica/fitfreed/actions/runs/33993889001). Its Windows host reaches presentation testing and exposes one test that combines four independent interaction contracts and exceeds the five-second boundary before native compilation. The current tree separates those contracts while preserving every assertion; the focused tests and all 496 presentation tests pass with two workers. Native Windows acceptance awaits the corrected exact hosted run. |
...
| M5.3 Packaged capability parity | In progress | Exact source `5e531e4` builds and installs the isolated NSIS package, and its first process passes the corrected local-library parent protection. Startup then rejects the installation because ordinary recovery reattachment still requests the fixed production package identity while the instrumented package deliberately uses `fitfreed-e2e`; its aborted WebView also exposes an immediate-cleanup race that masks the primary failure. The current tree derives only ordinary startup identity from the compiled package and running executable, retains fixed production identity for every update and recovery authority, and gives exact isolated data removal a bounded quiescence interval while preserving both failures. The complete local Rust and Windows contract suites pass. Shared capability parity and the remaining Windows-visible paths await corrected native execution. |
| M5.4 Windows performance and reliability admission | In progress | The manual hosted performance workflow, production NSIS cold-launch lifecycle, portable Windows benchmark paths, native peak-memory measurement, isolated NTFS disk-exhaustion recovery, and version 2 private-library filesystem boundary are implemented and locally contract-tested. Exact source `5e531e4` does not reach the Windows measurement campaign because the host presentation and packaged-startup gates fail first. Corrected native execution and exact Windows 11 evidence remain open. |
