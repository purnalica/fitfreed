...
| M5.5 Installation, operations, and documentation | In progress | The version-matched inactive Windows user guide covers the complete user lifecycle. Native PowerShell contributor scopes and the maintainer runbook now distinguish ordinary packaging, destructive isolated gates, protected Authenticode input, authority-free transport, version 7 composition, privacy-safe diagnostics, reproducibility, certificate rotation, compromise, and the still-absent public Windows workflow. The application now exposes offline `en-US` and `es-ES` Windows lifecycle help only on a Windows runtime, with focused component and application tests plus a Windows-conditional packaged E2E assertion. Native packaged execution, exact-candidate generation, and site integration remain open. |
...
composition, reproducibility, rotation, compromise, and the inactive Windows-publication boundary. Exact-candidate
generation and public-site activation remain separate later slices. The third slice adds Windows-only installed help:
...
pass locally; the platform-conditional packaged E2E assertion still requires the native Windows lane.
