...
| Exact evidence | Exact source `fb802e6` is synchronized and passes [repository safety run `34296600382`](https://github.com/purnalica/fitfreed/actions/runs/34296600382). Focused [Windows update run `34296638466`](https://github.com/purnalica/fitfreed/actions/runs/34296638466) passes all six exact native prerequisites and the complete release-shaped scenario set. Source `52abcb0` shares the already accepted bounded, revalidated application-data cleanup policy between isolated and production package lifecycles. Manual [Windows performance run `34302149928`](https://github.com/purnalica/fitfreed/actions/runs/34302149928) builds and installs that exact source-bound NSIS package and completes all 100 fresh-data launches without a cleanup or signal-transport failure. It rejects the measured cold-launch budget: median `1,570.249 ms`, p95 `3,363.939 ms`, and maximum `8,037.087 ms` against the unchanged `2,500 ms` p95 limit. Phase diagnostics attribute p95 `2,381.064 ms` to setup completion through renderer startup and command transport, while process creation and evidence transport remain at p95 `15.027 ms`, host startup through setup completion at p95 `939.178 ms`, locale initialization at p95 `264.3 ms`, and final paint signaling at p95 `20.8 ms`. Filesystem reliability and all data benchmarks are correctly skipped after that first rejection. Exact source `d66b3a5` then builds and installs its production NSIS package in [run `34307326155`](https://github.com/purnalica/fitfreed/actions/runs/34307326155), but the campaign stops after one process connects its startup channel and supplies no interactive-shell signal within ten seconds. The run therefore has no aggregate distribution, and all later reliability and data benchmarks are correctly skipped. |
| Causal conclusion | The named-pipe signal and shared WebView2 quiescence cleanup are effective. Static import-graph and production-bundle inspection traced the original measured interval to the renderer eagerly loading the complete application graph, both complete locale catalogs, and the complete application stylesheet before it could paint and report the shell. The failed source emitted `391.38 kB` of initial JavaScript and `224.21 kB` of initial CSS. The progressive boundary emits `206.41 kB` and `10.30 kB` respectively, applies persisted language, appearance, and zoom to a real navigable shell, then loads the complete application. Run `34307326155` exposed separate temporal coupling: the one-second product-continuation fallback canceled the only pending animation-frame observation, so a transient WebView2 paint suspension became permanent missing evidence even if a later frame was available. Product continuation and one-shot frame observation are now independent. The `2,500 ms` p95 budget and ten-second evidence timeout remain unchanged. |
| Smallest falsifying test | Focused component tests first reproduced that the one-second fallback started complete application work but permanently suppressed a later valid frame. They now prove persisted localized startup, navigation retention during deferred loading, non-blocking signal transport, continuation at one second, exactly one late-frame report, preference failure, and complete-application handoff. The production build walks the eager entry closure and rejects `App`, the complete application stylesheet, or either complete catalog. The final falsification remains one 100-process native Windows campaign; no capability, update, Linux, macOS, or local packaged E2E campaign is required. |
| Allowed change | The correction remains inside the proven setup-to-renderer startup boundary and preserves application behavior, package identity, painted-shell signal, one-second product-continuation boundary, ten-second benchmark timeout, 100-process sample, `2,500 ms` p95 budget, fixtures, cleanup policy, update behavior, platform scope, and release topology. No measurement or budget was relaxed. |
| Evidence retained | Focused run `34060294715` accepts native Windows package construction, installation, inventory, removal, and application-data preservation. Exact source `b94a8f4` accepts packaged Windows capability parity. Focused run `34061136225` accepts packaged Linux Insights and package removal. Run `34296638466` closes native Windows update recovery on exact source `fb802e6`. Run `34300497374` proves the named-pipe signal reaches the harness. Run `34302149928` additionally proves the shared cleanup correction across 100 launches and provides the first complete Windows cold-launch distribution; it is not performance acceptance. Run `34307326155` accepts the exact progressive package build and isolates a connected-channel, missing-frame failure without producing performance acceptance. |
| Exit or stop | Pass the applicable local source, documentation, formatting, repository-content, secret, and diff gates. Commit and synchronize the focused observation-lifecycle correction, then dispatch `windows-performance` exactly once on that changed source. Stop at the first new failure and do not rerun blindly. Do not run local E2E, `windows-capability`, `windows-update`, Linux or macOS package campaigns, or a complete candidate while this focused boundary is pending. |
