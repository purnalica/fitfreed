...
difference from 0.1.19, which painted all 100 processes. Version 0.1.23 created a fresh helper before every sample's
timer and closed it afterward. Exact descendant 0.1.24 additionally restored and verified the exact PID's native
window. Focused run `35282887511` admitted all 100 launches at p95 `2,107.398 ms`, but exact-candidate workflow
`35286342861` rejected the same product boundary at p95 `2,649.555 ms` after passing every preceding native,
protected, and Ubuntu gate. Both campaigns observed every painted shell and locate the difference in WebView2
startup. Immediate repeated forced teardown and restart is a benchmark-created desktop stress sequence rather than
100 independent user cold launches. Version 0.1.25 retains the same processes, activation, paint evidence, timeout,
percentile, and `2,500 ms` budget while placing a recorded unmeasured 500-millisecond desktop-settling interval
between completed samples. Exact-source CI, immutable tag, protected approvals, native admission, publication, and
