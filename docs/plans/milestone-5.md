...
| Field | Boundary |
|---|---|
| Acceptance outcome | Let the authenticated Windows runnable fallback resolve recovery startup against its bound installed target, expose the retained retry intervention, and preserve the already accepted ordering that prevents automatic update discovery during recovery. |
| Exact evidence | Commit `185c49c` sequences recovery resolution before update discovery, suppresses the one automatic check for a retained terminal outcome, and represents process replacement plus concurrent failure precedence explicitly. Focused [run `34292506635`](https://github.com/purnalica/fitfreed/actions/runs/34292506635) passes every exact native Windows state, durability, actor-ownership, process-lifetime, acknowledgement, and watchdog/fallback-role test. Its packaged campaign again passes successful replacement, installer-failure recovery, and candidate-failure recovery. In the recovery-retry scenario the first native recovery attempt fails as intended, preserves the active pointer and attempt manifest, and launches exactly one runnable fallback while no installed process remains. The fallback then presents the generic startup-confirmation failure instead of the retry intervention. Privacy-safe artifact `10082928349` records phase `native-recovery-unavailable`, one `installer-failed` attempt, installed version 0.2.0, absent terminal outcome, zero installed processes, and one runnable-predecessor process. No update-transport request occurs. |
| Causal hypothesis | `StartupMode::UpdateRecoveryFallback` authenticates and retains the absolute installed-application target in `RuntimeUpdateRecoveryTarget`. Intervention query and retry already resolve their recovery target through that state. `confirm_update_recovery_startup` does not receive it: even with no pending candidate, it calls `installed_update_recovery_target` on the currently running fallback executable inside the preserved runnable tree. The Windows resolver correctly rejects that non-installed layout, so `App` receives `update-recovery-outcome-failed`, marks startup recovery failed, and deliberately does not mount `UpdatePanel`; the still-valid intervention is never queried. This is a missing runtime-target propagation at the confirmation boundary, not failed restoration, lost state, or presentation timing. |
| Smallest falsifying test | A Rust unit test must prove that startup confirmation selects the authenticated installed target when the runtime is a fallback and retains ordinary platform installation resolution otherwise. The existing presentation tests must continue to prove confirmation-before-discovery and retained-outcome suppression. The existing Windows automation contracts must remain green, then one focused native campaign must expose the retry intervention and complete retry, exhaustion, and restart resumption without contacting closed transport. |
| Allowed change | Pass `RuntimeUpdateRecoveryTarget` into startup confirmation and centralize confirmation-target resolution so only a validated fallback uses its bound installed application; all other modes keep the existing platform resolver. Update the current Windows recovery architecture and testing documentation. Do not change package bytes, recovery phases, state schema, retry count, deadlines, watchdog behavior, platform identity, update trust, presentation behavior, release workflow topology, or product scope. |
| Evidence retained | Focused run `34060294715` accepts native Windows construction, installation, inventory, removal, and application-data preservation. Exact source `b94a8f4` accepts packaged Windows capability parity. Focused run `34061136225` accepts packaged Linux Insights and package removal. Runs through `34286543690` locate and close the earlier lifecycle, role, acknowledgement, process-replacement, transport-ordering, and diagnostic boundaries. Run `34292506635` accepts all six native prerequisites and the first three packaged recovery scenarios, retains the expected failed attempt and exact fallback process without any transport violation, and isolates only confirmation-target resolution. No Linux, macOS, capability, or performance input changed. |
| Exit or stop | Add the focused failing Rust test before production changes. Verify the affected Rust and Windows automation suites plus formatting, lint, architecture, documentation, repository-content, and secret gates; commit and push one coherent correction; cancel the redundant push-triggered broad workflow; then dispatch `windows-update` once. Stop at the first new failure and do not rerun blindly. Do not run local E2E, `windows-capability`, Linux or macOS package campaigns, Windows performance, or a complete candidate before this boundary closes. |
...
| M5.2 Windows-native update recovery | In progress | Native adapters derive current-user NSIS identity, invoke only recovery-owned silent installers, distinguish installer failure from an invalid resulting native identity, bind process control to creation time plus canonical executable path, preserve digest- and PE-identity-checked packages with a bounded complete runnable predecessor tree, and verify recovered critical files against that image. The closed version 3 state contract and Rust preparation atomically bind and reopen both packages, the runnable tree, matching library, manifest, lock files, and active authority. Serialized lifecycle mutation admits only legal transitions and binds `launching` to exact Windows process evidence; exclusive watchdog and candidate leases revalidate their distinct authorities, confirmation requires the exact leased target installation and library, restoration atomically recovers the verified source pair with a closed three-attempt failure policy, and terminal maintenance validates the complete pair before durable receipt and cleanup. Focused run `34292506635` accepts all exact native prerequisites plus successful replacement, installer-failure recovery, and candidate-failure recovery without an update-transport violation. It preserves the expected first failed retry attempt and exact runnable fallback, then isolates startup confirmation resolving that fallback executable as though it were the installed target. The current correction propagates the already authenticated installed target through confirmation while preserving ordinary platform resolution. One corrected focused native campaign remains required before M5.2 closes. |
