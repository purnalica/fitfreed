...
| Independent CI condition | Automatic [continuous-integration run `34315555299`](https://github.com/purnalica/fitfreed/actions/runs/34315555299) passes source, build, presentation, automation, Rust test, and Rust lint gates for exact source `43b04ff`, then rejects newly disclosed development-tool dependencies: `js-yaml` `4.3.1` at high severity and Vitest plus `@vitest/mocker` `4.1.10` at moderate severity. Fixed compatible releases `4.3.2` and `4.1.11` exist. This failure does not invalidate the simultaneously executing native package measurement, but the dependency lock must be corrected and the ordinary CI gate must pass before the source can be admitted. |
| Native result | Focused [Windows performance run `34315586139`](https://github.com/purnalica/fitfreed/actions/runs/34315586139) builds and installs exact source `43b04ff`, then admits all 100 fresh-data launches with median `1,650.595 ms`, p95 `2,175.046 ms`, and maximum `4,922.876 ms` against the unchanged `2,500 ms` p95 budget. Phase p95 is `17.678 ms` for process creation and evidence transport, `844.954 ms` for host startup through setup completion, `1,435.172 ms` for setup completion through renderer startup and command transport, `163.1 ms` for locale initialization, and `12.2 ms` for final paint signaling. The next filesystem gate passes five ordinary Windows library tests, then the exact disk-exhaustion case rejects the VHD volume root because NTFS owns that freshly formatted root as a system identity. Import, dense-history, and Insights gates are correctly skipped after this first new failure. |
| Causal conclusion | Critical-resource inlining closes the Windows cold-launch boundary without relaxing any measurement or timeout. The subsequent filesystem failure occurs before SQLite creation or disk exhaustion: the harness places `fitfreed.sqlite` directly under the freshly formatted NTFS volume root, while the product contract admits only a current-user-owned application-data directory and correctly rejects the system-owned root as a foreign parent. The defect originates in the initial Windows disk-exhaustion harness (`431e9d0`), predates the later ACL implementation, and was hidden until the campaign first crossed cold launch. The harness must create a dedicated application-data child on the isolated volume and expose that child—not the volume root—to the unchanged product boundary. |
| Smallest falsifying test | The PowerShell contract must require a dedicated real directory beneath the admitted NTFS root, place the isolation sentinel inside it, and bind `FITFREED_WINDOWS_FILESYSTEM_TEST_ROOT` to that child. It must continue to reject reparse points, retain the exact 64 MiB volume, real disk exhaustion, exact Rust test, junction matrix, and unconditional VHD cleanup. The existing Rust owner and ACL tests remain unchanged because the observed rejection is correct product behavior. A separate fail-closed resume contract must reopen run `34315586139`, require its successful package and cold-launch steps, require filesystem recovery as the next failure and all data gates as skipped, prove source ancestry, and reject any changed product or package graph beyond the exact development-tool correction. |
| Allowed change | Correct only the Windows filesystem-admission harness so its synthetic application-data parent has the same ownership shape as a real current-user application-data directory. Add a manual recovery-and-data workflow scope that can reuse only independently reopened predecessor evidence while rejecting product drift. Update its canonical operational and testing documentation, and apply compatible security-only development dependency revisions required by the independent CI gate. Preserve the product ACL contract, Rust recovery scenario, disk-capacity bounds, native package, cold-launch implementation and accepted metrics, benchmark budgets, fixtures, update behavior, platform scope, and release topology. |
...
| Exit or stop | Pass the focused harness and retained-evidence contracts first, then the complete applicable local source, dependency-audit, documentation, formatting, repository-content, secret, and diff gates. Commit and synchronize the harness, resume, and dependency correction. Ordinary CI must pass. Dispatch the recovery-and-data scope once with run `34315586139`; its preflight must reject drift before only the failed filesystem gate and the data gates it prevented execute. The accepted package build and 100-process cold-launch campaign must not be repeated. Stop at the first new failure and do not run capability, update, Linux, macOS, local packaged E2E, or a complete candidate while this boundary is pending. |
