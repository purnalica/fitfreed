...
|---|---|---|
| M5.0 Native portability admission | In progress | Exact source `5e9c893` passes [repository safety `34024890489`](https://github.com/purnalica/fitfreed/actions/runs/34024890489) plus the complete portable and Linux-host boundary in [hosted run `34024890491`](https://github.com/purnalica/fitfreed/actions/runs/34024890491). Windows reaches strict native Clippy, which rejects nine target-specific findings: three current-style divisibility expressions, one needless return, three test helpers admitted outside their host, and two intentionally retained lock handles without an intentional-unused name. The current tree corrects those exact boundaries without changing recovery behavior. Complete native Windows acceptance awaits one corrected exact run. |
...
| M5.3 Packaged capability parity | In progress | Exact source `5e9c893` builds and installs the isolated Linux and Windows packages, then passes both exhaustive functional journeys, distinct process restarts, adaptive session composition and restart, and sport-catalogue recognition and restart. Their Insights failures are confined to cross-WebView test driving: the Linux canvas oracle averages an unrelated accent mark into the handle y-coordinate, while WebView2 listens for pointer events and receives only mouse events. The current tree corrects both test adapters while retaining actual two-handle movement and application-responsiveness assertions. Exact hosted continuation remains open. |
| M5.4 Windows performance and reliability admission | In progress | The manual hosted performance workflow, production NSIS cold-launch lifecycle, portable Windows benchmark paths, native peak-memory measurement, isolated NTFS disk-exhaustion recovery, and version 2 private-library filesystem boundary are implemented and locally contract-tested. Exact source `5e9c893` passes the complete portable, Linux-host, Linux package update-recovery, and Windows packaged capability paths preceding Insights. Exact predecessor `d4d7a37` passes the complete macOS cold-launch, full-scale import, dense-history, Insights, installation, capability, and update-recovery campaign. The corrected Windows Insights driver, separate Windows update campaign, native Windows host, exact Windows performance, and Windows 11 evidence remain open. |
