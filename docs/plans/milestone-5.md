...
| Field | Boundary |
|---|---|
| Acceptance outcome | Compile the already verified watchdog/fallback role separation on the native Windows target, then resume the single focused recovery campaign without changing product behavior. |
| Exact evidence | Exact source `a6b7303` passes the affected local Rust, JavaScript, workflow, documentation, repository-content, secret, format, and lint gates. Focused [run `34285402952`](https://github.com/purnalica/fitfreed/actions/runs/34285402952) reuses accepted portable evidence in 17 seconds and selects only the Windows update job. Its first native step stops during test compilation with Rust error `E0382`: the platform-gated startup parser test transfers its `installed` `OsString` into the Windows resume case and then tries to clone the moved value for the new retry and fallback cases. No native test or packaged scenario executes, so the run provides no behavioral rejection of the role separation. A local macOS-to-MSVC check cannot replace native compilation because `ring` and bundled SQLite require a Windows C toolchain and SDK. |
| Causal hypothesis | The parser test predated the Windows-only retry and fallback cases. The resume assertion consumed its final platform argument because it had no later Windows consumer at that time. Commit `a6b7303` added those later consumers but retained the move. The host test passes because its Windows block is excluded by `cfg(windows)`; the first native Rust command correctly exposes the ownership error before any expensive package construction. |
| Smallest falsifying test | Compile `routes_only_exact_private_update_recovery_invocations_away_from_desktop_startup` as part of the first exact native Windows test command. The existing run is the failing evidence: compilation must accept the resume, retry, and fallback cases using the same immutable fixture value before the state-lock test can execute. |
| Allowed change | Clone the shared `installed` fixture in the Windows/Linux resume assertion so every platform-gated parser case owns its input independently. Update this execution record and run only the exact host parser test plus formatting and documentation checks locally. Do not change production code, workflow topology, package bytes, update phases, retry policy, timeouts, identity, or user-visible behavior. |
| Evidence retained | Focused run `34060294715` accepts native Windows construction, installation, inventory, removal, and application-data preservation. Exact source `b94a8f4` accepts packaged Windows capability parity. Focused run `34061136225` accepts packaged Linux Insights and package removal. Focused runs through `34261252899` locate and close earlier native lifecycle boundaries. Run `34268776525` isolates acknowledgement dispatch. Run `34273700195` accepts that correction plus the first three release-shaped recovery scenarios and locates the preserved-fallback defect. Run `34285402952` accepts the focused selection boundary and stops at the first native compilation error before rerunning any accepted scenario. |
| Exit or stop | Verify the exact host parser test, Rust formatting, documentation, repository-content, and secret gates; commit and push the test-only correction; cancel the redundant push-triggered broad workflow; then dispatch `windows-update` once. Stop at the first new failure and do not rerun blindly. Do not run local E2E, `windows-capability`, Linux or macOS package campaigns, Windows performance, or a complete candidate before this boundary closes. |
