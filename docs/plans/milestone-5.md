...
| Exact evidence | Commit `6f3824c` passes the focused local contracts and strict lint. Focused [run `34169564831`](https://github.com/purnalica/fitfreed/actions/runs/34169564831) then passes portable quality, native prepared-state durability, and the new native held-watchdog phase-read precondition. Its release-shaped success scenario installs 0.2.0 and loses the expected WebDriver session when 0.1.0 exits, but no `updated` outcome appears within 120 seconds. The prior watchdog-startup diagnostic is absent. The replacement application's setup acquires `candidate.lock` and `state.lock`, then its complete verifier tries to reopen the live watchdog's `watchdog.lock`. On Windows that peer authority is deliberately held with `share_mode(0)`, so candidate validation fails before startup confirmation. The same structural conflict exists when a live watchdog reads state while the candidate lease is held and when confirmation revalidates the peer watchdog. |
| Causal hypothesis | State changes remain serialized and manifest replacement remains atomic. Complete read verification can therefore receive one role-specific lock set: validate an owned handle directly; reopen and validate an available role; or treat only an exact Windows sharing collision as a busy peer role that grants no authority. This preserves both process leases without requiring either actor to reopen the other's no-sharing pathname. |
| Smallest falsifying test | A focused recovery-state contract must hold the watchdog lease, publish the exact replacement process and nonce, acquire one candidate lease while rejecting a second, read `launching` through the watchdog, confirm through the candidate, and read `confirmed` through the still-held watchdog. The exact test must run on native Windows before package construction so no-sharing semantics prove both peer directions. Existing exclusivity, mismatched-identity, transition, watchdog protocol, and diagnostic assertions remain intact. |
| Allowed change | Replace pathname-based held-lock skipping with typed role-lock expectations, route watchdog and candidate reads through their exact owned/busy-role contracts, and add the simultaneous-lease test as the focused native precondition. Do not change lock sharing, state serialization, atomic manifest writes, process identity, package contents, installation behavior, transitions, deadlines, user-visible copy, or any accepted path boundary. |
