...
summary while the recovery boundary required the validated source document. Version 0.1.16 separates those contracts
and exercises the production loader directly; exact-source CI, immutable tag, protected approvals, native candidate
admission, publication, and remote acceptance remain. The
...
| Allowed change | Prepare, verify, tag, and publish version 0.1.16 through the implemented unsigned public trust profile. Keep rejected 0.1.13, 0.1.14, and 0.1.15 immutable and unpublished. Remove or archive SignPath-specific material only after the mandatory full dependency inventory. Preserve application behavior, updater and release-checksum authority, accepted native and E2E evidence, performance budgets, and stable macOS-plus-Linux claims. |
...
| M5.6 Exact candidate and promotion | In progress | The manual Windows workflow builds and seals the unsigned native input on `windows-2025`, composes one protected complete-platform candidate, admits its exact Linux and Windows packages before promotion, and publishes only after a second protected approval. Immutable public 0.1.12 satisfies the predecessor dependency. Workflow `35102076630` rejected immutable 0.1.13 before sealing because Windows npm did not forward its leading mode option. Workflow `35107772122` proved that 0.1.14 corrected that boundary and built the independent Linux input, but its clean Windows build rejected the absent generated `icons/icon.ico` before sealing. Workflow `35111727698` proved that 0.1.15 corrected both native-input boundaries; its protected composition rejected the mismatch between the production upgrade-matrix summary adapter and the full document required for recovery discovery. Version 0.1.16 gives those contracts separate names and covers the real loader; exact-source CI, immutable tag, protected approvals, candidate execution, immutable publication, Pages convergence, and remote verification remain. |
