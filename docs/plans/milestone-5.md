...
| Field | Boundary |
|---|---|
| Acceptance outcome | Preserve the successful offline Windows recovery retry, prevent its automatic recovered-process startup from contacting update transport before the recovery handshake completes, and keep process replacement and concurrent failures diagnosable. |
| Exact evidence | Commit `d79637c` corrects the native-only parser ownership failure from run `34285402952`. Focused [run `34286543690`](https://github.com/purnalica/fitfreed/actions/runs/34286543690) then passes every exact native Windows state, durability, actor-ownership, process-lifetime, acknowledgement, and watchdog/fallback-role test. Its packaged campaign passes successful replacement, installer-failure recovery, and candidate-failure recovery before reaching the explicit retry for the first time. The retry restores version 0.1.0, removes active and attempt state, and launches the recovered application, but that process performs its immediate launch update check while the loopback channel remains deliberately closed. The coordinator reports `Windows recovery reached update transport while it was unavailable`; privacy-safe artifact `10080946058` records terminally absent recovery state at notice verification. The process-replacing retry click also invalidates its WebDriver session, which is expected native behavior but is not represented explicitly in the journey. The transport gate records only a count, and the concurrent coordinator can obscure the first failing branch. |
| Causal hypothesis | The watchdog's terminal recovered action correctly launches the restored installed application. `App` independently starts recovery-outcome confirmation and mounts the hidden update panel as soon as the general application-ready flag is set. The panel receives no recovery-startup ordering signal, observes no active intervention after terminal cleanup, and invokes `check_for_updates_on_launch` before the recovery outcome has been loaded. That request violates the documented offline interval even though package restoration itself uses only preserved local bytes. Separately, invoking `retry_update_recovery` intentionally exits the fallback process, so a WebDriver command may lose its session before returning success. Finally, `Promise.all` couples the journey and offline coordinator without preserving which branch failed first. |
| Smallest falsifying test | A presentation startup test must hold the update panel until `confirm_update_recovery_startup` resolves, then prove that an ordinary startup performs one launch check while a startup carrying a retained recovery outcome performs none and still permits an explicit manual check. A journey helper test must accept only session loss accompanied by exact process replacement. Transport-gate tests must report the closed request path and concurrent coordination must retain the primary journey error. The single focused native campaign must then pass the retry scenario without a request during the closed interval before continuing to exhaustion and restart resumption. |
| Allowed change | Sequence update discovery after recovery-startup resolution; suppress only the automatic launch check for a process that presents a retained terminal recovery outcome while preserving explicit manual checks and the 24-hour host schedule. Represent the retry click's verified process replacement in the Windows journey. Record privacy-safe closed-request paths and make concurrent scenario orchestration preserve the primary journey failure while still awaiting and reporting coordinator cleanup. Update current architecture, testing, automation, and user-development documentation in the same increment. Do not change package bytes, recovery phases, retry count, deadlines, platform identity, update trust, release workflow topology, or product scope. |
| Evidence retained | Focused run `34060294715` accepts native Windows construction, installation, inventory, removal, and application-data preservation. Exact source `b94a8f4` accepts packaged Windows capability parity. Focused run `34061136225` accepts packaged Linux Insights and package removal. Focused runs through `34261252899` locate and close earlier native lifecycle boundaries. Run `34268776525` isolates acknowledgement dispatch. Run `34273700195` accepts that correction plus the first three release-shaped recovery scenarios and locates the preserved-fallback defect. Run `34285402952` accepts the focused selection boundary and isolates the native compilation defect. Run `34286543690` accepts all six exact native prerequisites and the first three packaged recovery scenarios, proves successful local explicit recovery, and isolates premature launch discovery plus process-replacement synchronization. |
| Exit or stop | Add the focused failing tests before production or harness changes. Verify the affected presentation and Windows automation suites plus formatting, lint, architecture, documentation, repository-content, and secret gates; commit and push one coherent correction; cancel the redundant push-triggered broad workflow; then dispatch `windows-update` once. Stop at the first new failure and do not rerun blindly. Do not run local E2E, `windows-capability`, Linux or macOS package campaigns, Windows performance, or a complete candidate before this boundary closes. |
...
| M5.2 Windows-native update recovery | In progress | Native adapters derive current-user NSIS identity, invoke only recovery-owned silent installers, distinguish installer failure from an invalid resulting native identity, bind process control to creation time plus canonical executable path, preserve digest- and PE-identity-checked packages with a bounded complete runnable predecessor tree, and verify recovered critical files against that image. The closed version 3 state contract and Rust preparation atomically bind and reopen both packages, the runnable tree, matching library, manifest, lock files, and active authority. Serialized lifecycle mutation admits only legal transitions and binds `launching` to exact Windows process evidence; exclusive watchdog and candidate leases revalidate their distinct authorities, confirmation requires the exact leased target installation and library, restoration atomically recovers the verified source pair with a closed three-attempt failure policy, and terminal maintenance validates the complete pair before durable receipt and cleanup. Focused run `34286543690` accepts all exact native prerequisites plus successful replacement, installer-failure recovery, and candidate-failure recovery. It proves that explicit retry restores the local predecessor and reaches terminal cleanup, then isolates the recovered process's premature launch update check while transport remains closed. The current correction sequences recovery resolution before discovery, suppresses that single automatic check for a recovery-outcome startup, and makes process replacement and concurrent failures explicit. One corrected focused native campaign remains required before M5.2 closes. |
