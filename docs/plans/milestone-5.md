...
| Native result | Focused [Windows performance run `34315586139`](https://github.com/purnalica/fitfreed/actions/runs/34315586139) builds and installs exact source `43b04ff`, then admits all 100 fresh-data launches with median `1,650.595 ms`, p95 `2,175.046 ms`, and maximum `4,922.876 ms` against the unchanged `2,500 ms` p95 budget. Phase p95 is `17.678 ms` for process creation and evidence transport, `844.954 ms` for host startup through setup completion, `1,435.172 ms` for setup completion through renderer startup and command transport, `163.1 ms` for locale initialization, and `12.2 ms` for final paint signaling. Exact descendant `18e2bc1` moves the synthetic library from the system-owned VHD root to a dedicated application-data child. Focused recovery-and-data [run `34321543655`](https://github.com/purnalica/fitfreed/actions/runs/34321543655) independently reopens the retained package and cold-launch evidence, skips both accepted gates, and then stops before the native NTFS test because the clean checkout lacks Tauri's generated `src-tauri/icons/icon.ico`. Import, dense-history, and Insights gates remain correctly skipped after this first new failure. |
| Causal conclusion | Critical-resource inlining closes the Windows cold-launch boundary without relaxing any measurement or timeout. The dedicated application-data child corrects the first filesystem-harness mismatch. Run `34321543655` exposes a later automation defect rather than another product failure: the filesystem entry point invokes `cargo test`, whose Tauri build script requires generated Windows icons, but the command introduced in `431e9d0` does not generate them. Complete campaigns masked this dependency because package construction generated ignored `src-tauri/icons/` first. The shared contributor and CI command must generate the canonical icons itself so the reliability boundary is independently repeatable from a clean checkout. |
| Smallest falsifying test | The command contract must require `npm run icons` immediately before the native filesystem adapter, while the PowerShell contract continues to require the dedicated application-data child, exact 64 MiB volume, real disk exhaustion, exact Rust test, junction matrix, and unconditional VHD cleanup. The fail-closed resume contract must continue to reopen run `34315586139`, require its successful package and cold-launch steps, require filesystem recovery as the next failure and all data gates as skipped, prove source ancestry, and admit only this exact command-completeness correction in addition to the already accepted development-tool revisions. |
...
| Exit or stop | Make the shared filesystem command independently executable from a clean checkout and prove the exact retained-evidence allowance locally. Commit and synchronize that focused correction; ordinary CI must pass. Dispatch one corrected recovery-and-data scope with run `34315586139`; its preflight must reject drift before only the failed filesystem gate and the data gates it prevented execute. The accepted package build and 100-process cold-launch campaign must not be repeated. Stop at the first new failure and do not run capability, update, Linux, macOS, local packaged E2E, or a complete candidate while this boundary is pending. |
