...
| Exact evidence | Commit `3328ea6` corrects only chart resize paint and its packaged geometry oracle. Candidate run `34049176597` already passes the complete portable and Linux host, Debian construction and installation, Linux update recovery, functional journey, process restart, adaptive session composition and restart, and sport-catalogue recognition and restart before failing in packaged Insights. The existing `linux-capability` dispatch would rebuild and repeat every one of those scenarios. |
| Causal hypothesis | The packaged runner already owns seven named scenario descriptors and invokes each independently. A closed optional selection at that boundary can preserve the exact package installation and removal lifecycle while executing only `performance`; a dedicated CI scope can select it without weakening the candidate path. |
| Smallest falsifying test | Require the shared runner to execute exactly the named `performance` descriptor and reject empty, duplicate, or unknown selections. Require `linux-insights` classification and workflow admission to invoke a dedicated command that generates only the Insights fixture, builds the same package, and selects only that descriptor. The complete `linux-capability` path must remain unchanged. |
| Allowed change | The packaged scenario runner and its contracts, Linux package runner argument boundary, package commands, CI classifier and workflow, their focused contracts, automation and testing documentation, this execution record, and the readiness ledger. Product code, E2E assertions, package identity, native installation/removal behavior, budgets, and other platform jobs may not change. |
| Evidence retained | All source-bound native evidence named above remains valid. The 19 affected chart tests, 27 packaged-driver contracts, production build, documentation, presentation inventory, UI and performance policies, repository-content check, and secret scan pass for `3328ea6`. No local packaged E2E is required to prove the selection topology. |
| Exit or stop | Exit when local contracts prove the closed single-scenario dispatch. After synchronization, run `linux-insights` exactly once and stop on its first failure. Do not run `linux-capability`, any unrelated E2E scenario, or a complete candidate. |
