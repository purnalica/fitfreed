...
|---|---|---|
| M5.0 Native portability admission | In progress | Exact source `d4d7a37` passes [repository safety `34020134256`](https://github.com/purnalica/fitfreed/actions/runs/34020134256) and the complete portable and Linux-host boundary in [hosted run `34020134238`](https://github.com/purnalica/fitfreed/actions/runs/34020134238). The Windows host stops before native compilation because its portable watchdog test expects the macOS/Linux allowance while executing on Windows. The current tree derives the assertion from the executing platform and retains explicit expectations for all three targets; its focused configuration suite passes 27 tests. Complete native Windows acceptance awaits one corrected exact run. |
...
| M5.3 Packaged capability parity | In progress | Exact source `d4d7a37` builds, installs, and completes the full isolated NSIS functional journey in 14 minutes 44 seconds. Its second-process restart then queries a transient report-removal region before React publishes it. The same source's Linux package stops when the test tries to focus the instructions action while the preceding asynchronous account action still disables both controls. The current tree waits for the first terminal outcome and enabled sibling before preserving the focus assertion, and uses the established atomic live-region synchronizer after restart. Exact hosted Linux and Windows continuation remains open. |
| M5.4 Windows performance and reliability admission | In progress | The manual hosted performance workflow, production NSIS cold-launch lifecycle, portable Windows benchmark paths, native peak-memory measurement, isolated NTFS disk-exhaustion recovery, and version 2 private-library filesystem boundary are implemented and locally contract-tested. Exact source `d4d7a37` passes the complete macOS cold-launch, full-scale import, dense-history, Insights, installation, capability, and update-recovery campaign plus Linux package update recovery. The Windows capability job does not reach its separate update campaign after the restart assertion fails, and exact Windows performance and Windows 11 evidence remain open. |
