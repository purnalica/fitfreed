...
| Field | Boundary |
|---|---|
| Acceptance outcome | Native Windows update recovery accepts ordinary absolute current-user paths after Rust canonicalization while retaining link and reparse-point rejection, then completes the focused NSIS update-recovery campaign without repeating accepted capability behavior. |
| Exact evidence | Focused [run `34061992247`](https://github.com/purnalica/fitfreed/actions/runs/34061992247) on exact source `235e815` builds and installs both NSIS versions, skips the complete packaged Windows capability journey and every unrelated platform job, then stops before publishing recovery authority with `update-recovery-failed`. The retained application screenshot proves that download and authorization completed and that failure belongs to recovery preparation. [Rust documents](https://doc.rust-lang.org/std/fs/fn.canonicalize.html#platform-specific-behavior) and implements `canonicalize` on Windows with extended-length path syntax; the recovery boundary instead compared that canonical result byte-for-byte with the ordinary absolute path it had just created. The same incompatible comparison is present at the package, installed-state, and recovery-root validation boundaries. Focused Linux Insights [run `34061136225`](https://github.com/purnalica/fitfreed/actions/runs/34061136225) separately remains accepted. |
| Causal hypothesis | Recovery-root preparation creates an ordinary absolute `%APPDATA%` path and immediately rejects it because `Path::canonicalize` returns the equivalent `\\?\\` extended-length spelling on Windows. A single Windows-path equivalence rule that removes only the canonical prefix and preserves case-insensitive Windows comparison will admit the same resolved path while still rejecting a different canonical target, links, reparse points, malformed package identities, and changed files. |
| Smallest falsifying test | Pure path-contract tests must distinguish ordinary drive and UNC paths from their extended-length canonical forms and reject a different target or unsupported device namespace. All canonical-path identity guards must use that contract. The complete Rust suite, strict lint, and formatting must pass locally. The focused Windows run must compile and test the native target before its one update-recovery campaign; macOS cannot complete that cross-target check because the bundled C dependencies require the Windows SDK. |
| Allowed change | Correct only canonical Windows path equivalence and its tests at the recovery-root, installed-state, package-file, binary-inspection, and recovery-installer boundaries. Do not relax metadata type, reparse-point, package digest, PE identity, installation identity, process identity, transition, timeout, or update assertions. Do not change capability behavior or unrelated platform code. |
| Evidence retained | Focused run `34060294715` accepts native Windows construction, installation, inventory, removal, and application-data preservation. Exact source `b94a8f4` accepts packaged Windows capability parity. Focused run `34061136225` accepts packaged Linux Insights and package removal. Focused run `34061992247` accepts the isolated update scope, both NSIS builds, and predecessor installation through update authorization. None of those boundaries may run again in this iteration. |
| Exit or stop | Dispatch `windows-update` exactly once after local and cross-target verification. Exit when native update recovery passes. Stop on its first failure and record a new causal iteration. Do not run `windows-capability`, local E2E, any Linux or macOS package campaign, or a complete candidate. |
