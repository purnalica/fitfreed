...
| Field | Boundary |
|---|---|
| Acceptance outcome | Make the Windows recovery state lock a real serializer between the coordinator and watchdog, while preserving exclusive actor leases and complete active-attempt verification. |
| Exact evidence | Commit `92e5e55` separates ordinary desktop reattachment from private candidate and watchdog startup and passes its focused local contracts. Focused [run `34223611542`](https://github.com/purnalica/fitfreed/actions/runs/34223611542) accepts portable quality, native prepared-state durability, and native simultaneous watchdog/candidate ownership, but rejects the first release-shaped scenario. Its failure evidence shows the original 0.1.0 process still open, the 0.2.0 update still available, and the explicit `update-recovery-failed` installation error. The journey observed the published active attempt, so the failure occurs after publication and before successful coordinator handoff; the candidate-startup correction is valid but is not the cause of this failure. |
| Causal hypothesis | The state lock is specified as the serializer for lifecycle reads and mutations. Unix acquisition blocks on `flock`, while Windows opens the no-sharing file only once and returns a sharing violation immediately. After watchdog readiness, the watchdog polls the active phase while the coordinator concurrently transitions `prepared` to `replacement-started`; either actor can momentarily own `state.lock`. Treating that expected serialization collision as an installation failure leaves the predecessor running and discards the attempt, matching the hosted evidence. Windows acquisition must wait for the peer to release this mutex within a strict bound, without retrying any other I/O or validation error. |
| Smallest falsifying test | An exact native Windows concurrency contract holds `state.lock`, starts a second acquisition, proves that it neither succeeds nor fails while the first owner holds the lock, releases the first owner, and requires the waiting acquisition to complete inside a bounded deadline. The contract must fail against the current single-open implementation and pass before one further hosted packaged campaign. Existing lease exclusivity, complete-attempt validation, transition, process-identity, package, and recovery assertions remain intact. |
| Allowed change | Give Windows `StateLock::acquire` bounded blocking mutex semantics for exact sharing and lock violations only, with a short poll interval and an explicit timeout error. Add the native contract and its focused workflow admission, and document this cross-platform serialization contract. Do not change actor lease sharing, verifier expectations, lifecycle transitions, manifest contents, process identity, packages, installation, user-visible copy, or accepted path boundaries. |
| Evidence retained | Focused run `34060294715` accepts native Windows construction, installation, inventory, removal, and application-data preservation. Exact source `b94a8f4` accepts packaged Windows capability parity. Focused run `34061136225` accepts packaged Linux Insights and package removal. Focused run `34092790974` accepts icon preparation and native durability. Focused run `34141473809` locates the setup-wrapper identity defect. Focused run `34145475055` reaches a durably published recovery set. Focused run `34163763512` accepts ordinary/verbatim path identity. Focused run `34166654611` identifies the owning-watchdog lease-reopening defect. Focused run `34169564831` accepts its correction and reaches installed-candidate startup. Focused run `34220068393` accepts native simultaneous actor ownership. Focused run `34223611542` accepts the candidate-startup boundary and locates the coordinator/watchdog handoff failure before candidate installation. None of those accepted boundaries may run again in this iteration. |
| Exit or stop | Run only the new native state-serialization contract plus the focused recovery-state, workflow, documentation, repository-content, secret, format, and lint checks required by this correction. Commit and push it, cancel the redundant push-triggered broad workflow, and dispatch `windows-update` exactly once. Stop at the first new failure or accept the complete recovery journey; do not rerun blindly. Do not run `windows-capability`, local E2E, any Linux or macOS package campaign, Windows performance, or a complete candidate before this boundary closes. |
