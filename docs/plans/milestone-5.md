...
| Exact evidence | Exact source `fb802e6` is synchronized and passes [repository safety run `34296600382`](https://github.com/purnalica/fitfreed/actions/runs/34296600382). Focused [Windows update run `34296638466`](https://github.com/purnalica/fitfreed/actions/runs/34296638466) passes all six exact native prerequisites and the complete release-shaped scenario set. Source `52abcb0` shares the already accepted bounded, revalidated application-data cleanup policy between isolated and production package lifecycles. Manual [Windows performance run `34302149928`](https://github.com/purnalica/fitfreed/actions/runs/34302149928) builds and installs that exact source-bound NSIS package and completes all 100 fresh-data launches without a cleanup or signal-transport failure. It rejects the measured cold-launch budget: median `1,570.249 ms`, p95 `3,363.939 ms`, and maximum `8,037.087 ms` against the unchanged `2,500 ms` p95 limit. Exact source `d66b3a5` then builds and installs its production NSIS package in [run `34307326155`](https://github.com/purnalica/fitfreed/actions/runs/34307326155), but the campaign stops after one process connects its startup channel and supplies no interactive-shell signal within ten seconds. Exact source `38c6c09` separates product continuation from the retained one-shot frame observation. [Run `34309405574`](https://github.com/purnalica/fitfreed/actions/runs/34309405574) builds and installs that exact package and completes all 100 launches without missing evidence. It rejects the unchanged budget with median `2,058.387 ms`, p95 `3,402.271 ms`, and maximum `3,910.572 ms`. Phase p95 is `12.356 ms` for process creation and evidence transport, `861.707 ms` for host startup through setup completion, `2,517.245 ms` for setup completion through renderer startup and command transport, `305 ms` for locale initialization, and `7.2 ms` for final paint signaling. All later reliability and data benchmarks are correctly skipped after that first rejection. |
| Causal conclusion | The named-pipe signal, shared WebView2 quiescence cleanup, and retained late-frame observation are effective. The first progressive split removed the complete application, both complete catalogs, and the complete stylesheet, reducing eager JavaScript from `391.38 kB` to `206.39 kB` and eager CSS from `224.21 kB` to `10.30 kB`, but run `34309405574` proves that split is insufficient: the setup-to-renderer interval remains the dominant p95 at `2,517.245 ms`. A source-mapped production build contains 21 eager source modules; its largest source by an order of magnitude is the `536,016`-byte production React DOM client, which is evaluated before the renderer timestamp and shell signal. React, React DOM, Scheduler, the React shell, and the complete application do not need to precede the first interactive shell. The framework-independent correction moves that runtime behind a small imperative startup adapter whose navigation data and contracts remain shared with the React shell, reducing eager JavaScript to `13.20 kB` and `5.21 kB` gzip while retaining `10.30 kB` of critical CSS. The `2,500 ms` p95 budget and ten-second evidence timeout remain unchanged. |
| Smallest falsifying test | Startup-adapter tests prove persisted localization, appearance, zoom, accessible navigation, disabled History semantics, destination retention, continuation at one second, exactly one late-frame report, preference failure, and complete-application handoff without React. React takes ownership afterward under the existing application-startup contract. The production build rejects React, React DOM, Scheduler, `App`, `ApplicationShell`, the complete application stylesheet, and either complete catalog from the eager entry closure. The final falsification remains one 100-process native Windows campaign; no capability, update, Linux, macOS, or local packaged E2E campaign is required. |
| Allowed change | Replace the React-owned progressive root with a framework-independent startup adapter and a deferred React application root, while sharing navigation identity and icon definitions with the existing shell. Preserve application behavior, package identity, persisted preference semantics, painted-shell signal, one-second product-continuation boundary, ten-second benchmark timeout, 100-process sample, `2,500 ms` p95 budget, fixtures, cleanup policy, update behavior, platform scope, and release topology. No measurement or budget may be relaxed. |
| Evidence retained | Focused run `34060294715` accepts native Windows package construction, installation, inventory, removal, and application-data preservation. Exact source `b94a8f4` accepts packaged Windows capability parity. Focused run `34061136225` accepts packaged Linux Insights and package removal. Run `34296638466` closes native Windows update recovery on exact source `fb802e6`. Run `34300497374` proves the named-pipe signal reaches the harness. Run `34302149928` proves the shared cleanup correction across 100 launches and provides the first complete Windows cold-launch distribution. Run `34307326155` isolates the frame-observation lifecycle defect. Run `34309405574` proves that defect corrected across 100 launches and isolates the remaining eager renderer boundary; none of these performance runs is acceptance. |
| Exit or stop | Pass the applicable local source, documentation, formatting, repository-content, secret, and diff gates. Commit and synchronize one framework-deferred startup correction, then dispatch `windows-performance` exactly once on that changed source. Stop at the first new failure and do not rerun blindly. Do not run local E2E, `windows-capability`, `windows-update`, Linux or macOS package campaigns, or a complete candidate while this focused boundary is pending. |
