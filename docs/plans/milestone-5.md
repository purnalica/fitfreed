...
| Field | Boundary |
|---|---|
| Acceptance outcome | Retain a closed, privacy-safe failure snapshot that identifies the exact post-handoff recovery phase without exposing recovery identifiers, paths, process identifiers, URLs, hashes, or user data. |
| Exact evidence | Commit `52e87b4` gives the Windows state lock bounded mutex semantics and passes its focused local contracts. Focused [run `34229085774`](https://github.com/purnalica/fitfreed/actions/runs/34229085774) accepts portable quality, the new native state-lock serialization contract, native prepared-state durability, and native simultaneous watchdog/candidate ownership. Its first release-shaped `success` scenario observes the published attempt and the original 0.1.0 application exits, but no terminal `updated` receipt appears within 120 seconds. WebDriver is already unavailable when failure capture begins, and artifact `10058236520` retains only backend logs; therefore the evidence locates the defect after coordinator handoff but cannot distinguish replacement installation, candidate launch, confirmation, or terminal maintenance. |
| Causal hypothesis | The state-lock correction is valid and the remaining failure lies somewhere in the closed lifecycle interval from `replacement-started` through terminal receipt publication. The present harness destroys the active attempt during cleanup without first retaining its closed phase and recovery counters, so production behavior cannot yet be changed without guessing at the cause. |
| Smallest falsifying test | Pure diagnostic-projection tests require a closed evidence schema, retain only enumerated lifecycle and outcome values plus bounded counts and a strict version, represent absent or unreadable state explicitly, and prove that sentinel identifiers, paths, process identifiers, URLs, and hashes cannot enter the serialized artifact. The packaged journey must write that projection on failure before session and package cleanup. |
| Allowed change | Add failure-stage tracking and a race-tolerant read-only diagnostic projection to the packaged Windows update harness, plus its focused tests and documentation. Do not change recovery production code, actor ownership, locks, lifecycle transitions, timeouts, packages, installation, product behavior, user-visible copy, or accepted path boundaries. |
| Evidence retained | Focused run `34060294715` accepts native Windows construction, installation, inventory, removal, and application-data preservation. Exact source `b94a8f4` accepts packaged Windows capability parity. Focused run `34061136225` accepts packaged Linux Insights and package removal. Focused run `34092790974` accepts icon preparation and native durability. Focused run `34141473809` locates the setup-wrapper identity defect. Focused run `34145475055` reaches a durably published recovery set. Focused run `34163763512` accepts ordinary/verbatim path identity. Focused run `34166654611` identifies the owning-watchdog lease-reopening defect. Focused run `34169564831` accepts its correction and reaches installed-candidate startup. Focused run `34220068393` accepts native simultaneous actor ownership. Focused run `34223611542` locates the state-lock collision. Focused run `34229085774` accepts that correction and narrows the unresolved defect to the post-handoff lifecycle. None of those accepted boundaries may run again in this iteration. |
| Exit or stop | Run only the new pure diagnostic contracts plus the focused workflow, documentation, repository-content, secret, format, and lint checks affected by this harness-only increment. Commit and push it, cancel the redundant push-triggered broad workflow, and dispatch `windows-update` exactly once. Stop at the first new failure and use the retained phase to design the smallest native falsifier; do not rerun blindly. Do not run `windows-capability`, local E2E, any Linux or macOS package campaign, Windows performance, or a complete candidate before this boundary closes. |
