...
| Field | Boundary |
|---|---|
| Acceptance outcome | The focused Windows lane satisfies Tauri's generated-icon build precondition, executes the native prepared-state durability regression before packaging, and then completes NSIS update recovery without repeating accepted capability behavior. |
| Exact evidence | Focused [run `34092015048`](https://github.com/purnalica/fitfreed/actions/runs/34092015048) on exact source `0f891f4` skips the complete packaged capability journey and every unrelated platform job, then stops during native compilation before executing the selected test because Tauri requires the generated `icons/icon.ico`. The ordinary complete Rust command generates those versioned icons first, but the new focused command invoked Cargo directly. This is a missing harness precondition, not a durability result. The production correction remains grounded in focused [run `34064970864`](https://github.com/purnalica/fitfreed/actions/runs/34064970864): after update authorization, `sync_prepared_attempt` reopened nine closed files read-only even though the accepted Windows durability boundary requires writable regular-file handles. Source `0f891f4` routes them through the existing writable, no-reparse `sync_regular_file` adapter and adds a Windows-only complete prepared-tree regression test. |
| Causal hypothesis | Generating the canonical application icons before the selected Cargo test will let the Tauri build script finish and expose the native durability result. If that test passes, recovery preparation should cross its final file barrier: the previous first read-only `sync_all` failure can no longer remove the unpublished attempt, so recovery authority can be published without weakening file-type or reparse-point validation. |
| Smallest falsifying test | The Windows workflow contract must require `npm run icons` before the exact native Cargo test. The focused lane must stop before package construction when that test fails and proceed to its one update-recovery campaign only when it passes. Local workflow, formatting, documentation, repository-content, and secret checks must pass; production behavior and accepted E2E evidence are unchanged. |
| Allowed change | Add only the missing generated-icon precondition and its workflow contract, then update this evidence record. Do not modify the production durability correction, package construction, path equivalence, metadata type, reparse-point, digest, PE identity, installation identity, process identity, transition, timeout, update, or user-interface behavior. |
| Evidence retained | Focused run `34060294715` accepts native Windows construction, installation, inventory, removal, and application-data preservation. Exact source `b94a8f4` accepts packaged Windows capability parity. Focused run `34061136225` accepts packaged Linux Insights and package removal. Focused run `34064970864` accepts both NSIS builds, predecessor installation, update discovery, authorization, and the canonical-path correction through the later durability boundary. Focused run `34092015048` accepts isolation and fail-fast ordering: capability and every unrelated campaign remain skipped, and package construction never begins after the native prerequisite fails. None of those accepted boundaries may run again in this iteration. |
| Exit or stop | Dispatch `windows-update` exactly once after local verification. Its generated-icon precondition and native regression test must pass before the update-recovery campaign starts. Exit when native update recovery passes. Stop on the first new failure and record its causal boundary. Do not run `windows-capability`, local E2E, any Linux or macOS package campaign, Windows performance, or a complete candidate before this boundary closes. |
