...
| Field | Boundary |
|---|---|
| Acceptance outcome | A chart that changes from three to four lanes paints its controls at the current 420-pixel geometry before they can be inspected or used. Both zoom handles then move by at least 20 logical pixels through the existing sustained-input contract. |
| Exact evidence | Candidate run `34049176597`, Linux capability job `101532033591`, records a 420-pixel canvas whose initial handles remain at the preceding 360-pixel chart position. The attempted drag causes the pending 420-pixel paint to flush, moving both controls vertically without changing the selected range. The retained screenshots independently show the same 360-to-420-pixel transition. ECharts 6.0.0 `resize()` resizes zrender and performs an update whose paint remains scheduled on zrender's animation stage. |
| Causal hypothesis | The ResizeObserver calls the adapter's `resize()` after the four-lane layout changes, but no synchronous zrender flush follows it. An occluded packaged WebView can suspend the animation stage, leaving the resized canvas with the previous chart geometry until input triggers a later paint. The broad lower-band oracle introduced by `8e0e67b` admits that stale paint as a handle instead of waiting for current geometry. |
| Smallest falsifying test | A lifecycle unit test must require `resize()` followed by exactly one zrender flush. The packaged-driver contract must again admit canvas handles only in the bottom control band, so a 360-pixel paint in a 420-pixel canvas is not interactable evidence. Run the focused presentation and driver-contract tests locally; run the Linux packaged Insights boundary once only after synchronization. |
| Allowed change | The ECharts lifecycle adapter and test, the packaged chart-geometry oracle and its static contract, current chart architecture and test documentation, this execution record, and the readiness ledger. Domain, application, persistence, chart configuration, interaction semantics, budgets, and unrelated platform automation may not change. |
| Evidence retained | The same run retains the complete portable and Linux host, Debian construction and installation, Linux update recovery, and every packaged Linux capability phase before the four-lane zoom interaction. Runs `34053818461` and `34052200258` retain unchanged portable and Linux-host inputs. No local packaged E2E is required to establish the lifecycle correction. |
| Exit or stop | Exit when focused unit and static driver contracts prove synchronous resize paint and reject the stale control band. After synchronization, run only the Linux packaged Insights capability boundary. Stop on its first new failure; do not start a complete candidate or repeat an unaffected packaged journey. |
