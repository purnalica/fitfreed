...
| Field | Boundary |
|---|---|
| Acceptance outcome | The next native Windows experiment executes only release-shaped NSIS construction and installation inventory, without repeating the presentation, portable automation, Rust, updater, Authenticode, or packaged-capability evidence already retained from the same product inputs. |
| Exact evidence | Exact source `9e88fdb`, hosted run `34054394508`, job `101543534431`, passes all 496 presentation tests, the presentation build, Rust formatting, the complete Windows desktop-host tests, strict Clippy, updater tests, and release-shaped NSIS construction before the native inventory reports only `native-installation`. Local commit `0048f52` closes the missing allowlisted diagnostic contract without changing package or product behavior. The existing `windows-host` focus would nevertheless repeat every retained Windows check before reaching the one unresolved command. |
| Causal hypothesis | The focused topology is still too coarse: it treats the complete Windows host and one failed package-inventory experiment as the same boundary. A narrower explicit scope can reuse the retained evidence while preserving the exact source-bound NSIS build and native installation cycle needed to reveal the corrected diagnostic. |
| Smallest falsifying test | Classify an explicit `windows-package` dispatch, admit the Windows job for that scope, and prove from the workflow that dependency installation, NSIS construction, and inventory run while every unrelated Windows-host step and complete-evidence write remain guarded by full host verification. No product E2E is required. |
| Allowed change | The impact classifier, CI workflow, their focused topology tests, automation policy, this execution record, and the canonical readiness ledger. Product code, package identity, installation behavior, test budgets, and every other platform job may not change. |
| Evidence retained | Run `34054394508` retains successful presentation, Rust, lint, updater, and NSIS-construction evidence and explicitly skips all four unrelated packaged E2E campaigns. Runs `34053818461` and `34052200258` retain the complete portable/Linux-host boundary for the unchanged application inputs. The package must still be rebuilt because the failed run retained no exact setup artifact. |
| Exit or stop | Exit when local workflow contracts prove the narrow dispatch, then run `windows-package` exactly once after the pending commits reach `origin/main`. Stop on its first concrete installation or evidence-contract phase; do not run `windows-host`, a packaged E2E campaign, or a complete candidate. |
