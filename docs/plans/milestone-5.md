...
| Field | Boundary |
|---|---|
| Acceptance outcome | Preserve the Windows recovery watchdog across candidate installation so it can record `replacement-installed`, launch the candidate, and retain the terminal outcome. |
| Exact evidence | Commit `642b143` adds the closed failure projection without changing product behavior. Focused [run `34235494551`](https://github.com/purnalica/fitfreed/actions/runs/34235494551) accepts portable quality, native state-lock serialization, prepared-state durability, and simultaneous watchdog/candidate ownership. Its first release-shaped `success` scenario exits the original 0.1.0 process and installs version 0.2.0, but retained artifact `10061019083` records the attempt still at `replacement-started`, zero recorded replacement processes, zero installed application processes, zero runnable predecessor processes, and no terminal outcome. The candidate package therefore completed installation while the owning watchdog disappeared before it could publish the next phase. |
| Causal hypothesis | The preserved watchdog executes under the product image name `fitfreed.exe`. Candidate NSIS installation must replace that same product while the watchdog is running; the exact evidence is consistent with installer process handling terminating the watchdog by product image name after installation begins. The failure is before lifecycle transition, candidate launch, confirmation, or terminal maintenance. Installer-source inspection and one native process-survival contract must confirm or reject this mechanism before production code changes. |
| Smallest falsifying test | Inspect the exact pinned Tauri NSIS process-handling contract, then execute one Windows-native package test that keeps a recovery-owned process under the current product image name alive while the candidate NSIS package installs. The test must prove whether installation terminates that process and must separately admit a recovery image name outside the installer's product-process boundary. No complete WebDriver journey is needed to falsify this cause. |
| Allowed change | If the native test confirms the hypothesis, give the preserved recovery executable a dedicated closed filename, bind that filename through preparation, manifest validation, context resolution, reattachment, retry, restoration, and tests, while retaining the original packaged executable for predecessor restoration and identity verification. Do not change phases, leases, timeouts, package authority, application identity, user-visible behavior, or unrelated platform paths. |
| Evidence retained | Focused run `34060294715` accepts native Windows construction, installation, inventory, removal, and application-data preservation. Exact source `b94a8f4` accepts packaged Windows capability parity. Focused run `34061136225` accepts packaged Linux Insights and package removal. Focused run `34092790974` accepts icon preparation and native durability. Focused run `34141473809` locates the setup-wrapper identity defect. Focused run `34145475055` reaches a durably published recovery set. Focused run `34163763512` accepts ordinary/verbatim path identity. Focused run `34166654611` identifies the owning-watchdog lease-reopening defect. Focused run `34169564831` accepts its correction and reaches installed-candidate startup. Focused run `34220068393` accepts native simultaneous actor ownership. Focused run `34223611542` locates the state-lock collision. Focused run `34229085774` accepts that correction. Focused run `34235494551` accepts the diagnostic projection and locates the unresolved defect at watchdog survival during candidate installation. None of those accepted boundaries may run again in this iteration. |
| Exit or stop | Run only the smallest Windows-native process-survival falsifier plus focused source contracts and the workflow, documentation, repository-content, secret, format, and lint checks affected by the correction. Commit and push only after those gates pass, cancel the redundant push-triggered broad workflow, and dispatch `windows-update` exactly once. Stop at the first new failure; do not rerun blindly. Do not run `windows-capability`, local E2E, any Linux or macOS package campaign, Windows performance, or a complete candidate before this boundary closes. |
