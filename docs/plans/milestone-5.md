...
|---|---|---|
| M5.0 Native portability admission | In progress | Exact source `b94a8f4` passes the complete portable and Linux-host boundary, compiles and tests the complete Windows desktop host, passes strict native lint, and builds the release-shaped NSIS package in [hosted run `34031313078`](https://github.com/purnalica/fitfreed/actions/runs/34031313078). Native installation inventory fails after its adapter diagnostic is collapsed to a generic phase. The current tree preserves the closed safe subphase. One corrected exact native run remains. |
...
and then rejects native installation inventory. The PowerShell adapter emits a closed lifecycle subphase, but the
inventory wrapper collapses that evidence into the generic `native-installation` phase. The current tree preserves
only an allowlisted lowercase subphase in the outer diagnostic while continuing to redact adapter detail, paths, and
host state. The failed boundary therefore requires one corrected native execution rather than another undirected
package campaign. The same run passes the complete portable, Linux-host, packaged Linux capability, packaged Linux
update-recovery, and packaged macOS jobs. Its packaged Windows application passes the complete capability journey and
