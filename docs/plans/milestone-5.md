...
| Exact evidence | Exact source `fb802e6` is synchronized and passes [repository safety run `34296600382`](https://github.com/purnalica/fitfreed/actions/runs/34296600382). Focused [Windows update run `34296638466`](https://github.com/purnalica/fitfreed/actions/runs/34296638466) passes all six exact native prerequisites and the complete release-shaped scenario set: successful replacement, installer failure, candidate failure, retained-recovery retry, recovery exhaustion, and restart resumption. Every unrelated platform and packaged-capability job is skipped. Manual [Windows performance run `34032179775`](https://github.com/purnalica/fitfreed/actions/runs/34032179775) previously built and installed the source-bound NSIS package, then rejected its first cold launch because the GUI-subsystem executable could not deliver the painted-shell signal through standard output. |
| Causal hypothesis | The earlier performance rejection is a measurement-transport defect rather than a slow or failed launch: the measured application remained alive, but a Windows GUI-subsystem process has no reliable console output channel. The current implementation creates a fresh one-connection local named pipe before each launch, passes an unpredictable process-scoped identity to the child, and measures process creation, connection, painted-shell delivery, and transport inside the unchanged budget. Focused JavaScript and Rust contracts already falsify malformed, reused, inherited, or missing channels; native execution must now establish the complete package boundary. |
| Smallest falsifying test | One manual `windows-performance` workflow on the exact synchronized revision must build and install the production-identity NSIS package, pass 100 independent cold launches through the named-pipe channel, and pass the existing native filesystem reliability plus shared import, dense-history, Insights, report, and export benchmarks. A single failed assertion rejects the boundary; no broad product E2E rerun is required. |
| Allowed change | Record the accepted M5.2 evidence, then execute the existing performance workflow unchanged. Do not change budgets, sample counts, package identity, measured signal, timer boundaries, fixtures, retry policy, capability behavior, update behavior, platform scope, or release topology before the run. If it fails, diagnose the first failure to its causal boundary before considering any implementation change. |
| Evidence retained | Focused run `34060294715` accepts native Windows package construction, installation, inventory, removal, and application-data preservation. Exact source `b94a8f4` accepts packaged Windows capability parity. Focused run `34061136225` accepts packaged Linux Insights and package removal. Run `34296638466` closes native Windows update recovery on exact source `fb802e6`; no capability, Linux, macOS, or performance input changed in its correction. |
| Exit or stop | Verify and commit this documentation-only evidence transition, synchronize it, and cancel any redundant broad push-triggered workflow. Dispatch `windows-performance` exactly once. Stop at the first new failure and do not rerun blindly. Do not run local E2E, `windows-capability`, `windows-update`, Linux or macOS package campaigns, or a complete candidate while this focused boundary is pending. |
