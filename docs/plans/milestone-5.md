...
| Field | Boundary |
|---|---|
| Acceptance outcome | Native Windows update recovery durably synchronizes every closed prepared-state file through the shared writable, no-reparse regular-file adapter, publishes recovery authority, and completes the focused NSIS update-recovery campaign without repeating accepted capability behavior. |
| Exact evidence | Focused [run `34064970864`](https://github.com/purnalica/fitfreed/actions/runs/34064970864) on exact source `b4ef4c9` builds both NSIS versions, installs the predecessor, discovers and authorizes the update, skips the complete packaged Windows capability journey and every unrelated platform job, then stops before publishing recovery authority with `update-recovery-failed`. The retained application screenshot and absence of an active recovery marker place the failure inside preparation. The preceding source correction admits ordinary and extended-length paths. Static causal tracing then reaches `sync_prepared_attempt`: nine closed files are reopened by `File::open` and passed to `sync_all`, although [the version 3 recovery contract](../data-formats/release/update-recovery-v3.md) and the accepted native Windows boundary require reopening closed regular files with read and write access before flushing. The shared `sync_regular_file` adapter already implements that rule and rejects reparse points. The Windows recovery-state tests are currently compiled only on Unix, so native Windows verification did not exercise this composition. |
| Causal hypothesis | Recovery preparation reaches its final durability barrier after packages, runnable predecessor, library backup, and manifest have been created. Its first read-only `File::open(...).sync_all()` fails under Windows `FlushFileBuffers` semantics, the armed staging guard removes the unpublished attempt, the installation command returns a Windows recovery I/O error, and the presentation layer safely collapses that cause to `update-recovery-failed`. Routing every listed prepared-state file through `sync_regular_file` removes the access mismatch without weakening file-type or reparse-point validation. |
| Smallest falsifying test | Add a Windows-only recovery-state test that creates the complete minimal prepared-file tree and requires `sync_prepared_attempt` to succeed; the previous read-only implementation fails that test on Windows. Reuse `sync_regular_file` for every file in the durability barrier and keep directory synchronization unchanged. The one focused `windows-update` lane must run that native test before packaging and stop before E2E if the hypothesis is false. Local complete Rust, strict lint, formatting, documentation, repository-content, and secret checks must pass; no local packaged E2E is relevant. |
| Allowed change | Correct only the prepared-state durability barrier and add its Windows-native regression test. Do not alter package construction, path equivalence, metadata type, reparse-point, package digest, PE identity, installation identity, process identity, transition, timeout, update, or user-interface behavior. Do not change unrelated platform code. |
| Evidence retained | Focused run `34060294715` accepts native Windows construction, installation, inventory, removal, and application-data preservation. Exact source `b94a8f4` accepts packaged Windows capability parity. Focused run `34061136225` accepts packaged Linux Insights and package removal. Focused run `34064970864` accepts the isolated update scope, both NSIS builds, predecessor installation, update discovery, authorization, and the canonical-path correction through the later durability boundary. None of those accepted boundaries may run again in this iteration. |
| Exit or stop | Dispatch `windows-update` exactly once after local verification; its native regression test must pass before its update-recovery campaign starts. Exit when native update recovery passes. Stop on the first new failure and record its causal boundary. Do not run `windows-capability`, local E2E, any Linux or macOS package campaign, Windows performance, or a complete candidate before this boundary closes. |
