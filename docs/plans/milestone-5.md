...
| Exit or stop | Prove the zero-cluster pressure policy and the exact test-only retained-evidence allowance locally. Commit and synchronize that focused correction; ordinary CI must pass. Dispatch one corrected recovery-and-data scope with run `34315586139`; its preflight must reject drift before only the failed filesystem gate and the data gates it prevented execute. The accepted package build and 100-process cold-launch campaign must not be repeated. Stop at the first new failure and do not run capability, update, Linux, macOS, local packaged E2E, or a complete candidate while this boundary is pending. |
