...
| Causal conclusion | The named-pipe signal, shared WebView2 quiescence cleanup, retained late-frame observation, and framework deferral are effective. Moving React behind the startup adapter reduces the setup-to-renderer p95 from `2,517.245 ms` to `2,044.763 ms` and total p95 from `3,402.271 ms` to `2,772.774 ms`, but remains `272.774 ms` outside the budget. The production HTML still blocks its first shell on two separate asset-protocol requests: `13.20 kB` of startup JavaScript and `10.30 kB` of critical CSS. The renderer timestamp begins only after the external module has loaded and started evaluating, so those requests remain inside the dominant phase. The next correction keeps the versioned files required by deferred chunks but inlines exact copies of the critical entry and stylesheet into the generated HTML, guarded so the retained entry cannot bootstrap twice when React later imports its shared exports. The `2,500 ms` p95 budget and ten-second evidence timeout remain unchanged. |
| Smallest falsifying test | The production build must preserve every existing eager-graph rejection and emit generated HTML with one inline module and one inline critical stylesheet, no blocking entry `src` or critical stylesheet `href`, no source map, and exact byte equality with the retained versioned assets. Deferred chunks must still resolve the retained entry, while a process-global one-shot guard prevents its second module identity from starting the application twice. Existing startup-adapter tests continue to prove persisted localization, accessible navigation, deferred React handoff, and late-frame evidence. The final falsification remains one 100-process native Windows campaign; no capability, update, Linux, macOS, or local packaged E2E campaign is required. |
| Allowed change | Add deterministic production-build inlining for only the eager entry and its critical stylesheet plus a one-shot bootstrap guard. Preserve source modules, deferred chunk resolution, application behavior, package identity, persisted preference semantics, painted-shell signal, one-second product-continuation boundary, ten-second benchmark timeout, 100-process sample, `2,500 ms` p95 budget, fixtures, cleanup policy, update behavior, platform scope, and release topology. No measurement or budget may be relaxed. |
| Evidence retained | Focused run `34060294715` accepts native Windows package construction, installation, inventory, removal, and application-data preservation. Exact source `b94a8f4` accepts packaged Windows capability parity. Focused run `34061136225` accepts packaged Linux Insights and package removal. Run `34296638466` closes native Windows update recovery on exact source `fb802e6`. Run `34300497374` proves the named-pipe signal reaches the harness. Run `34302149928` provides the first complete Windows cold-launch distribution. Run `34307326155` isolates the frame-observation lifecycle defect. Run `34309405574` proves that defect corrected and isolates the eager framework boundary. Run `34312164102` proves framework deferral across 100 launches and isolates the remaining external critical-resource boundary; none of these performance runs is acceptance. |
| Exit or stop | Pass the applicable local source, documentation, formatting, repository-content, secret, and diff gates. Commit and synchronize one critical-resource delivery correction, then dispatch `windows-performance` exactly once on that changed source. Stop at the first new failure and do not rerun blindly. Do not run local E2E, `windows-capability`, `windows-update`, Linux or macOS package campaigns, or a complete candidate while this focused boundary is pending. |
