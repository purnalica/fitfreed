...
| Exact evidence | Exact source `fb802e6` is synchronized and passes [repository safety run `34296600382`](https://github.com/purnalica/fitfreed/actions/runs/34296600382). Focused [Windows update run `34296638466`](https://github.com/purnalica/fitfreed/actions/runs/34296638466) passes all six exact native prerequisites and the complete release-shaped scenario set. Documentation-only descendant `546b1d2` passes repository safety and impact-aware CI, then manual [Windows performance run `34300497374`](https://github.com/purnalica/fitfreed/actions/runs/34300497374) builds and installs the exact source-bound NSIS package. The cold-launch step advances beyond the former absent-output failure, completes an interactive launch, terminates its direct application process, and then fails before the next measurement while `reset-data` removes the exact local application-data directory: WebView2 still holds `EBWebView/Default/declarative_performance_observer.db`. Filesystem reliability and all data benchmarks are correctly skipped after that first failure. |
| Causal hypothesis | The named-pipe transport correction is effective and exposes the next lifecycle boundary. `measureFreshProcess` sends termination to the exact spawned application and waits for that direct process to exit. The following `reset-data` action sees no remaining installed FitFreed process, but a short-lived WebView2 descendant can still hold its private database. `Remove-OwnedData` validates the owned root and reparse-point boundary, then attempts removal only once. The isolated packaged-E2E lifecycle encountered the same Windows child-process quiescence behavior and gained bounded, revalidated cleanup in commit `5da60b7`; the production-package performance lifecycle retained its earlier one-shot copy. This is duplicated cleanup-policy drift made reachable by the corrected signal transport, not an application startup, budget, package-identity, or arbitrary filesystem failure. |
| Smallest falsifying test | Adapt the existing Windows automation contracts to require one shared application-data cleanup policy for both isolated packaged E2E and production-package performance. The policy must revalidate the exact non-reparse root and descendants on every bounded attempt, retry only the owned directory, and fail after the existing finite interval. The focused Windows script suite must pass, then one manual `windows-performance` workflow must complete all 100 independent cold launches before proceeding through native filesystem reliability and the shared import, dense-history, Insights, report, and export benchmarks. |
| Allowed change | Extract the already accepted bounded Windows application-data cleanup behavior into one repository-owned PowerShell helper and make both package lifecycle scripts use it. Preserve their distinct identity, sentinel, installation, removal, and diagnostic contracts. Do not change budgets, sample counts, package identity, measured signal, timer boundaries, fixtures, application termination, retry duration, capability behavior, update behavior, platform scope, or release topology. |
| Evidence retained | Focused run `34060294715` accepts native Windows package construction, installation, inventory, removal, and application-data preservation. Exact source `b94a8f4` accepts packaged Windows capability parity. Focused run `34061136225` accepts packaged Linux Insights and package removal. Run `34296638466` closes native Windows update recovery on exact source `fb802e6`. Run `34300497374` accepts package construction and proves that the named-pipe signal reaches the harness; it rejects only the next owned-data cleanup boundary and is not performance acceptance. |
| Exit or stop | Add the focused failing automation contracts before changing the shared cleanup policy. Verify the Windows script suite plus documentation, formatting, repository-content, secret, and diff gates; commit and synchronize one coherent correction. Dispatch `windows-performance` exactly once. Stop at the first new failure and do not rerun blindly. Do not run local E2E, `windows-capability`, `windows-update`, Linux or macOS package campaigns, or a complete candidate while this focused boundary is pending. |
