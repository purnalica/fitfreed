...
| Field | Boundary |
|---|---|
| Acceptance outcome | A failed native Windows installation boundary retains one allowlisted, privacy-safe decision code through the JavaScript inventory boundary, including failures in returned-evidence syntax or contract validation. |
| Exact evidence | Exact source `9e88fdb`, hosted run `34054394508`, job `101543534431`, passes all 496 presentation tests, the presentation build, Rust formatting, the complete Windows desktop-host tests, strict Clippy, updater tests, and release-shaped NSIS construction. The 69-second native inventory command then reports only `native-installation`. Static tracing shows that this generic result is possible after the PowerShell process returns when evidence parsing or contract validation raises an error outside the one recognized lifecycle-error shape. |
| Causal hypothesis | The native verifier has a closed PowerShell lifecycle vocabulary, but its JavaScript evidence handoff has no equivalent closed vocabulary. The inventory wrapper therefore erases a returned-evidence failure that is already safe to classify, leaving no basis for a package correction. Its lifecycle parser is also tested only with LF although native Windows emits CRLF, so the complete diagnostic boundary is not yet falsified under its real transport. |
| Smallest falsifying test | Pass CRLF lifecycle output, invalid JSON evidence, and one contract-invalid facts object through the verifier and then the inventory wrapper. Each must produce its distinct allowlisted phase, while an injected unknown phase must still collapse to the generic boundary. No packaged or end-to-end journey is required. |
| Allowed change | The Windows installation verifier, inventory wrapper, their focused contract tests, troubleshooting guidance, automation policy, and this exact evidence record. Package identity, installation behavior, product code, test budgets, and unrelated platform jobs may not change. |
| Evidence retained | Run `34054394508` retains successful presentation, Rust, lint, updater, and NSIS-construction evidence and explicitly skips all four unrelated packaged E2E campaigns. Runs `34053818461` and `34052200258` retain the complete portable/Linux-host boundary for the unchanged application inputs. |
| Exit or stop | Exit the diagnostic increment when every declared safe phase survives both JavaScript layers locally, then run the focused Windows host exactly once. Stop on its first concrete installation or evidence-contract phase; do not repeat an already successful product journey or guess a package fix. |
