...
| Field | Boundary |
|---|---|
| Acceptance outcome | An explicit `windows-update` dispatch builds only the two instrumented NSIS packages and exercises native update and recovery without repeating the already accepted packaged Windows capability journey. |
| Exact evidence | Exact source `b94a8f4` passes the complete packaged Windows capability journey and builds both update packages before stopping on the stale `packageActionScript` reference. The current tree exports the canonical action-script path from the installed-package boundary and proves that the update harness consumes it. Focused Linux Insights [run `34061136225`](https://github.com/purnalica/fitfreed/actions/runs/34061136225) separately accepts the chart correction from `3328ea6` on exact source `f1bf3a3`; it executes only Insights while every other packaged scenario and platform job is skipped. |
| Causal hypothesis | Commit `8f638cb` centralized installed-package invocation but left the update harness with one removed local variable. Reusing the exported canonical script path removes that reference failure; no packaged capability behavior changed, so repeating its accepted journey cannot test this hypothesis. |
| Smallest falsifying test | Workflow contracts must admit a closed `windows-update` scope, skip `verify:windows-e2e` for that scope, and run `verify:windows-update-e2e`. The one hosted run must then execute the complete native NSIS update-recovery campaign and retain only its privacy-safe failure evidence. |
| Allowed change | Add only the focused scope and its fail-closed workflow contracts before the hosted run. Do not change product behavior, update assertions, package identity, recovery semantics, budgets, or already accepted capability scenarios. If the hosted run fails, only its retained update evidence may define the next causal iteration. |
| Evidence retained | Focused run `34060294715` accepts native Windows construction, installation, inventory, removal, and application-data preservation. Exact source `b94a8f4` accepts packaged Windows capability parity. Focused run `34061136225` accepts packaged Linux Insights and package removal. None of those boundaries may run again in this iteration. |
| Exit or stop | Dispatch `windows-update` exactly once and exit when native update recovery passes. Stop on its first failure and record a new causal iteration. Do not run `windows-capability`, local E2E, any Linux or macOS package campaign, or a complete candidate. |
