...
| Field | Boundary |
|---|---|
| Acceptance outcome | Explicitly acknowledge the verified Windows update result even when a short-lived native reader temporarily denies deletion of the durable outcome receipt. |
| Exact evidence | Exact source `c8dab34` passes the complete clean local gate. Focused [run `34261252899`](https://github.com/purnalica/fitfreed/actions/runs/34261252899) accepts native state-lock serialization, prepared-state durability, simultaneous watchdog/candidate ownership, and the new watchdog process-lifetime boundary. Its release-shaped `success` scenario installs and runs 0.2.0, retains `updated`, removes the active pointer and complete attempt, and presents the localized success result. The final acknowledgement fails and leaves the receipt. Privacy-safe artifact `10071049126` records `notice-verification`, absent active and attempt-manifest state, retained `updated`, installed 0.2.0, one installed application process, and no predecessor process. The journey had already required the entire attempt directory to be absent before that restart, so terminal recovery cleanup is no longer the failing boundary. |
| Causal hypothesis | Outcome acknowledgement owns the receipt lock but performs one `DeleteFile` attempt. Windows documents that deletion fails while another handle omits delete sharing or while the file is memory-mapped. A short-lived native reader can therefore reject this final user action even though the update, receipt, and cleanup are valid. The failure is after result presentation and does not implicate installation, recovery, or the library. |
| Smallest falsifying test | On native Windows, open the real outcome receipt without delete sharing, prove direct deletion is denied, release that handle from another thread, and require the production removal path to acknowledge the outcome within its bounded deadline. Run this exact test before package construction. |
| Allowed change | Retry receipt removal only on native Windows `ERROR_ACCESS_DENIED` or `ERROR_SHARING_VIOLATION`, with a five-second deadline and fixed polling interval. Preserve the outcome lock, exact receipt validation, attempt-absence requirement, durable removal, user-visible failure on exhaustion, and every existing E2E assertion. Do not change update phases, package authority, recovery cleanup, application identity, other platform behavior, or functional scope. |
| Evidence retained | Focused run `34060294715` accepts native Windows construction, installation, inventory, removal, and application-data preservation. Exact source `b94a8f4` accepts packaged Windows capability parity. Focused run `34061136225` accepts packaged Linux Insights and package removal. Focused run `34092790974` accepts icon preparation and native durability. Focused runs through `34235494551` locate and close earlier native lifecycle boundaries. Run `34252175539` rejects a DELETE-access proxy before packaging. Run `34261252899` accepts the process-lifetime correction and locates the remaining failure at receipt acknowledgement. None of those accepted boundaries may run again in this iteration. |
| Exit or stop | Run only the outcome-store tests, Windows target source check where the local toolchain permits it, focused workflow and documentation contracts, and repository safety gates. Commit and push only after those gates pass, cancel the redundant push-triggered broad workflow, and dispatch `windows-update` exactly once. Stop at the first new failure; do not rerun blindly. Do not run `windows-capability`, local E2E, any Linux or macOS package campaign, Windows performance, or a complete candidate before this boundary closes. |
