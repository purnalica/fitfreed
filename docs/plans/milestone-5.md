...
| Field | Boundary |
|---|---|
| Acceptance outcome | Keep ordinary interrupted-recovery reattachment out of an already launched update candidate, while preserving strict active-attempt verification for a genuine desktop restart. |
| Exact evidence | Commit `4e7d7fc` passes the focused local contracts and repairs the Windows-only lock-opener return contract. Focused [run `34220068393`](https://github.com/purnalica/fitfreed/actions/runs/34220068393) passes portable quality, native prepared-state durability, and the exact native simultaneous-watchdog-and-candidate lease test. The release-shaped success scenario then installs 0.2.0 but observes no `updated` outcome within 120 seconds. The installed candidate acquires and retains `candidate.lock` during Tauri setup, after which the host unconditionally invokes ordinary-startup watchdog reattachment. Its unowned active-attempt resolver must reopen every lock for complete validation; Windows correctly rejects reopening the candidate's own no-sharing handle before the resolver can observe the already live watchdog. The candidate setup therefore aborts before confirmation. |
| Causal hypothesis | Watchdog reattachment belongs only to `Desktop` startup. A recovery candidate is already the replacement actor inside a watchdog-owned attempt: after it validates and holds its candidate lease, it must continue to library preparation and confirmation rather than resolve or start a second watchdog. Explicitly deriving reattachment eligibility from the private startup mode removes the invalid self-conflict without relaxing lock validation or lifecycle authority. |
| Smallest falsifying test | A host unit contract requires ordinary `Desktop` startup to reattach and rejects reattachment for candidate, watchdog, and invalid private startup modes. It must fail before the orchestration changes, then pass with the focused host and recovery-state suites. The hosted campaign must still pass native prepared-state durability and simultaneous actor ownership before building packages, then produce the durable `updated` outcome for its first release-shaped scenario. Existing exclusivity, complete-attempt validation, restart, retry, transition, process-identity, and package assertions remain intact. |
| Allowed change | Derive watchdog-reattachment eligibility from `StartupMode`, run Linux and Windows reattachment only for ordinary desktop startup, and document that actor boundary. Do not change lock sharing, verifier expectations, state serialization, manifest writes, process identity, packages, installation, transitions, deadlines, user-visible copy, or any accepted path boundary. |
| Evidence retained | Focused run `34060294715` accepts native Windows construction, installation, inventory, removal, and application-data preservation. Exact source `b94a8f4` accepts packaged Windows capability parity. Focused run `34061136225` accepts packaged Linux Insights and package removal. Focused run `34092790974` accepts icon preparation and native durability. Focused run `34141473809` locates the setup-wrapper identity defect. Focused run `34145475055` reaches a durably published recovery set. Focused run `34163763512` accepts ordinary/verbatim path identity. Focused run `34166654611` identifies the owning-watchdog lease-reopening defect. Focused run `34169564831` accepts its correction and reaches installed-candidate startup. Focused run `34220068393` accepts native simultaneous actor ownership and locates the remaining candidate-host orchestration defect. None of those accepted boundaries may run again in this iteration. |
| Exit or stop | Run only the focused host, recovery-state, workflow, documentation, repository-content, and secret checks required by this orchestration correction. Commit and push it, cancel the redundant push-triggered broad workflow, and dispatch `windows-update` exactly once. Stop at the first new failure or accept the complete recovery journey; do not rerun blindly. Do not run `windows-capability`, local E2E, any Linux or macOS package campaign, Windows performance, or a complete candidate before this boundary closes. |
