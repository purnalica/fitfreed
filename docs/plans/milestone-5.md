...
| Field | Boundary |
|---|---|
| Acceptance outcome | The release-shaped Windows NSIS package installs into a clean current-user profile, emits a complete inventory whose paths are uniquely ordered by UTF-8 bytes, removes every package-owned artifact, and preserves application data. |
| Exact evidence | Exact source `6d0b51f` builds the NSIS package in focused [run `34059108525`](https://github.com/purnalica/fitfreed/actions/runs/34059108525), completes the native install-inspect-remove cycle, and is rejected only at `native-installation/evidence-entry-order`. The native adapter invokes Windows PowerShell 5.1 through `powershell.exe`; commit `7b50b90` introduced `Sort-Object -Property sortKey` over ordered dictionaries. PowerShell documents hash-table key sorting as available only from PowerShell 6, so 5.1 observes no sortable `sortKey` property and preserves an unsuitable filesystem enumeration. |
| Causal hypothesis | Materializing each temporary sort record as a `PSCustomObject` exposes `sortKey` as a real property to Windows PowerShell 5.1. The existing hexadecimal UTF-8 key then retains the normative byte order, and the independent JavaScript validator accepts the native evidence without changing the inventory contract. |
| Smallest falsifying test | Require the versioned PowerShell adapter to materialize the sortable record as a `PSCustomObject` before `Sort-Object`, while preserving the UTF-8 hexadecimal key and the independent JavaScript rejection of reversed entries. Run only the Windows installation and inventory contracts. |
| Allowed change | The Windows native installation adapter, its source-level compatibility contract, the current execution record, release-delivery explanation, and readiness ledger. The inventory schema, byte-order requirement, package contents, installer identity, E2E journeys, budgets, and other platform jobs may not change. |
| Evidence retained | All source-bound evidence from `6d0b51f` remains valid except the Windows installed-entry ordering outcome. The focused run skipped every packaged E2E campaign and proves package construction plus the preceding installation/removal observations. The Linux Insights correction and its closed dispatch remain locally and remotely verified at their focused contract boundary; native `linux-insights` execution remains queued until this Windows boundary closes. |
| Exit or stop | Exit when the focused contracts pass and one exact `windows-package` dispatch accepts installation inventory. Stop on any different native phase and record a new causal iteration before another correction. Do not run Windows capability E2E, Linux E2E, or a complete candidate in this iteration. |

| Increment | Status | Current evidence boundary |
|---|---|---|
| M5.0 Native portability admission | In progress | Exact source `6d0b51f` uses the package-only scope in [hosted run `34059108525`](https://github.com/purnalica/fitfreed/actions/runs/34059108525): it reuses the already accepted host boundary, builds the release-shaped NSIS package, and reaches `native-installation/evidence-entry-order` without starting any packaged E2E campaign. The current tree exposes the UTF-8 sort key as a real `PSCustomObject` property to the invoked Windows PowerShell 5.1 adapter while retaining independent byte-order validation. One corrected exact native run remains. |
