...
| Field | Boundary |
|---|---|
| Acceptance outcome | Identify the exact pre-readiness stage that prevents the preserved Windows recovery watchdog from starting, without repeating accepted capability or path-identity work. |
| Exact evidence | Commit `0ceb18f` preserves exact Windows path identity under ordinary and verbatim representations. Focused [run `34163763512`](https://github.com/purnalica/fitfreed/actions/runs/34163763512) passes the Windows-native durability regression, builds and installs the two ordinary NSIS packages, discovers and authorizes 0.2.0, accepts both recovery-manifest path assertions, and then reports `update-watchdog-failed`. The candidate is not installed and no `updated` receipt appears. The preserved child can fail before readiness while resolving its active context, acquiring its exclusive lease, reopening the active phase, binding its direct parent process, or writing the readiness record. Its standard error is deliberately discarded, so the retained Webdriver log cannot distinguish those stages. |
| Causal hypothesis | A feature-gated, privacy-safe diagnostic emitted by the preserved child and inherited through the already captured E2E backend stream will identify the first failed startup stage. The production build must continue to discard child diagnostics and present the unchanged generic recovery error. |
| Smallest falsifying test | A focused Rust contract must prove that each failed startup operation emits only its fixed stage and error category to a supplied diagnostic writer while a successful operation emits nothing. A source boundary must keep child standard error null in production and inherited only in Windows E2E builds. Existing watchdog protocol and recovery-state tests must still pass; no package or local E2E build is required. |
| Allowed change | Add only feature-gated watchdog-startup diagnostics and their focused contracts. Do not change recovery validation, leases, process identity, readiness transport, package contents, installation behavior, transitions, timeouts, user-visible copy, or any accepted path boundary. |
| Evidence retained | Focused run `34060294715` accepts native Windows construction, installation, inventory, removal, and application-data preservation. Exact source `b94a8f4` accepts packaged Windows capability parity. Focused run `34061136225` accepts packaged Linux Insights and package removal. Focused run `34092790974` accepts icon preparation and native durability. Focused run `34141473809` locates the setup-wrapper identity defect. Focused run `34145475055` reaches a durably published recovery set. Focused run `34163763512` accepts the ordinary/verbatim path contract and stops only at watchdog readiness. None of those accepted boundaries may run again in this iteration. |
| Exit or stop | Run only focused Rust, documentation, repository-content, and secret checks locally. Commit and push the diagnostic increment, cancel the redundant push-triggered broad workflow, and dispatch `windows-update` exactly once. Stop at its first fixed diagnostic stage and use that evidence to design the correction; do not rerun blindly. Do not run `windows-capability`, local E2E, any Linux or macOS package campaign, Windows performance, or a complete candidate before this boundary closes. |
