...
|---|---|---|
| M5.0 Native portability admission | In progress | Exact source `57a96ec` passes the complete portable and Linux-host boundary in [hosted run `34027883481`](https://github.com/purnalica/fitfreed/actions/runs/34027883481). The Windows host stops before native compilation because one presentation test exceeds its five-second budget by 50 ms and continues into the following case. The current tree uses realistic paste entry for bulk values while preserving every validation, persistence, editing, reopening, privacy, focus, and export assertion. Complete native Windows acceptance awaits one corrected exact run. |
