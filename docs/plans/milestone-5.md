...
remote verification pipeline are implemented and pass their local contracts. Immutable 0.1.13 was rejected before
candidate sealing when Windows npm consumed the leading mode option instead of forwarding it to the native-input
script. Immutable 0.1.14 corrected that invocation and reached the native Windows build, which rejected the clean
checkout because the new package command omitted generation of Tauri's derived Windows icon. Immutable 0.1.15 made
that reusable command self-contained and passed both native input jobs. Its protected composition then rejected an
internal adapter mismatch before building macOS: the production upgrade-matrix inspector returned its documented
summary while the recovery boundary required the validated source document. Version 0.1.16 separated those contracts,
passed composition, signing, notarization, and sealing, then exposed two independent admission-contract defects:
Linux still accepted only the former two-platform manifest, while the cross-platform transport carried a non-published
macOS application directory whose link metadata cannot be reproduced on hosted Windows. Version 0.1.17 made the
sealed boundary the portable public distribution, admitted manifest versions 6 through 8 on Linux, and passed both
...
that focus remains with the window after a successful activation. Version 0.1.22 kept that persistent helper and tree
isolation while reasserting the exact PID at a bounded cadence until the painted-shell signal or unchanged timeout.
It passed composition and both Ubuntu admissions but reproduced the missing painted-shell failure on Windows. This
falsifies loss of focus after one activation and isolates the shared automation session as the remaining lifecycle
difference from 0.1.19, which painted all 100 processes. Version 0.1.23 creates a fresh helper before every sample's
timer and closes it afterward, without changing product code or the measured activation, process, paint, signal,
sample-count, timeout, percentile, or budget boundaries. Exact-source CI, immutable tag, protected approvals, native
admission, publication, and remote acceptance remain.
The implemented
...
eligibility from release automation: a certificate that an individual can buy is not necessarily usable from a
GitHub-hosted unattended build.

| Route | Current evidence | Delivery effect | Evaluation |
|---|---|---|---|
| [HARICA Code Signing IV](https://www.harica.gr/en/code-signing/) | A Greek EU authority offers individual-validated code signing from €59 per year. Its [individual guide](https://www.harica.gr/en/guides/code-signing-certificate-request-for-individuals-or-sole-proprietorships-iv/) describes identity validation and a cryptographic device prepared and shipped by HARICA. | Removes `Unknown publisher` and lets Authenticode identity accumulate reputation. The documented hardware-token path requires a controlled signing host and custody step; it is not a drop-in GitHub-hosted signer. | Best currently orderable European certificate candidate for an individual, subject to confirming the complete checkout price, Spanish identity validation, token middleware, and non-interactive release feasibility before purchase. |
| [Certum Open Source Code Signing](https://www.certum.eu/en/code-signing-certificates/) | A Polish EU authority offers a one-year open-source certificate to individuals. The main catalogue starts at €25; the [cloud listing](https://shop.certum.eu/open-source-code-signing-on-simplysign.html) starts at €49 and the [card set](https://shop.certum.eu/open-source-code-signing.html) at €69. Both shop listings currently report unavailable stock. | The cloud route requires SimplySign applications, online access, and access-code authorization; the card route uses Certum's cryptographic card and reader. Neither published route currently proves unattended hosted release signing. | Economically attractive and aligned with GPL distribution, but not actionable while the official products are unavailable and their release-automation boundary remains unproven. Recheck only when native signing is reopened. |
| [Camerfirma code signing](https://shop.camerfirma.com/certificado-digital-de-firma-de-codigo) | The Spanish offering costs €505 excluding VAT, is currently unavailable, and is issued to entities, companies, or other legal persons rather than an individual publisher. | Would add Authenticode identity but does not fit the current owner or cost boundary. | Rejected for the bootstrap project. |
| Transparent unsigned Windows preview | No certificate purchase or external signing operator is required. Existing independent updater signatures, release checksums, provenance, immutable release origin, SBOM, and package evidence can still authenticate FitFreed-controlled distribution. | Microsoft documents an initial `Windows protected your PC` warning with `Run anyway`; enterprise policy can prohibit continuation. Smart App Control can block unknown unsigned code entirely. Unsigned file reputation also restarts for every new binary version. | Technically viable only as a clearly labelled preview, not as parity with the signed stable macOS release. It is the proportionate way to test presently unproven Windows demand if the owner accepts the installation boundary. |

The recommended bootstrap route is a transparent unsigned **Windows preview**, while macOS and Linux remain the
stable supported release. This recommendation avoids recurring certificate and custody work before any Windows user
has expressed interest, but it does not hide the resulting platform friction. It becomes selectable only through a
later ADR and must meet all of these conditions:
...
4. Stable macOS and Linux claims remain unchanged. Windows is not described as equally trusted or universally
   installable until native signing is restored and admitted.
5. The download page provides a direct, account-free feedback route. No telemetry is added merely to measure demand.
6. Native signing is reconsidered before promoting Windows from preview to stable, or when real Windows feedback
...
| Field | Boundary |
|---|---|
| Acceptance outcome | Admit the release-shaped Windows package against the shared cold-launch, import, reimport, dense-history, Insights, report, export, filesystem-safety, and recovery budgets without repeating already accepted capability or update campaigns. |
| Exact evidence | Exact source `fb802e6` is synchronized and passes [repository safety run `34296600382`](https://github.com/purnalica/fitfreed/actions/runs/34296600382). Focused [Windows update run `34296638466`](https://github.com/purnalica/fitfreed/actions/runs/34296638466) passes all six exact native prerequisites and the complete release-shaped scenario set. Source `52abcb0` shares the already accepted bounded, revalidated application-data cleanup policy between isolated and production package lifecycles. Manual [Windows performance run `34302149928`](https://github.com/purnalica/fitfreed/actions/runs/34302149928) completes all 100 fresh-data launches but rejects p95 `3,363.939 ms` against the unchanged `2,500 ms` limit. Exact source `d66b3a5` then builds and installs its production NSIS package in [run `34307326155`](https://github.com/purnalica/fitfreed/actions/runs/34307326155), but the campaign stops after one process connects its startup channel and supplies no interactive-shell signal within ten seconds. Exact source `38c6c09` separates product continuation from the retained one-shot frame observation; [run `34309405574`](https://github.com/purnalica/fitfreed/actions/runs/34309405574) completes all 100 launches without missing evidence but rejects p95 `3,402.271 ms`. Exact source `7257e8d` defers React and emits only `13.20 kB` of eager JavaScript. [Run `34312164102`](https://github.com/purnalica/fitfreed/actions/runs/34312164102) builds and installs that exact package and completes all 100 launches. It rejects the unchanged budget with median `2,193.122 ms`, p95 `2,772.774 ms`, and maximum `6,534.367 ms`. Phase p95 is `14.46 ms` for process creation and evidence transport, `943.744 ms` for host startup through setup completion, `2,044.763 ms` for setup completion through renderer startup and command transport, `192 ms` for locale initialization, and `13 ms` for final paint signaling. All later reliability and data benchmarks are correctly skipped after that first rejection. |
| Independent CI condition | Exact source `23b0f1e` passes [repository-safety run `34333179144`](https://github.com/purnalica/fitfreed/actions/runs/34333179144) and complete [continuous-integration run `34333179226`](https://github.com/purnalica/fitfreed/actions/runs/34333179226), including dependency audit, portable and native tests, lint, formatting, documentation, workflow, repository-content, identity, and secret gates. Its impact classifier skips unchanged packaged platform campaigns. |
| Native result | Focused [Windows performance run `34315586139`](https://github.com/purnalica/fitfreed/actions/runs/34315586139) builds and installs exact source `43b04ff`, then admits all 100 fresh-data launches with median `1,650.595 ms`, p95 `2,175.046 ms`, and maximum `4,922.876 ms` against the unchanged `2,500 ms` p95 budget. Phase p95 is `17.678 ms` for process creation and evidence transport, `844.954 ms` for host startup through setup completion, `1,435.172 ms` for setup completion through renderer startup and command transport, `163.1 ms` for locale initialization, and `12.2 ms` for final paint signaling. Exact descendant `18e2bc1` moves the synthetic library from the system-owned VHD root to a dedicated application-data child. Focused recovery-and-data [run `34321543655`](https://github.com/purnalica/fitfreed/actions/runs/34321543655) independently reopens the retained package and cold-launch evidence, skips both accepted gates, and then stops before the native NTFS test because the clean checkout lacks Tauri's generated `src-tauri/icons/icon.ico`. Exact descendant `8d295d6` makes that shared command self-contained and passes complete [continuous-integration run `34322990457`](https://github.com/purnalica/fitfreed/actions/runs/34322990457). Corrected [run `34323967060`](https://github.com/purnalica/fitfreed/actions/runs/34323967060) again reopens the retained evidence, generates the icons, compiles the release-mode host, and passes all five ordinary native filesystem tests. The disk-exhaustion test then reports a successful additional import instead of the required failure; later data gates remain skipped. Exact source `8c012f5` corrects that test precondition and passes [continuous-integration run `34327554022`](https://github.com/purnalica/fitfreed/actions/runs/34327554022) plus [repository-safety run `34327554036`](https://github.com/purnalica/fitfreed/actions/runs/34327554036). Focused [run `34328676018`](https://github.com/purnalica/fitfreed/actions/runs/34328676018) stops in the retained-evidence preflight before environment setup or native execution because the verifier cannot read the accepted test source. Exact descendant `8db5b9f` replaces that unbounded source capture with exact Git blob identities and passes complete [continuous-integration run `34329193435`](https://github.com/purnalica/fitfreed/actions/runs/34329193435). Focused [run `34330393200`](https://github.com/purnalica/fitfreed/actions/runs/34330393200) then reopens the retained evidence, skips package construction and all 100 accepted cold launches, and passes all five ordinary native filesystem tests. Its isolated NTFS filler receives Windows disk-full with one cluster still reported free, so the zero-reported-cluster precondition rejects the test before the product import executes; later data gates remain skipped. Exact source `23b0f1e` closes that model and [recovery-and-data run `34334118449`](https://github.com/purnalica/fitfreed/actions/runs/34334118449) independently reopens the accepted package and cold-launch evidence without rebuilding either. All ordinary filesystem cases and the isolated real disk-exhaustion, rollback, integrity, and retry path pass. Seven fresh full-scale imports of 10,000 artifacts and `5,453,138,315` expanded bytes pass at p95 `337,436.774 ms`; exact repeat passes at p95 `350.902 ms`, paired queries at p95 `13.249 ms`, and peak memory at p95 `20.863 MiB`. Three fresh dense histories containing 520 sessions and `7,490,080` signal samples pass first import at p95 `57,820.347 ms`, equivalent changed-package reimport at p95 `33,853.134 ms`, every read boundary below `76 ms`, peak memory at p95 `14.527 MiB`, and database size at `134,696,960` bytes. The 10-year Insights model passes every one of its 100-run interaction distributions; its largest p95 is `1,120.696 ms` for self-contained report export against `2,000 ms`, and peak memory is `16.547 MiB`. |
| Causal conclusion | Critical-resource inlining closes the Windows cold-launch boundary without relaxing any measurement or timeout. The dedicated application-data child and self-contained icon generation correct the first two harness defects. Run `34323967060` reaches the intended test and proves a third precondition defect: the filler stops when a cached one-mebibyte write or flush returns Windows disk-full, but it never measures the clusters still available to the caller. The small additional SQLite transaction fits in that residual allocation and correctly succeeds. The write-through correction consumes every allocation unit that NTFS permits the filler file to claim. Run `34330393200` proves that the terminal `GetDiskFreeSpaceW` count is not itself a writable-capacity oracle: NTFS rejects the final allocation-unit write with disk-full while still reporting one free cluster. Requiring a literal zero therefore confuses reported volume state with an observed failed allocation and prevents the recovery behavior from being tested. The import must instead begin only after an allocation-unit-aligned write covering the complete reported remainder has failed with Windows disk-full without making more reported capacity available. The unchanged import result must then independently prove whether SQLite is denied the space it needs. Run `34334118449` validates that closed exhaustion model against native NTFS and proves the unchanged product behavior, recovery, and all prevented data budgets. Run `34328676018` exposes a separate preflight implementation limit: Node's synchronous child-process capture defaults to one mebibyte, while `src-tauri/src/infrastructure.rs` is 1,281,649 bytes. `git show` therefore terminates with `ENOBUFS` before hashing even though the complete-history checkout contains the accepted object. Git already provides a bounded exact content identity for both committed blobs, so materializing either source is unnecessary. |
| Smallest falsifying test | The portable filler policy must distinguish exhaustion caused by an allocation-unit write rejected as disk-full from a clean zero-cluster termination. Every successful write must strictly reduce reported free clusters; a disk-full write may leave the terminal count unchanged or reduce it after a partial write, but must never increase it. The ignored native test must establish one of those closed exhaustion outcomes before invoking the unchanged import and must still require SQLite to return disk-full. |
| Allowed change | Correct only the test-only Windows filler termination contract and its focused portable tests so observed disk-full allocation is a valid terminal exhaustion signal. Update the retained-evidence identity allowlist and the current execution record. Preserve production code, application-data and ACL contracts, 64 MiB NTFS capacity, package and cold-launch implementation and accepted metrics, benchmark budgets, fixtures, update behavior, platform scope, and release topology. |
| Evidence retained | Focused run `34060294715` accepts native Windows package construction, installation, inventory, removal, and application-data preservation. Exact source `b94a8f4` accepts packaged Windows capability parity. Focused run `34061136225` accepts packaged Linux Insights and package removal. Run `34296638466` closes native Windows update recovery on exact source `fb802e6`. Run `34315586139` accepts Windows cold launch. Run `34334118449` independently reopens that immutable evidence and accepts the native filesystem-recovery and complete data-performance boundary on exact descendant `23b0f1e`. No accepted capability, update, package, or cold-launch evidence was repeated. |
| Exit or stop | The hosted Windows Server engineering admission is complete. Preserve its source-bound evidence and do not repeat it for documentation-only descendants. Audit M5.1, M5.3, M5.5, and M5.6 against the current exact-candidate and promotion prerequisites; continue any independent work still absent, then stop only at the documented production-authority, predecessor-release, clean Windows 11, human-acceptance, or publication gates that cannot be supplied from the repository. |
...
| M5.6 Exact candidate and promotion | In progress for 0.1.23 after rejected 0.1.22 | The manual Windows workflow builds and seals the unsigned native input on `windows-2025`, composes one protected complete-platform candidate, admits its exact Linux and Windows packages before promotion, and publishes only after a second protected approval. Immutable public 0.1.12 satisfies the predecessor dependency. Workflows `35102076630` through `35270281675` rejected immutable 0.1.13 through 0.1.22 at progressively later authority-safe boundaries without publishing a Release or Pages snapshot. Version 0.1.22 passed both native inputs, protected Apple signing and notarization, portable sealing, and both Ubuntu admissions; its Windows admission proved that repeated exact-PID activation through a campaign-long helper does not correct the intermittent loss of painted-shell evidence. Version 0.1.23 uses one fresh pre-timing activation helper per sample while retaining exact-PID activation until the signal, complete process-tree cleanup, the timeout, sample count, p95 budget, package, and product behavior. Exact-source CI, immutable tag, protected approvals, candidate execution, immutable publication, Pages convergence, and remote verification remain. |
...
requires the persisted PID, creation `FILETIME`, executable, nonce, and target native identity. The role-specific lock
verifier validates owned handles directly, reopens available roles, and treats only an exact Windows sharing collision
as peer activity, so the watchdog and candidate can coexist without either acquiring the other's authority. The hosted
native lane remains the execution authority for those operating-system APIs. Candidate confirmation additionally requires the held lease, active launch
...
file ACLs and no reparse descendants. The second slice creates a bounded 64 MiB NTFS VHD only on the elevated disposable runner, derives its
real allocation unit and reported free clusters, and uses write-through allocation-unit-aligned writes until zero
free clusters prove actual exhaustion. It then requires failed-import invisibility, restores capacity, exercises ordinary startup
...
Exact source `d66b3a5` builds and installs the production NSIS package in focused run `34307326155`. One cold-launch
process connects its startup channel but supplies no interactive-shell signal within the unchanged ten-second
benchmark timeout, so the campaign has no aggregate result and skips every later gate. History traces the regression
to the one-second product-continuation fallback introduced with the painted-shell diagnostic: the progressive root
arms that boundary earlier than the former complete application, and the timeout canceled its sole animation-frame
observation. A transient WebView2 paint suspension could therefore become permanent missing evidence. The corrected
lifecycle keeps product continuation non-blocking at one second while retaining one pending frame observation; a
later frame reports exactly once, and unmounting still cancels it. Focused component tests failed before this change
and pass after it. No benchmark timeout, percentile budget, sample size, or evidence definition changes.

Exact source `38c6c09` retains the pending animation-frame observation after the one-second product-continuation
boundary. Focused run `34309405574` completes all 100 launches without missing evidence and reduces the maximum from
`8,037.087 ms` to `3,910.572 ms`, closing that lifecycle defect. It still rejects performance: median is
`2,058.387 ms` and p95 is `3,402.271 ms`. The dominant p95 remains the setup-complete to renderer-startup and command-
transport interval at `2,517.245 ms`; the final paint interval is only `7.2 ms`. Production source-map inspection
shows that React DOM is the overwhelmingly largest remaining eager source and is evaluated before the renderer timing
origin. The current bounded correction renders and signals the localized shell through a framework-independent
adapter, reduces eager JavaScript to `13.20 kB`, and then loads React and the complete application without changing
any benchmark or product boundary.

Exact source `7257e8d` passes all 100 startup evidence exchanges in focused run `34312164102`. Framework deferral
reduces the dominant setup-to-renderer phase p95 from `2,517.245 ms` to `2,044.763 ms` and total p95 from
`3,402.271 ms` to `2,772.774 ms`; it is effective but not sufficient for the unchanged `2,500 ms` budget. The
generated HTML still retrieves the `13.20 kB` eager module and `10.30 kB` critical stylesheet as separate blocking
asset-protocol resources before the localized shell can paint. The current bounded correction inlines byte-identical
copies into the generated HTML while retaining the versioned entry required by deferred shared imports and guarding
that second module identity against duplicate startup. The production build now emits a `24.11 kB` HTML document,
retains the `13.35 kB` entry and `10.30 kB` stylesheet, and verifies exact inline equality with no external startup
module or stylesheet tag. Native evidence remains pending for the changed source.
...
Immutable `v0.1.22` at `6124af1fc38c43a5c18d3406e36afa9eb729f4b8` passed exact-source CI, both authority-free
native input jobs, protected complete-platform composition, Apple signing and notarization, portable sealing, and
both Ubuntu admission rows in workflow
[`35270281675`](https://github.com/purnalica/fitfreed/actions/runs/35270281675). Its Windows admission repeatedly
reasserted activation for every exact PID through the same campaign-long `WScript.Shell` helper and still observed one
connected host without painted-shell evidence inside ten seconds. This falsifies focus loss after a single activation
as the explanation for 0.1.21. Version 0.1.19 painted all 100 samples while using a fresh helper per launch, making the
shared automation session the remaining lifecycle difference introduced immediately before the failures began.
Promotion was skipped and no Release or Pages deployment began. The next candidate creates a fresh helper before each
