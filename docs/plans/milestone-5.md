...
| Field | Boundary |
|---|---|
| Acceptance outcome | Preserve distinct, simultaneous Windows watchdog and candidate authority from replacement launch through candidate confirmation, while retaining atomic state snapshots and no-sharing exclusion. |
| Exact evidence | Commit `6f3824c` passes the focused local contracts and strict lint. Focused [run `34169564831`](https://github.com/purnalica/fitfreed/actions/runs/34169564831) then passes portable quality, native prepared-state durability, and the new native held-watchdog phase-read precondition. Its release-shaped success scenario installs 0.2.0 and loses the expected WebDriver session when 0.1.0 exits, but no `updated` outcome appears within 120 seconds. The prior watchdog-startup diagnostic is absent. The replacement application's setup acquires `candidate.lock` and `state.lock`, then its complete verifier tries to reopen the live watchdog's `watchdog.lock`. On Windows that peer authority is deliberately held with `share_mode(0)`, so candidate validation fails before startup confirmation. Commit `4e17575` implements the role-specific verifier and passes every applicable local gate. Focused [run `34218746825`](https://github.com/purnalica/fitfreed/actions/runs/34218746825) stops at the first native compile boundary, before either the simultaneous-lease test or package construction: the Windows-only lock opener validates its `File` but returns `Ok(())` after the helper extraction, contradicting its `Result<File, ...>` contract. The regression is confined to that Windows-only return expression; the compiler rejects every Windows library or test build that includes it. |
| Causal hypothesis | The metadata-validation extraction in `4e17575` correctly separated validation from opening on Unix, but copied the helper's unit return into the Windows opener instead of returning the already validated file. Returning that exact handle restores the declared contract without changing sharing, validation, ownership, or lifecycle behavior. The native simultaneous-lease test can then evaluate the original no-sharing hypothesis. |
| Smallest falsifying test | The existing native prepared-state durability command is the smallest Windows type-checking boundary and must compile and pass first. The immediately following focused recovery-state contract must then hold the watchdog lease, publish the exact replacement process and nonce, acquire one candidate lease while rejecting a second, read `launching` through the watchdog, confirm through the candidate, and read `confirmed` through the still-held watchdog. No package is built unless both native tests pass. Existing exclusivity, mismatched-identity, transition, watchdog protocol, and diagnostic assertions remain intact. |
| Allowed change | Return the already opened and validated `File` from the Windows-only lock opener and record the rejected native evidence. Do not change lock sharing, lock-role expectations, state serialization, atomic manifest writes, process identity, package contents, installation behavior, transitions, deadlines, user-visible copy, or any accepted path boundary. |
| Evidence retained | Focused run `34060294715` accepts native Windows construction, installation, inventory, removal, and application-data preservation. Exact source `b94a8f4` accepts packaged Windows capability parity. Focused run `34061136225` accepts packaged Linux Insights and package removal. Focused run `34092790974` accepts icon preparation and native durability. Focused run `34141473809` locates the setup-wrapper identity defect. Focused run `34145475055` reaches a durably published recovery set. Focused run `34163763512` accepts ordinary/verbatim path identity. Focused run `34166654611` identifies the owning-watchdog lease-reopening defect. Focused run `34169564831` accepts its correction and reaches installed-candidate startup. None of those accepted boundaries may run again in this iteration. |
| Exit or stop | Run only the focused recovery-state, workflow, documentation, repository-content, and secret checks needed by this one-line correction. Commit and push it, cancel the redundant push-triggered broad workflow, and dispatch `windows-update` exactly once. Native prepared-state durability and the simultaneous-lease precondition must pass before package construction. Stop at the first new failure or accept the complete recovery journey; do not rerun blindly. Do not run `windows-capability`, local E2E, any Linux or macOS package campaign, Windows performance, or a complete candidate before this boundary closes. |
