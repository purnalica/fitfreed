...
In progress as of 2026-09-07, with independent portability engineering authorized by ADR 0039. Public promotion
...
| Field | Boundary |
|---|---|
| Acceptance outcome | Preserve authenticated x86 NSIS setup wrappers and require the runnable predecessor application to remain x86-64, without weakening product, version, digest, path, or installed-identity validation. |
| Exact evidence | Focused [run `34141473809`](https://github.com/purnalica/fitfreed/actions/runs/34141473809) on exact source `83115fa` skips every unrelated platform campaign, passes the exact Windows-native prepared-state durability regression, builds only the ordinary 0.2.0 and gated 0.1.0 packages, installs and launches the predecessor, and reaches an authorized update. The retained backend diagnostic then identifies `package-and-runnable-preparation: InvalidPackageIdentity` before `active` is published. Commit `8307f66` introduced one shared validator requiring x86-64 PE identity from both NSIS setup wrappers and the installed runnable. Tauri 2.11.4 builds its NSIS container from an x86 Unicode stub while `x64` describes the installed payload target. The adapter therefore rejects the authentic wrapper before it can preserve either package. |
| Causal hypothesis | Separating setup-wrapper identity from runnable-application identity will admit only the expected x86 NSIS PE container while retaining the x86-64 requirement for the copied FitFreed executable. Product name, description, file version, product version, byte length, digest, canonical path, installed registration, and no-reparse validation remain unchanged. |
| Smallest falsifying test | The package adapter must accept x86 setup identities for the predecessor and candidate together with an x86-64 runnable predecessor, and must reject either machine type when presented in the opposite role. The existing focused recovery-state unit test must still publish and reopen one complete attempt. Formatting, strict lint, documentation, repository-content, and secret checks must pass. No local or unrelated packaged E2E campaign is permitted. |
| Allowed change | Distinguish the PE machine identity of an NSIS setup wrapper from that of the installed runnable application, update the synthetic package-port evidence accordingly, and correct the current architecture and testing documentation. Do not change file durability, path equivalence, metadata, reparse-point, digest, product identity, version identity, installation identity, process identity, transition, timeout, update, or user-interface behavior. |
| Evidence retained | Focused run `34060294715` accepts native Windows construction, installation, inventory, removal, and application-data preservation. Exact source `b94a8f4` accepts packaged Windows capability parity. Focused run `34061136225` accepts packaged Linux Insights and package removal. Focused run `34064970864` accepts both NSIS builds, predecessor installation, update discovery, and authorization. Focused run `34092015048` accepts isolation and fail-fast ordering. Focused run `34092790974` accepts icon preparation and the exact Windows-native durability regression. Focused run `34141473809` accepts the deferred failure-only build and locates the first real preparation rejection. None of those accepted boundaries may run again in this iteration. |
| Exit or stop | Run only the focused package-adapter and recovery-state tests locally. After the correction is verified and pushed, dispatch `windows-update` exactly once. Stop at the first new failure boundary or accept the complete recovery journey; do not retry blindly. Do not run `windows-capability`, local E2E, any Linux or macOS package campaign, Windows performance, or a complete candidate before this boundary closes. |
