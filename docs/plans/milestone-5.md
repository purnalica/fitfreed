...
macOS application directory whose link metadata cannot be reproduced on hosted Windows. Version 0.1.17 makes the
sealed boundary the portable public distribution, admits manifest versions 6 through 8 on Linux, and omits redundant
immediate verification after digest-bound extraction. Exact-source CI, immutable tag, protected approvals, native
...
| Allowed change | Prepare, verify, tag, and publish version 0.1.17 through the implemented unsigned public trust profile. Keep rejected 0.1.13 through 0.1.16 immutable and unpublished. Remove or archive SignPath-specific material only after the mandatory full dependency inventory. Preserve application behavior, updater and release-checksum authority, accepted native and E2E evidence, performance budgets, and stable macOS-plus-Linux claims. |
