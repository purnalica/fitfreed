...
|---|---|---|
| M5.0 Native portability admission | In progress | Exact source `fff7df8` passes repository safety, the complete portable and Linux-host job, installed Linux capability, and packaged Linux update recovery in hosted run [`33977220148`](https://github.com/purnalica/fitfreed/actions/runs/33977220148). Exact correction `26a231d` passes repository safety, while [hosted run `33993588621`](https://github.com/purnalica/fitfreed/actions/runs/33993588621) exposes a pre-existing component-test race before platform jobs start: the range-list heading can render before its selected-summary effect invokes the application command. The current tree waits for that exact invocation, preserving the complete query assertion, and passes the focused test plus all 493 presentation tests with a constrained two-worker pool. Native Windows acceptance still awaits the corrected exact hosted run. |
...
| M5.3 Packaged capability parity | In progress | Exact source `fff7df8` builds and installs the isolated NSIS package, then its first packaged launch fails closed while protecting the local-library parent. Stage-specific evidence proves that the exclusive verified directory handle was being reopened to request owner authority, which Windows forbids while the original no-sharing handle remains open. The correction requests owner authority on the initial handle and plans DACL-only, DACL-plus-owner, rejection, or no-op behavior before mutation. Shared capability parity and the remaining Windows-visible paths await corrected native execution. |
| M5.4 Windows performance and reliability admission | In progress | The manual hosted performance workflow, production NSIS cold-launch lifecycle, portable Windows benchmark paths, native peak-memory measurement, isolated NTFS disk-exhaustion recovery, and version 2 private-library filesystem boundary are implemented and locally contract-tested. Exact source `fff7df8` does not reach the Windows measurement campaign because the earlier host and packaged-startup gates fail. Corrected native execution and exact Windows 11 evidence remain open. |
