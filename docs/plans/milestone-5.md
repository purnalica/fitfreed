...
|---|---|---|
| M5.0 Native portability admission | In progress | Exact source `5da60b7` passes [repository safety `33998318356`](https://github.com/purnalica/fitfreed/actions/runs/33998318356), the complete portable and Linux-host job, installed Linux capability, and packaged Linux update recovery in [hosted run `33998318315`](https://github.com/purnalica/fitfreed/actions/runs/33998318315). Native Windows compilation exposes two test-only POSIX leaks before running the host suite. The current tree confines the Linux signal adapter to Unix test hosts, constructs synthetic process status through the executing target's standard-library extension, removes the resulting Windows lint failures, and passes all 423 Rust tests plus strict Clippy locally. Native Windows acceptance awaits the corrected exact hosted run. |
...
| M5.3 Packaged capability parity | In progress | Exact source `5da60b7` builds, installs, and launches the isolated NSIS application, then passes source acquisition, import, English exploration, training discovery, session workbench and evidence, report composition, and report-library export. The journey stops only because its report-removal assertion retains an element handle obtained before the asynchronous notice exists. The current tree uses the established atomic live-region query while preserving deletion, history integrity, duplicate independence, and every later assertion; the complete shared packaged E2E campaign passes locally. Exact hosted Windows continuation remains open. |
| M5.4 Windows performance and reliability admission | In progress | The manual hosted performance workflow, production NSIS cold-launch lifecycle, portable Windows benchmark paths, native peak-memory measurement, isolated NTFS disk-exhaustion recovery, and version 2 private-library filesystem boundary are implemented and locally contract-tested. Exact source `5da60b7` does not reach the Windows measurement campaign because the packaged capability journey stops first. Its macOS cold-launch p95 is 3,034 ms against the unchanged 2,500 ms budget, after exact predecessor `5e531e4` passed at 2,433 ms on the same hosted profile; no budget is weakened and the next exact campaign must independently pass. Corrected native execution and exact Windows 11 evidence remain open. |
