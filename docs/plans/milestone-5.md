...
| M5.4 Windows performance and reliability admission | In progress | The manual hosted performance workflow, production NSIS cold-launch lifecycle, portable Windows benchmark paths, and native peak-memory measurement are implemented and locally contract-tested. Native performance and filesystem reliability runs remain open. |
...
starts. The native hosted run, Windows filesystem-failure matrix, and exact Windows 11 measurements remain open.
