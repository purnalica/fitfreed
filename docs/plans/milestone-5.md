...
|---|---|---|
| M5.0 Native portability admission | In progress | The first hosted run exposed Git line-ending conversion in checksum-governed vendored source before native compilation. Exact source `e4026bd` passed that boundary and the portable Node.js entry points, then exposed CRLF conversion in domain source. Exact source `16bf050` passed that parser boundary and exposed native Windows separators in a repository-relative documentation reference, PowerShell 7 module-path inheritance through the Node.js package adapter, and a host-scoped Cargo inventory that omitted the direct Windows production dependency from macOS release evidence. The changed tree distinguishes portable repository identifiers from native filesystem paths, isolates Windows PowerShell behind the existing native environment allowlist, inventories every target-specific Cargo production branch independently of the generating host, pins every detected text file to LF while preserving binary bytes, and retains locked Node.js package entry points without platform-specific npm shims. Local portability tests pass; acceptance awaits successful immutable hosted runs for the exact revision. |
