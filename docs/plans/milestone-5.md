...
In progress as of 2026-09-09, with independent portability engineering authorized by ADR 0039. Public promotion
...
| Evidence retained | Focused run `34060294715` accepts native Windows package construction, installation, inventory, removal, and application-data preservation. Exact source `b94a8f4` accepts packaged Windows capability parity. Focused run `34061136225` accepts packaged Linux Insights and package removal. Run `34296638466` closes native Windows update recovery on exact source `fb802e6`. Run `34300497374` proves the named-pipe signal reaches the harness. Run `34302149928` provides the first complete Windows cold-launch distribution. Run `34307326155` isolates the frame-observation lifecycle defect. Run `34309405574` proves that defect corrected and isolates the eager framework boundary. Run `34312164102` proves framework deferral. Run `34315586139` accepts Windows cold launch and isolates the system-owned VHD-root mismatch. Run `34330393200` independently reopens all retained evidence and proves the remaining failure is confined to the terminal NTFS pressure assertion. No accepted capability, update, or cold-launch evidence will be repeated. |
| Exit or stop | Prove the closed filler termination contract and exact source identity locally. Commit and synchronize only that focused test-harness correction. Dispatch one corrected recovery-and-data scope with run `34315586139`; its preflight must reject drift before only the failed filesystem gate and the data gates it prevented execute. The accepted package build and 100-process cold-launch campaign must not be repeated. Stop at the first new failure and do not run capability, update, Linux, macOS, local packaged E2E, or a complete candidate while this boundary is pending. |
...
| M5.4 Windows performance and reliability admission | In progress | The manual hosted performance workflow, production NSIS cold-launch lifecycle, portable Windows benchmark paths, native peak-memory measurement, isolated NTFS disk-exhaustion recovery, and version 2 private-library filesystem boundary are implemented and locally contract-tested. Exact source `57a96ec` passes packaged Windows Insights performance plus the complete portable, Linux-host, Linux capability, Linux update-recovery, and macOS performance campaigns. Manual [run `34032179775`](https://github.com/purnalica/fitfreed/actions/runs/34032179775) rejected unavailable GUI-subsystem standard output; [run `34300497374`](https://github.com/purnalica/fitfreed/actions/runs/34300497374) then exposed duplicated cleanup-policy drift. Exact source `52abcb0` shares the bounded cleanup policy and [run `34302149928`](https://github.com/purnalica/fitfreed/actions/runs/34302149928) completes all 100 fresh-data launches, but rejects p95 `3,363.939 ms`. Exact source `38c6c09` corrects the progressive shell's frame-observation lifecycle and [run `34309405574`](https://github.com/purnalica/fitfreed/actions/runs/34309405574) completes all 100 launches but rejects p95 `3,402.271 ms`. Exact source `7257e8d` defers React, reduces eager JavaScript to `13.20 kB`, and [run `34312164102`](https://github.com/purnalica/fitfreed/actions/runs/34312164102) lowers p95 to `2,772.774 ms`. Exact source `43b04ff` inlines only the critical entry and stylesheet; [run `34315586139`](https://github.com/purnalica/fitfreed/actions/runs/34315586139) accepts 100 Windows launches at p95 `2,175.046 ms`, then exposes that the disk-exhaustion harness incorrectly uses the system-owned VHD root as the product's user-owned library parent. The focused correction moves only the synthetic library beneath a dedicated application-data child before resuming the prevented reliability and data gates. Exact Windows 11 evidence remains open. |
