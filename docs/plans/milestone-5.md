...
| Field | Boundary |
|---|---|
| Acceptance outcome | Route explicit acknowledgement of a retained Windows update result through the native Windows recovery-state boundary so the validated receipt is removed only under its platform lock and authority checks. |
| Exact evidence | Exact source `e57053d` passes the complete local gate. Focused [run `34268776525`](https://github.com/purnalica/fitfreed/actions/runs/34268776525) accepts the real transient-file-sharing test added for the preceding hypothesis, then fails identically in the release-shaped `success` journey. Privacy-safe artifact `10073943945` again records `notice-verification`, absent active and attempt state, retained `updated`, installed 0.2.0, one installed application process, and no predecessor process. Source tracing identifies command `acknowledge_update_recovery_notice`, introduced by `92492ec`, as unconditionally invoking the generic Unix/macOS recovery adapter. On Windows that adapter reaches its `cfg(not(unix))` lock implementation and returns `UnsupportedPlatform` before receipt deletion. The transient-deletion hypothesis is therefore refuted. |
| Causal hypothesis | The application command is missing the platform dispatch already used by startup confirmation and maintenance. The Windows-native state module owns the no-sharing outcome lock and private-directory validation, but exposes no acknowledgement operation; the command therefore calls an adapter that cannot run on Windows. This explains the deterministic failure before deletion and the unchanged receipt. |
| Smallest falsifying test | Complete a synthetic Windows recovery through `OutcomeRetained`, invoke a new Windows-state acknowledgement operation, require the validated receipt to disappear, and require a second acknowledgement to report no outcome. Execute that exact test on native Windows before package construction. The unchanged release-shaped journey then verifies the command dispatch. |
| Allowed change | Add acknowledgement to the Windows recovery-state adapter using its existing canonical private-directory validation, exclusive outcome lock, exact receipt parsing, and receipt-bound attempt-absence check. Dispatch the Tauri command to that adapter on Windows and retain the current generic adapter on Unix platforms. Remove the refuted transient-deletion retry, its test, and its documentation. Do not change update phases, package authority, terminal cleanup, application identity, user-visible behavior, or any E2E assertion. |
| Evidence retained | Focused run `34060294715` accepts native Windows construction, installation, inventory, removal, and application-data preservation. Exact source `b94a8f4` accepts packaged Windows capability parity. Focused run `34061136225` accepts packaged Linux Insights and package removal. Focused runs through `34261252899` locate and close earlier native lifecycle boundaries. Run `34268776525` accepts the native transient-sharing behavior but refutes it as the release-shaped failure cause. None of those accepted boundaries may run again in this iteration. |
| Exit or stop | Run only the Windows recovery-state acknowledgement test, affected Rust and command tests, focused workflow and documentation contracts, and repository safety gates. Commit and push only after those gates pass, cancel the redundant push-triggered broad workflow, and dispatch `windows-update` exactly once. Stop at the first new failure; do not rerun blindly. Do not run `windows-capability`, local E2E, any Linux or macOS package campaign, Windows performance, or a complete candidate before this boundary closes. |
