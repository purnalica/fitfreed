...
| Causal conclusion | The named-pipe signal and shared WebView2 quiescence cleanup are effective. Static import-graph and production-bundle inspection traced the remaining measured interval to the renderer eagerly loading the complete application graph, both complete locale catalogs, and the complete application stylesheet before it could paint and report the shell. The failed source emitted `391.38 kB` of initial JavaScript and `224.21 kB` of initial CSS. The progressive boundary now emits `206.41 kB` and `10.30 kB` respectively, applies persisted language, appearance, and zoom to a real navigable shell, then loads the complete application. The `2,500 ms` p95 budget remains unchanged. |
| Smallest falsifying test | Focused component tests prove persisted localized startup, navigation retention during deferred loading, non-blocking signal transport, no-frame continuation, preference failure, and complete-application handoff. The production build walks the eager entry closure and rejects `App`, the complete application stylesheet, or either complete catalog. The final falsification remains one 100-process native Windows campaign; no capability, update, Linux, macOS, or local packaged E2E campaign is required. |
| Allowed change | The correction remains inside the proven setup-to-renderer startup boundary and preserves application behavior, package identity, painted-shell signal, timer boundaries, 100-process sample, `2,500 ms` p95 budget, fixtures, cleanup policy, update behavior, platform scope, and release topology. No measurement or budget was relaxed. |
...
| Exit or stop | Pass the complete applicable local source, documentation, formatting, repository-content, secret, and diff gates. Commit and synchronize one coherent correction, then dispatch `windows-performance` exactly once. Stop at the first new failure and do not rerun blindly. Do not run local E2E, `windows-capability`, `windows-update`, Linux or macOS package campaigns, or a complete candidate while this focused boundary is pending. |
