...
| Field | Boundary |
|---|---|
| Acceptance outcome | An explicit `linux-insights` dispatch builds and installs the isolated Debian package, runs only the packaged Insights scenario, preserves its synthetic library through removal, and retains privacy-safe failure evidence. It cannot repeat the functional, restart, adaptive-session, or sport-catalogue scenarios. |
| Exact evidence | Commit `3328ea6` corrects only the chart resize paint race and its packaged geometry oracle; commit `6d0b51f` adds the closed single-scenario dispatch. Candidate run `34049176597` already passes the complete portable and Linux host, Debian construction and installation, Linux update recovery, functional journey, process restart, adaptive session composition and restart, and sport-catalogue recognition and restart before failing only in packaged Insights. Exact descendant `ae59828` changes only the independent Windows inventory adapter after those Linux inputs. |
| Causal hypothesis | ECharts updates canvas geometry synchronously during `resize()` but schedules the visible zrender paint through its animation stage, which an occluded Linux WebView may suspend. Flushing zrender immediately after resize makes the handle paint geometry match the resized canvas before WebDriver interaction. The closed `performance` selection can prove that correction without repeating six accepted scenarios. |
| Smallest falsifying test | The chart lifecycle contract requires one resize followed by one zrender flush, and the packaged geometry oracle rejects handles painted outside the bottom fourteen percent of the resized canvas. The hosted boundary must generate the Insights fixture, install the same Debian package, execute exactly the named `performance` scenario, and preserve the library through removal. |
| Allowed change | No product or automation change is admitted before this exact hosted run. If it fails, only the retained packaged Insights evidence may define a new causal iteration. E2E assertions, package identity, native installation/removal behavior, budgets, and other scenario jobs may not change here. |
| Evidence retained | The 19 affected chart tests, 27 packaged-driver contracts, production build, closed scenario-selection contracts, workflow validation, documentation, presentation inventory, UI and performance policies, repository-content check, and secret scan pass. The Windows package boundary independently passes in focused run `34060294715`; neither it nor any previously accepted Linux scenario may run again. |
| Exit or stop | Dispatch `linux-insights` exactly once and exit when its one packaged scenario and package lifecycle pass. Stop on its first failure and record a new causal iteration. Do not run `linux-capability`, any unrelated E2E scenario, or a complete candidate. |
