...
| M4.2 | Correction under verification | Linux recovery version 2, stable channel version 3, upgrade matrix version 2, recoverable Linux manifest version 5, exact predecessor discovery, Rust authorization, public build mapping, atomic staging, complete candidate/Pages verification, native package identity and fixed candidate/rollback commands, full `/proc` process identity, bounded authenticated predecessor transfer and no-clobber package handoff, exact package preservation, bounded Debian extraction, deterministic runnable-image verification, private atomic version 2 attempt preparation, packaged lifecycle policy, serialized transitions, process-bound launch records, preserved-watchdog context resolution, exact exclusive process leases, candidate-bound target confirmation, quiescent prepared-attempt discard, atomic library restoration, verified native rollback, closed three-attempt failure accounting, shared terminal receipts, exact-pair terminal cleanup, receipt-bound restart resumption, host installation coordination, private watchdog/candidate startup dispatch, verified runnable fallback, active-attempt restart resolution, duplicate-watchdog exclusion, interrupted-phase reattachment, privacy-minimized intervention discovery, and bounded explicit native-recovery retry exist. The native Debian E2E definition covers successful replacement, automatic rollback after either native installer failure or candidate rejection, authorization-unavailable runnable fallback, explicit retry after authorization becomes available, three-attempt exhaustion with manual reinstall guidance, and restart resumption after both live actors are terminated at durable `replacement-started`. The first hosted package build exposed an obsolete display-name filename assumption before any scenario started. The corrected build then exposed that its shared Cargo target retained the already normalized candidate beside the generated predecessor. After package isolation was corrected, success, installer failure, and candidate failure passed; authorization retry restored the predecessor pair, database, and locale but a transport assertion incorrectly included the later notice-verification restart and its ordinary update discovery. The harness now requires Tauri's technical output, applies the production byte-preserving normalizer, consumes each version into the isolated scenario store before building the next, and uses an explicit terminal-recovery handshake to keep the strict offline assertion separate from post-recovery notice verification. Exact hosted Ubuntu evidence for the corrected campaign remains the admission gate. |
| M4.3 | Correction under verification | The hosted installed Debian application has completed the primary import journey, process restart, adaptive-session composition and restart, and sport-catalogue recognition and restart without retries. The subsequent 1024-by-720 performance journey exposed a fixed-width activity value column that clipped a valid localized count at 200% text size. A rebuilt packaged macOS performance journey proves the intrinsic-width correction across every domain and budget; exact hosted WebKitGTK completion remains the admission gate. |
