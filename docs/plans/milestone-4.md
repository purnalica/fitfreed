...
- Linux public promotion remains blocked until the exact 0.1.9 Linux candidate passes its own gates.
- The Linux expansion uses the next unreleased semantic version after that immutable macOS Release and contains newly
  ...
  is 0.1.9; its release policy advances the stable sequence to 9 and retains 0.1.7 as the exact macOS predecessor.
...
- **Production correction:** both graphical gates in the supported Ubuntu admission matrix use the confirmed bounded
  ...
  acquire that correction, and its source-bound provenance cannot be recreated from `main`; the next patch candidate
  must carry it through the existing exact-tag publication pipeline.
