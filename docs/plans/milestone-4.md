...
- The Linux expansion uses the next unreleased semantic version after that immutable macOS Release and contains newly
  ...
  is 0.1.12; its release policy advances the stable sequence to 12 and retains 0.1.7 as the exact macOS predecessor.
  Sequences 9, 10, and 11 remain consumed by authenticated but unpublished immutable candidates and are not reused.
- Exact candidate creation, native admission, and public promotion remain protected workflow gates. Ordinary
  engineering continues without them.
...
**Acceptance evidence:** documentation and localized-content checks, contributor commands, synthetic complete-platform
candidate reopening, remote-snapshot verification, and support-link contracts pass without production authority.
Reopening and remote convergence for the production-authority candidate remain part of M4.6 rather than being inferred
...
- **Disproof:** run
  [`34780144942`](https://github.com/purnalica/fitfreed/actions/runs/34780144942) added only `at-spi2-core`. The warning
...
- **Confirmed owner:** run
  [`34781734823`](https://github.com/purnalica/fitfreed/actions/runs/34781734823) reopened the same signed candidate and
  observed its interactive shell on hosted Ubuntu 24.04 after 6.25 seconds total, of which 5.90 seconds belonged to
  package setup; renderer startup through the interactive signal completed in 273 milliseconds. The package, library,
  and source revision were unchanged. This confirms that the original admission failure belongs to its incomplete
  graphical test session rather than FitFreed or the Debian package.
- **Production correction:** both graphical gates in the supported Ubuntu admission matrix use the confirmed bounded
...
- **Hosted readiness variance:** immutable 0.1.10 workflow
  ...
  first-launch observation remains separate and unchanged. Read-only diagnostic run
...
- **Installed first-launch variance:** immutable 0.1.11 workflow
  ...
  Read-only diagnostic run `34827996617` reopened the retained 0.1.11 bytes on Ubuntu 26.04 and passed in
  `9,338.862 ms`. Hosted desktop startup and setup accounted for `8,821.798 ms`; renderer locale and interactive
  signaling accounted for `297 ms`. This admits the corrected availability observer before the 0.1.12 successor is
  built, without changing the separate p95 product-performance budget.
