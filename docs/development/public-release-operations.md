...
| A product workflow and release workflow deployed distinct artifacts with the same revision identity | GitHub may report the release deployment successful while the public origin still serves the earlier complete snapshot | Commit the version-gated correction without changing the exact product-site source used by the candidate. Reconstruct the update snapshot from authenticated immutable Release assets, never from the stale Pages origin. Its new revision may deploy because source and public versions match. Verify the public bytes, then re-run only the original release run's failed remote-verification job. Never rebuild or replace Release assets. |
