...
The command audits the complete JavaScript production and build graph, verifies the candidate-bound upgrade matrix, builds the normal Tauri production package, rejects E2E instrumentation and build-host paths across the complete application bundle, generates and validates production-only CycloneDX inventories, normalizes machine-local Cargo references, scans the complete evidence set for secrets, and stages it under `.artifacts/releases/<version>/`. The production build wrapper embeds the exact Git revision and clean-tree state used by the application startup signal, remaps Rust compiler source locations to stable virtual prefixes, preserves explicit encoded compiler arguments, and refuses a source change during the build.
