...
| Dense-history import, storage, session-page, overview, or exact-page budget fails | Archive mapping, SQLite physical storage, or application query boundary | Run `npm run benchmark:dense-history` once and retain its privacy-safe JSON. Separate import phases, checkpointed database bytes, session discovery, bounded overview, and exact pagination before changing code. A storage failure is not a query-budget failure, and the accepted workload or budget must not be reduced to obtain a pass. |
