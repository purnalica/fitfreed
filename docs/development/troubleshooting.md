...
| Packaged Windows update does not replace, recover, resume, or clean up | Production-identity NSIS recovery boundary | Preserve `.artifacts/windows-update-e2e/evidence` and diagnose the first failed package action, recovery phase, or exact process check. Run `npm run verify:windows-update-e2e` only in a disposable native x86-64 Windows user with no FitFreed installation or application data. Pre-existing production state is a safety rejection, not test debris to delete; never point cleanup at a real library. |
