...
Training additionally measures the first 25-session complete-history discovery page, one source-separated
calendar month, and ordered resolution of four opaque session capabilities against one coherent snapshot.
These paths include the concrete SQLite adapter and application validation and use the common-interaction
budget.
