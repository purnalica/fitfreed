...
The complete sealed portable distribution then passes both Ubuntu admission rows and a separate secret-free `windows-2025` job.
...
The first protected job has read-only repository permission. After complete local candidate verification it seals only the portable public contents of `release/` and `pages/` into one transport archive, records its SHA-256 digest, retains it for seven days as a private Actions artifact, and unconditionally removes Apple and updater authority. The unpacked `release/FitFreed.app` directory is a composition intermediate, not a public asset, and is excluded; the signed updater archive and DMG remain included. `npm run pack:public-release -- <candidate> <archive>` verifies the complete local candidate, while `npm run unpack:public-release -- <archive> <sha256> <candidate>` verifies the resulting portable distribution and rejects mutation, additional roots, unsafe paths, partial extraction, or evidence drift.
