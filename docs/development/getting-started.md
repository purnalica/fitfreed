...
- `TrainingRangesPanel` owns the personal-range library, editor draft, atomic command lifecycle, optimistic-conflict recovery, guarded removal, and revision-bound summary disclosure. `training-session-range.ts` mirrors the public desktop transport, while `training-range-editor-model.ts` owns exact `BigInt` elapsed parsing, choice labeling, immutable established-coordinate lookup, and local validation. Presentation receives choices only from the application context, uses local selector indexes rather than rendering opaque capabilities, and re-queries durable state after remount. Do not introduce another range editor in the route workbench; map, signal, structure, and exact-evidence controls must compose the same state when R8.6 connects them. The [training exploration architecture](../architecture/training-exploration.md) and [personal range guide](../user/session-ranges.md) are canonical.
