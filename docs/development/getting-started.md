...
- `TrainingRangeInteractionProvider` owns the session-level personal-range query, selected durable range, editor draft, atomic command lifecycle, optimistic-conflict recovery, guarded removal, and revision-bound summary disclosure. `TrainingRangesPanel` composes its library and result, while the single `TrainingRangeEditor` form is shared with `TrainingRouteWorkbench`. `training-session-range.ts` mirrors the public desktop transport, while `training-range-editor-model.ts` owns exact `BigInt` elapsed parsing, choice labeling, immutable established-coordinate lookup, and local validation. Presentation receives coordinates only from the application context and re-queries durable state after remount. The route map projects exact returned point indexes and never snaps a typed boundary to nearby geometry. Signal, structure, and exact-evidence controls must compose the same controller and editor rather than introducing another command or draft. The [training exploration architecture](../architecture/training-exploration.md) and [personal range guide](../user/session-ranges.md) are canonical.
