...
- `TrainingRouteWorkbench` owns the semantic route controls and selection derived from one `SessionStory`; `route-workbench-model.ts` owns anti-meridian, direction, selection, and overlay projection; `route-viewport.ts` is the application-owned spatial port; and `leaflet-route-adapter.ts` is the sole allowed Leaflet import. The adapter receives bounded prepared points, performs no command, and must remain vector-only and local. Run `npm run check:ui-contracts`, the focused workbench and session-library tests, and packaged E2E after changing this boundary. [Training exploration architecture](../architecture/training-exploration.md), [ADR 0026](../architecture/decisions/0026-use-leaflet-for-the-local-route-workbench.md), and the [user route guide](../user/session-routes.md) are the canonical design, decision, and behavior references.
