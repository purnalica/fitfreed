...
Tauri's asset-CSP modification remains enabled and derives the exact inline-script hash during host compilation. The
configured `script-src` stays strict and never permits arbitrary inline scripts.

Normal production builds bind the exact Git revision and clean-tree state into the host through the versioned build wrapper. Startup has an explicit progressive boundary. Durable import recovery starts on a blocking worker while the WebView initializes; the application-preference load and import commands share a process-scoped completion barrier, and import and preference mutation remain disabled until recovery releases it. The renderer exposes only a neutral startup surface until the complete persisted preference set has been validated and recovered if necessary. The initial renderer graph contains a framework-independent bootstrap, an imperative shell adapter, critical shell styles, and build-generated shell-only projections of the canonical `en-US` and `es-ES` catalogs. Those projections are derived from the canonical catalogs and never become a second translation source. The production compositor places byte-identical copies of that entry and its critical stylesheet directly in `index.html`, so neither requires a separate asset-protocol request before the first shell. It retains the versioned files because deferred chunks import shared entry exports; a renderer-global claim prevents that second module identity from starting the application again. The build rejects ambiguous HTML resources, unsafe closing tags, source maps, missing retained assets, or a non-exact composition boundary. After loading preferences, the bootstrap applies the complete persisted language, appearance, and zoom set and reveals the localized shell with Home, History, Reports, Sources, and Settings as distinct destinations. History remains truthfully unavailable until the library read model establishes whether explorable evidence exists; the other destinations are real controls, and a selection made while the complete application loads is retained. The bootstrap waits for the next animation frame and invokes the host's one-shot interactive-shell signal. Only after dispatching that command does it load React, React DOM, the complete application, complete locale catalog, application stylesheet, source guidance, provider-neutral Library Home, latest import outcome, update discovery, update-recovery confirmation, and destination-specific presentation modules. React replaces the startup adapter under the existing typed application-startup contract and receives the latest selected destination. A build-time static-import-closure guard rejects React, React DOM, Scheduler, `App`, `ApplicationShell`, the complete application stylesheet, or either complete catalog in the eager entry graph. Deferred work never awaits diagnostic settlement, so a blocked output consumer cannot hold the product in a partial startup state. If a suspended renderer supplies no frame callback within one second, FitFreed continues loading the complete application without claiming painted-shell evidence while retaining the single pending frame observation. A later frame reports the shell exactly once, and a benchmark that receives no signal within its independent bound fails closed.
