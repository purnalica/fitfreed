...
9. On explicit installation, rerun steps 2 through 8 and require the requested candidate to equal the fresh installable release. Retain the authenticated sequence, payload digest, target schema, target, URL, expected size, SHA-256 digest, and package signature as one host-internal authorization.
10. Preserve and verify the current application and library, download through Tauri's signature-verifying updater, and compare the resulting bytes with the authorization's signed size and SHA-256 digest before native replacement.
11. Record the update as pending, launch a recovery watchdog, replace and relaunch the application, migrate the library transactionally, and mark success only after the new application opens the expected library and confirms its version and schema. Any owned failure restores the matching application and library pair.
