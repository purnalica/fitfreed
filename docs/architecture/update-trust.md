...
Current Milestone 2 implementation under [ADR 0008](decisions/0008-authenticate-update-policy-above-tauri.md). The versioned contracts, application policy, bounded HTTPS transport, exact-byte signature verification, restart-safe replay and notification state, fresh exact-package installation authorization, typed host orchestration, localized check and preference presentation, native bounded-download adapter, recovery-transition policy, atomic verified application/library recovery-pair preparation, preserved-executable watchdog runner, and replacement-side startup confirmation exist. Installer integration, restart-safe watchdog ownership, native replacement, periodic scheduling, and complete packaged update evidence are still being implemented. No real update endpoint or production signing key is configured.
...
The host routes the exact private watchdog invocation before Tauri initialization. The installer-side launcher starts only the preserved executable, disconnects input and diagnostics, and requires an exact process-bound readiness record within ten seconds. The runner accepts only `prepared`, observes the locked persisted lifecycle, enforces installation and confirmation deadlines, launches the exact installed executable behind a process-bound startup gate, stops only a child it owns, and allows an installation-timeout parent stop only while that identifier still names its direct parent. It restores the verified pair after atomically winning recovery authority and relaunches only after `recovered`. The candidate host accepts only the exact private recovery identifier and nonce shape, requires the matching startup record, validates and retains its process lease before ordinary startup recovery, and can win `launching → confirmed` only through the existing complete pair and destination validation. Confirmation failure retains the claim for diagnosis while the watchdog remains authoritative. The runner is not wired to an installation command yet. A restarted watchdog still refuses an inherited `launching` phase until it consumes the persisted process record and lease evidence; watchdog resumption, installer orchestration, and terminal cleanup remain active work.
