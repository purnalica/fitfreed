...
The host routes the exact private watchdog invocation before Tauri initialization. The installer-side launcher starts only the preserved executable, disconnects input and diagnostics, and requires an exact process-bound readiness record within ten seconds. The runner accepts only `prepared`, observes the locked persisted lifecycle, enforces bounded installation and confirmation deadlines, starts the exact installed executable with a recovery identifier, stops only a child it owns, and allows an installation-timeout parent stop only while that identifier still names its direct parent. It restores the verified pair after atomically winning recovery authority and relaunches only after `recovered`. The candidate host accepts only the exact private identifier shape, completes ordinary startup recovery, and can win `launching → confirmed` only through the existing complete pair and destination validation. Confirmation failure retains the claim for diagnosis while the watchdog remains authoritative. The runner is not wired to an installation command yet. A restarted watchdog currently refuses an inherited `launching` phase because replacement-process ownership cannot be proven; restart-safe ownership transfer, installer orchestration, and terminal cleanup remain active work.
