...
including a watchdog lease transferred at terminal handoff, before deleting the exact receipt-bound attempt. A restart
with no active pointer can resume only the attempt named by that receipt; busy process ownership defers maintenance and
invalid destinations retain all evidence. Watchdog orchestration, native Windows execution, and release-shaped
recovery evidence remain required before Windows recovery is operational.
