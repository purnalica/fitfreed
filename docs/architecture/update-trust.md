...
reopens the complete package pair and tree. Failed preparation removes only assets created by that operation. These
primitives are not yet the complete Linux recovery path: predecessor transfer, the version 2 manifest and library
pair, lifecycle integration, runnable fallback, and packaged failure recovery remain open in M4.2.
