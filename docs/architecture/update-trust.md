...
Current Milestone 2 implementation under [ADR 0008](decisions/0008-authenticate-update-policy-above-tauri.md). The versioned contracts, application policy, bounded HTTPS transport, exact-byte signature verification, restart-safe replay and notification state, fresh exact-package installation authorization, typed host orchestration, localized check and preference presentation, native bounded-download adapter, recovery-transition policy, and atomic verified application/library recovery-pair preparation exist. Release-shaped package transfer, native replacement, watchdog launch and restoration, periodic scheduling, and complete packaged update evidence are still being implemented. No real update endpoint or production signing key is configured.
