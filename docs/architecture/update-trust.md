...
Current public update delivery follows [ADR 0008](decisions/0008-authenticate-update-policy-above-tauri.md) and [ADR 0020](decisions/0020-compose-product-and-update-pages.md). The private-alpha version 1, public stable version 2, and recovery-capable stable version 3 contracts, application policy, bounded HTTPS transport, exact-byte signature verification, restart-safe replay and notification state, fresh exact-package installation authorization, typed host orchestration, localized launch, recurring and manual checks, preference and installation presentation, native bounded-download and replacement adapters, recovery-transition policy, atomic verified application/library recovery-pair preparation, restart-safe preserved-executable watchdog runner, replacement-side startup confirmation, terminal recovery cleanup, privacy-minimized outcome presentation, packaged success-and-recovery update journey, release-bound target-aware upgrade matrix, compile-time public trust boundary, atomic update-only Pages staging, and the complete product-site and update-channel compositor exist. Public macOS 0.1.7 serves the live sequence 8 stable-v2 snapshot authenticated by `stable.primary-1`. The proposed 0.1.10 macOS-plus-Linux expansion retains that updater trust, selects stable-v3, and declares 0.1.7 as its exact macOS recovery baseline. Its independent release-checksum authority is active, while its exact candidate remains open in the [Milestone 4 ledger](../plans/milestone-4.md). [ADR 0011](decisions/0011-schedule-update-discovery-in-the-desktop-host.md) defines the implemented periodic schedule. [ADR 0012](decisions/0012-publish-two-dimensional-upgrade-support.md) separates real application release baselines from direct library-schema migration.
...
Explicit result acknowledgement is a later, independent platform filesystem boundary. On Windows it runs through the
Windows recovery-state adapter rather than the generic Unix adapter: it revalidates the private recovery root,
exclusively opens the native no-sharing outcome lock, reopens the closed receipt, proves its receipt-bound attempt is
absent, and removes only that receipt. Success remains the durable absence of the receipt; any error leaves the valid
receipt and visible result intact. Linux and macOS retain their existing Unix lock boundary.
...
requires the recovered critical files and library to equal their preserved counterparts. It writes the shared minimal
outcome receipt before removing the active pointer. Windows then closes every owned no-sharing attempt handle,
including the watchdog lease transferred at terminal handoff, before deleting the exact receipt-bound attempt. A busy
