...
SQLite owns bounds, distinct origin catalogs, inclusive indexed summary retrieval, exact sleep detail retrieval, canonical facts, and provenance but no report rule. Sleep overview and comparison avoid loading high-resolution transition rows; selecting one available night retrieves its complete ordered timeline. Nightly recovery currently has indexed canonical range and identity retrieval; its application read models and host transport are the active next slice. The Tauri transport validates typed range and detail boundaries and encodes integer facts and signed changes as decimal strings to preserve precision in JavaScript. React owns the localized activity, training, and sleep journeys, including accessible visual and exact-table alternatives, explicit optional-measurement coverage, and disposable detail and comparison state.
