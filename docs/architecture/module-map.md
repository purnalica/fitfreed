...
The first promoted slice recognizes a deliberately small synthetic daily-activity shape. Its current canonical concept, source mapping, and persistence schema are indexed in the [data format documentation](../data-formats/README.md). The application now queries terminal outcomes through a dedicated provider-neutral read port and presents complete artifact coverage and actionable failures for that boundary. Stable Polar subject identity, richer daily activity, full real-export family classification, and the remaining MVP contexts are not implied by this foundation. They enter through the provider adapter and application ports without weakening the dependency direction above.
