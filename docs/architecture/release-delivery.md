...
The Pages compositor has two closed modes. Product-only composition renders the checked-in inactive site without
download links. Release-candidate composition receives the already closed manifest and derives the exact versioned
GitHub Release installer links for its macOS, Linux, and Windows target set. It also renders release-current status
copy in both locales. These generated links neither create nor authorize a Release: the candidate Pages tree remains
unreachable until every applicable platform gate passes and promotion publishes the exact GitHub Release as immutable
before deploying that same Pages snapshot.

Product composition and deployment are separate decisions under
[ADR 0048](decisions/0048-reserve-unreleased-pages-revisions.md). The ordinary product-site workflow always validates
and composes its complete tree, but it deploys only when the repository package version equals the latest immutable
public Release version. A revision whose version is not yet public is reserved for the protected exact-tag workflow,
preventing two distinct Pages artifacts from sharing one Git revision as their external deployment identity. A later
public-version revision may deploy changed product sources only after recomposing them with the authenticated current
Release snapshot.
...
WebdriverIO 9.30.1 currently accepts Puppeteer's browser-management package version 2, whose unmaintained `extract-zip` dependency has no patched release for its symlink traversal advisory. FitFreed overrides that transitive package to version 3.2.0, which no longer contains `extract-zip`; the packaged Tauri E2E journey verifies the embedded-driver path against the newer dependency. WebdriverIO 9.30.1 also requests `deepmerge-ts` 7, whose recursive-object handling has a stack-exhaustion advisory, so FitFreed overrides it to 8.0.1; WebdriverIO only uses the compatible `deepmerge` and `deepmergeCustom` APIs, and the packaged journey verifies the configuration and driver paths. Mocha 10.8.2 requests `serialize-javascript` 6, so FitFreed overrides it to 7.1.0, which contains the published code-injection and denial-of-service corrections. Mocha and the SBOM generator also resolve `js-yaml` through compatible version-4 ranges; the explicit `4.3.2` override retains those APIs while excluding the merge-key denial-of-service advisory. Vitest is fixed at `4.1.11` so its runner, mocker, snapshot, formatting, and assertion packages remain one compatible revision containing the redirect-mock path-traversal correction. The matching nested override prevents the optional broad Vitest peer declared by `@testing-library/jest-dom` from selecting a different major revision during npm graph resolution. These overrides and exact tool revisions remain explicit compatibility obligations until upstream dependency ranges remove the need for them.

CycloneDX evidence uses the locked npm graph and the default-feature Cargo graph without development or build dependencies. Cargo generation explicitly selects all targets, making the release-scoped inventory independent of the generating host and retaining platform-specific production branches such as the Windows native boundary. `cargo-cyclonedx` emits absolute file references for local workspace packages; preparation rewrites those references and their dependency edges to stable `pkg:cargo` identifiers, then rejects any remaining repository path or `file://` value. Validation compares every target's direct Cargo manifest production dependencies with the SBOM root edges. A development-only crate blocks staging when it appears on a root edge, while the same crate remains valid when reached only through a normal production dependency; pruning that transitive component would make the inventory false. Missing direct production components and missing license declarations also block staging.
...
GitHub Releases owns the immutable human-facing packages and evidence record. The canonical `https://fitfreed.org/` GitHub Pages origin serves the English fallback and locale negotiation at its root, deterministic Spanish product content beneath `/es/`, and the current stable signed envelope with its exact versioned updater packages beneath `/updates/`. One compositor generates localized product pages from the canonical English source and complete locale catalogs, then combines them with any byte-preserved update snapshot so product publication and release promotion cannot replace one another's files. After channel activation, product-only publication reads the current immutable GitHub Release, verifies its exact assets and source-bound provenance, sources current update objects from that Release, and sources any declared recovery package from its exact immutable predecessor Release. It never treats mutable Pages bytes as update authority. A final preflight preserves an identical live snapshot, permits only a higher authenticated sequence, and rejects erasure, replay, or same-sequence replacement. Product-only deployment additionally requires the source version to equal the immutable public version; an unreleased revision remains reserved for its exact release deployment. Public release preparation is manual and exact-tag-bound; secret-free verification precedes a protected environment that alone may access production signing credentials. The ordinary application stays unconfigured, while a public build embeds the stable endpoint and public trust set at compile time.

The checked-in [public update configuration](../data-formats/release/public-update-configuration-v1.md) contains the
reviewed public `stable.primary-1` trust key. Its activation is a trust-root change but supplies no private authority. A
public candidate wrapper strips inherited update values, maps only that complete configuration into compile-time
inputs, applies a closed Tauri overlay that creates updater artifacts, and adds a final command-scoped Tauri merge
containing only the explicitly selected canonical public key. No update endpoint enters Tauri configuration. Atomic
channel staging validates stable version 2, signs the exact payload through an injected
release authority, and promotes only a complete envelope/package pair. This engineering path is locally verified with
synthetic credentials; it does not create a public release or substitute for Apple trust evidence.
...
The final DMG is its own Apple distribution boundary. Tauri 2.11.4 signs the image it creates after finalizing the
application, but does not notarize and staple that later container. FitFreed therefore verifies the generated image,
re-signs it with the admitted Developer ID identity, a secure timestamp, and the stable
`org.fitfreed.desktop.dmg` identifier, submits those exact bytes to Apple's notary service, requires an accepted log
with no reported issues, and staples and validates the ticket. The macOS-only and both platform-expansion composers
invoke this same boundary before trust inspection, copying, hashing, manifest construction, or sealing.
...
Protected preparation repeats preflight, removes stale generated bundle output, and builds with path-based updater authority and one complete Apple notarization credential mode. The ephemeral installer supplies the same private absolute path through FitFreed's path input and the pinned Tauri bundle command's `TAURI_SIGNING_PRIVATE_KEY` input; it never supplies key contents inline, and a missing or mismatched pair fails before packaging. The protected parent retains both Tauri names for their distinct consumers, while the standalone stable-metadata signer child receives only `TAURI_SIGNING_PRIVATE_KEY_PATH` and the inherited password because Tauri maps the two private-key names to mutually exclusive options. The candidate builder independently resolves the selected updater-key identifier against the canonical active public trust set and passes that exact Base64-wrapped public key to Tauri through the last command-scoped configuration merge. This keeps key rotation explicit, leaves ordinary builds unconfigured, and prevents Tauri artifact signing from silently using empty or different public trust. Explicit final-DMG signing, notarization, log admission, and stapling precede objective trust inspection and evidence assembly. The ignored atomic candidate directory separates GitHub Release inputs from the deployable Pages artifact. The ADR 0020 compositor combines the verified update snapshot with the exact tagged localized product site and manifest-derived installer links before candidate verification, so a release cannot replace the site with an update-only tree or advertise an undeclared package. Stable metadata is signed over exact payload bytes without placing the updater password on a command line. Checksums, inventories, and manifest digests cover the final signed and notarized bytes; local-path and secret scans run before the directory is promoted. No network publication occurs during preparation.

[ADR 0019](decisions/0019-separate-candidate-build-from-public-promotion.md) closes the hosted authority sequence. Exact-revision CI, repository safety, origin tag, protected-environment, and HTTPS Actions-backed Pages evidence must pass before the GitHub-hosted Apple Silicon runner receives authority. Private inputs are materialized outside the checkout in an ephemeral keychain and private files, and cleanup restores runner state. The build job cannot publish: it seals the verified candidate, records its transport digest, retains it for separate exact-candidate evaluation, and removes authority. A second protected approval admits only those exact reopened bytes to promotion without Apple or updater secrets. Under [ADR 0047](decisions/0047-permit-bootstrap-solo-release-approval.md), the same accountable maintainer may approve both deployments while bootstrap governance remains active. Every checksum subject, the checksum inventory, and the detached platform-expansion checksum signature receive source-bound GitHub attestations. The still-private complete Pages artifact is uploaded before the exact draft is published, and the GitHub Release must become immutable before Pages can replace the stable snapshot. Final verification downloads all public Release bytes, validates provenance against the exact workflow, tag, and revision, and accepts every direct no-redirect product and update object only after the complete remote site converges to the tagged source and declared package digests.
