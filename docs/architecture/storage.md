...
The current implemented schema and compatibility boundary are documented in the [SQLite version 37 persistence specification](../data-formats/persistence/sqlite-v37.md). It preserves the complete provider-neutral fitness library through version 8, authenticated-update state from version 9, application preferences from version 10, a resumable exploration destination from version 11, user-authored sport classifications from version 12, coherent full-history training discovery evidence from version 13, the disposable training-discovery workspace from version 14, mapped training exercises, laps, and pauses from version 15, primary and transition route assessments with exact points from version 16, regular temporal training signals with exact unavailable slots from version 17, reusable personal segmentation criteria from version 18, provider-recorded training-zone assessments from version 19, durable versioned report definitions from version 20, composable route-report intent from version 21, user-selected training-period report queries from version 22, question-, exploration-, session-, and blank-origin reports from version 23, compact exact-signal storage from version 24, durable user-authored training-session ranges from version 25, exercise-owned range coordinates with lossless legacy preservation from version 26, exact exercise, route, or signal coordinate authority from version 27, immutable provider-sport catalogue evidence plus active selection from version 28, exact session-scoped training-target sport evidence from version 29, lazy migration from source-profile discovery filters to exact represented-session filters from version 30, revisioned provider-neutral planned-training intent with provenance, reconciliation, conflicts, and favourite snapshots from version 31, single-library planned-training report definitions from version 32, truthful absence for unnamed planned phases from version 33, independent package-identity evidence on import outcomes from version 34, compatibility-preserving conversion of combined training-sport workspace filters from version 35, explicit user-authored relationships between represented sport collections from version 36, and reusable private sport-correlation selectors from version 37. Earlier specifications remain immutable migration history.
...
Schema version 10 renames the locale-only row to `application_preference` and extends it with a fixed preference-contract version, system/light/dark appearance, and 100%–200% content zoom. The application reads, validates, recovers, saves, backs up, and restores the complete persisted set atomically. Preview state, a default-valued draft, and the selected Settings category remain presentation-only. Moving between categories preserves the draft. **Restore defaults** changes that draft without writing; **Save changes** is the sole preference write; **Cancel changes** restores the persisted values without writing. Requested navigation away from an unsaved draft proceeds only after an explicit discard.
...
Schema version 31 adds planned-training targets as a provider-neutral, revisioned aggregate separate from recorded
sessions, source laps, calculated segments, and authored ranges. Current heads point to immutable evidence-and-mapping
revisions containing ordered exercises, ordered phases, goals, intensity zones, transitions, and bounded repeat
graphs. Provenance records every import decision; explicit conflicts retain both revisions while preserving the prior
visible head. Favourite exports are immutable ordered snapshots, including an explicit empty snapshot, so absence
from a later export never destroys earlier target history. Private source sport codes and unmapped source-field
locators remain infrastructure evidence; source values that are not mapped are not promoted into canonical data.

Schema version 32 rebuilds only `report_definition` and `report_block`. It retains every preceding definition and
block byte-for-byte at the logical contract, adds `planned-training` as a version-5 origin and block kind, accepts
`planned-snapshot-` capabilities for that origin, and stores one matching `planned-target-` capability in both origin
and block. Domain restoration owns semantic equality and the one-evidence-library invariant. Report rows never copy
planned target structure, and missing imported evidence never cascades into deletion of user-authored reports.

Schema version 33 rebuilds only `planned_training_phase`. It preserves every named phase and all structural
relationships while making the source-authored name nullable. Canonical queries and portable export retain absence;
localized ordinal display labels remain presentation-only.

Schema version 34 adds one nullable, closed-vocabulary package-identity field to `import_operation`. The adapter
records it after inspecting the ZIP inventory and before the complete protection scan. A later safety or resource
rejection therefore retains both the package identity and its independent terminal reason without storing member
names, local paths, or personal source evidence.

Schema version 35 rebuilds only `training_discovery_workspace` so it can retain legacy versions 1 and 2 while
writing current version-3 state. Loading either legacy version maps source-profile and formerly combined filters to
the complete set of current exact and unresolved collections represented by that profile. The conversion preserves
all other disposable restoration state and never rewrites imported evidence or personal classification.

Schema version 36 adds the user-authored unified-sport relationship header and member tables. One member can occur in
at most one active relationship; header and member mutations advance the existing training-discovery revision in the
same immediate transaction. The application restores complete domain aggregates and fails closed on invalid rows.
Import and reimport never write these tables, and removing a relationship does not modify source evidence, personal
classification, reports, sessions, or represented-session filter capabilities.

Schema version 37 adds private source selectors and reviewed-member-to-selector relationships for the reusable local
matching rule in ADR 0037. A save derives them only from current reviewed groups inside the relationship transaction;
query projection applies them to compatible groups introduced by later imports without changing user revision. The
selectors remain adapter-owned sensitive metadata and never cross application or presentation ports. Migration from
version 36 derives only relationships that current evidence can prove and rolls back on selector overlap.
