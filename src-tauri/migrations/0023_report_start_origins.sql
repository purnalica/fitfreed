CREATE TABLE report_definition_v23 (
    ...
    origin_kind TEXT NOT NULL CHECK (origin_kind IN ('session', 'question', 'exploration', 'blank')),
    ...
    definition_version INTEGER NOT NULL CHECK (definition_version IN (1, 2, 3, 4)),
    ...
    CHECK (
        origin_baseline_from IS NULL OR (
            ...
            AND julianday(origin_comparison_through) - julianday(origin_comparison_from) BETWEEN 0 AND 365
        )
    ),
    CHECK (
        (
            origin_kind = 'session'
            AND origin_session_ref IS NOT NULL
            ...
            AND origin_session_ref IS NULL
...
CREATE TABLE report_block_v23 (
    report_ref TEXT NOT NULL REFERENCES report_definition_v23 (report_ref) ON DELETE CASCADE,
    ...
    CHECK (
        (
            ...
            AND session_ref IS NOT NULL AND include_physiological_context IS NOT NULL
            ...
            kind = 'route'
            AND session_ref IS NOT NULL AND include_physiological_context IS NULL
...
INSERT INTO report_definition_v23 (
...
INSERT INTO report_block_v23
SELECT * FROM report_block;

DROP TABLE report_block;
DROP TABLE report_definition;

ALTER TABLE report_definition_v23 RENAME TO report_definition;
ALTER TABLE report_block_v23 RENAME TO report_block;
