...
pub enum Error {
    ...
    #[error("`{0}`")]
    Network(String),
    /// Downloaded package size did not match the caller's authenticated expectation.
    #[error("downloaded package size does not match the authenticated expectation")]
