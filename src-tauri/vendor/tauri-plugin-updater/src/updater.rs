...
impl Updater {
    /// Prepares an update from release metadata authenticated by the caller without
    /// performing an endpoint request or applying the configured version comparator.
    pub fn prepare_update(
        ...
        let target = self
            ...
            .as_deref()
            .map(str::to_owned)
            .or_else(|| updater_os().map(str::to_owned))
    ...
    pub async fn check(&self) -> Result<Option<Update>> {
        ...
        let update = if should_update {
            Some(self.prepare_update(release, raw_json.unwrap())?)
        } else {
            None
        };
...
impl Update {
    ...
    pub async fn download<C: FnMut(usize, Option<u64>), D: FnOnce()>(
        ...
        self.download_inner(None, on_chunk, on_download_finish)
            .await
    }

    /// Downloads the updater package with an authenticated exact byte expectation,
    /// verifies its signature, and returns it as bytes.
    ///
    /// The transfer stops before copying a chunk that would exceed `expected_size`.
    pub async fn download_with_expected_size<C: FnMut(usize, Option<u64>), D: FnOnce()>(
        &self,
        expected_size: u64,
        on_chunk: C,
        on_download_finish: D,
    ) -> Result<Vec<u8>> {
        self.download_inner(Some(expected_size), on_chunk, on_download_finish)
            .await
    }
    ...
    async fn download_inner<C: FnMut(usize, Option<u64>), D: FnOnce()>(
        ...
        let expected_size = expected_size
            .map(|expected| validate_expected_download_size(expected, content_length))
            .transpose()?;
        ...
        while let Some(chunk) = stream.next().await {
            ...
            if let Some(expected_size) = expected_size {
                ensure_chunk_fits(buffer.len(), chunk.len(), expected_size)?;
            }
            on_chunk(chunk.len(), content_length);
            buffer.extend(chunk);
        }
        if expected_size.is_some_and(|expected| buffer.len() != expected) {
...
fn validate_expected_download_size(
    expected_size: u64,
    content_length: Option<u64>,
) -> Result<usize> {
    if expected_size == 0 || content_length.is_some_and(|actual| actual != expected_size) {
        return Err(Error::DownloadSizeMismatch);
    }
    usize::try_from(expected_size).map_err(|_| Error::DownloadSizeMismatch)
}

fn ensure_chunk_fits(current: usize, incoming: usize, expected: usize) -> Result<()> {
    if incoming > expected.saturating_sub(current) {
...
mod tests {
    ...
    fn validates_declared_and_streamed_package_size_before_copying() {
        assert_eq!(validate_expected_download_size(8, Some(8)).unwrap(), 8);
        assert!(matches!(
            validate_expected_download_size(8, Some(7)),
            Err(Error::DownloadSizeMismatch)
        ));
        assert!(matches!(
            validate_expected_download_size(8, Some(9)),
            Err(Error::DownloadSizeMismatch)
        ));
        assert!(matches!(
            validate_expected_download_size(0, None),
            Err(Error::DownloadSizeMismatch)
        ));

        assert!(ensure_chunk_fits(0, 5, 8).is_ok());
        assert!(ensure_chunk_fits(5, 3, 8).is_ok());
        assert!(matches!(
